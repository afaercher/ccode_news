...
> Letzte Aktualisierung: 2026-09-07 06:00 UTC (**Crawl 07.09. 06:00 UTC — vierter Lauf in Folge ohne Bewegung, Montagmorgen UTC.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — die Git-Refs `v2.1.264` und `v2.1.265` antworten HTTP **404**, `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den drei Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze mit dem bekannten 60-Byte-Platzhalter-Body. `v2.1.262` bleibt HTTP **404** und damit dauerhaft übersprungen. Die Lücke seit dem letzten Publish beträgt **27,9 Stunden**. Der Zeitpunkt erklärt den Leerlauf vollständig: 07.09. 06:00 UTC ist **Sonntag, 23:00 Uhr US-Pazifikzeit** — nach dem im 18:00-Lauf belegten Muster ist der Sonntag ein echter Leertag, und das prognostizierte Fenster für v2.1.264 (Montag **17:00–22:30 UTC**) beginnt erst in elf Stunden. **Die Prognose ist zu diesem Zeitpunkt also weder bestätigt noch widerlegt.** **Fund dieses Laufs — der wöchentliche Digest hat genau ein Loch, und das ist kein Versäumnis der Doku:** Die Übersicht unter `whats-new` listet lückenlos Week 13 bis Week 34 — **mit einer einzigen Ausnahme: Week 31 (27.–31.07.2026) fehlt.** Der Sprung von Week 32 direkt auf Week 30 steht so sowohl auf der Übersichtsseite als auch im Doku-Index `llms.txt`, und `whats-new/2026-w31.md` antwortet HTTP **404** — sechs Wochen später ist nichts nachgetragen worden. Die Ursache ist in der npm-Registry nachweisbar: In **ISO-Woche 31 gab es null Publishes**. Die 239-Stunden-Lücke zwischen v2.1.220 (24.07. 23:11 UTC) und v2.1.221 (03.08. 22:16 UTC) deckt exakt diese Kalenderwoche ab. **Der Digest ist also release-getrieben, nicht kalendergetrieben** — ohne Releases keine Wochenseite, und die Woche wird auch nicht nachgereicht. Daraus folgt zugleich eine **Korrektur an den letzten fünf Crawl-Blöcken**: Der dort wiederholt genannte „Digest-Rückstand von drei Wochen“ war zu hoch gegriffen. `2026-w37` antwortet 404, weil **Woche 37 heute erst beginnt** — das ist kein Rückstand. Der echte Rückstand beträgt **zwei Wochen**, W35 und W36, und die sind alles andere als ereignisarm: **8 Publishes** in W35 (v2.1.242–v2.1.251) und **7** in W36 (v2.1.252–v2.1.263), zusammen die dichtesten Wochen der gesamten Serie. Dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08., v2.1.234–v2.1.239) weiterhin oben. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **sechsten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API, Git-Refs und der Doku-Index `llms.txt` per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Digest-Lücke Week 31 und korrigierter Rückstand). — Vorheriger **Crawl 06.09. 18:00 UTC — dritter Lauf des Sonntags, keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — der Git-Ref `v2.1.264` antwortet HTTP **404**, `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zum 06:00- und 12:00-Lauf (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze mit einem 60-Byte-Body — der bekannten Platzhalter-Zeile. Auch `v2.1.262` bleibt HTTP **404** und damit dauerhaft übersprungen. Die Lücke seit dem letzten Publish beträgt **15,9 Stunden**. **Fund dieses Laufs — der Sonntag ist der einzige Tag, an dem Claude Code praktisch nie ausgeliefert wird:** Über die **gesamte** Publish-Historie des npm-Pakets — **511 Versionen** seit Mai 2025 — fallen nach US-Pazifikzeit nur **8 auf einen Sonntag (1,6 %)** gegenüber 19 auf einen Samstag (3,7 %) und 87 bis 107 pro Werktag. Im laufenden Jahr 2026 sind es **2 von 226 (0,9 %)**, und der letzte Sonntags-Publish überhaupt war **v2.1.159 am 31.05.2026** — zum Crawl **97 Tage** her. Noch schärfer ist das Anschluss-Muster: Von den sechs Wochenend-Publishes seit Mai wurde **kein einziger** am Sonntag nachgefasst. Entweder folgte die nächste Version noch am selben Samstag (2.1.240 -> 2.1.241 nach 10,9 h) oder erst am **Montag zwischen 11:03 und 13:19 Uhr PT** (Lücken von 42,6 / 43,3 / 43,4 / 49,2 h). Damit ist der heutige Lauf statistisch der erwartbar leere: **Der Sonntag ist kein verzögerter Samstag, sondern ein echter Leertag.** Daraus folgt eine belastbare Prognose für v2.1.264 — dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08., v2.1.234–v2.1.239) weiterhin oben; `whats-new/2026-w35`, `-w36` und `-w37` antworten alle **HTTP 404**, der Digest-Rückstand bleibt bei rechnerisch drei Wochen. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der große Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **fünften Lauf in Folge** auf **03.09. 17:24 UTC** — seit Donnerstag nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Sonntags-Muster und Prognose für v2.1.264). — Ältere Crawl-Historie in den Git-Commits.)
