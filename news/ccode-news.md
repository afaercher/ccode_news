...
> Letzte Aktualisierung: 2026-08-15 12:00 UTC (Bestätigungs-Crawl 15.08. 12:00 UTC: **keine neuen Einträge** seit dem Crawl am 15.08. um 06:00 UTC. **v2.1.233** (14.08., 22:20 UTC GitHub-Release / 18:50 UTC npm-Publish) ist weiterhin die neueste CLI-Version — gegenbestätigt über GitHub-Releases-API (Top-Release unverändert v2.1.233, darunter v2.1.232/13.08. und v2.1.231/13.08.; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = 2.1.233, `stable` unverändert **2.1.224**; letzter Registry-Schreibzugriff unverändert 14.08. 22:19 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.233, unverändert die 20 bekannten Punkte) — **kein v2.1.234 und kein Release vom 15.08.** What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest — der Digest zur abgelaufenen Woche 33 mit v2.1.228–v2.1.233 steht damit weiterhin aus; `code.claude.com` im ersten WebFetch-Versuch erreichbar). Blog-Announcements Top unverändert **13.08.** („Claude Tag now reads even more of the room", darunter 12.08. Claude in Chrome / Cowork und 11.08. Compliance API) — **kein Eintrag vom 14. oder 15.08.**; die Quelle antwortet weiterhin mit HTTP 200. Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`, darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — weiterhin **kein Eintrag vom 12.–15.08.**, die längste Pause dieser Quelle in der laufenden Serie. Dabei bestätigte sich erneut die notierte Abruf-Regel: `platform.claude.com/docs/en/release-notes` lieferte in WebFetch HTTP 404 (per curl dahinter 200 über Redirect), erst `…/release-notes/overview` gab den Inhalt zurück — künftige Crawls sollten direkt die overview-URL nutzen. Alle vier Quellen gegengeprüft. — Vorheriger Crawl 15.08. 06:00 UTC: **v2.1.233 ist erschienen** (14.08., 22:20 UTC GitHub-Release / 18:50 UTC npm-Publish) — **20 Changelog-Punkte**, unten als eigene Sektion dokumentiert. Das Release ist vor allem eines der **Korrekturen am Vorgänger**: Zwei der in v2.1.232 gefeierten Bash-Permission-Änderungen — die Behandlung von **Cygwin-Symlinks unter Windows** und die **Permission-Prüfung von Eingabeumleitungen `< datei`** — wurden **zurückgerollt**; eine engere Fassung soll später zurückkommen. Die entsprechenden Einträge unter v2.1.232 in dieser Datei sind damit **nicht mehr wirksam** und oben unter v2.1.233 als Rücknahme vermerkt. Dazu passend ein **Windows-Auto-Mode-Fix**: Die v2.1.232-Regression ließ den Auto-Mode bei gewöhnlichem `cd <dir> && <befehl> > datei` ständig um manuelle Freigabe bitten — punktgenau zum Auto-Mode-Standard-Rollout am 14.08. Der auffälligste inhaltliche Eingriff: **Todo-/Task-Tracking-Tools** (`TaskCreate/Get/Update/List`, `TodoWrite`) stehen auf **Opus 4.8, Sonnet 5, Fable 5, Mythos 5 und neueren Modellen nicht mehr zur Verfügung** (`CLAUDE_CODE_ENABLE_TODO_TOOLS=1` holt sie zurück) — in der Aufzählung taucht mit **Mythos 5** ein bislang nicht dokumentierter Modellname auf. Ein weiterer **Windows-Sicherheits-Fix**: Pfade mit NT-Gerätepräfix **`\??\`** umgingen die UNC-Pfadvalidierung und öffneten damit einen **NTLM-Credential-Leak** — die zweite Lücke dieser Familie binnen einer Woche nach dem `\\?\`-Fix aus v2.1.229. Neu außerdem: **GitLab-Merge-Request-URLs** in `--worktree` und `claude agents` (`!N`-Notation), **`CLAUDE_CODE_TOOL_MEMORY_LIMIT`** (Memory-Cgroup für Bash-Kommandos unter Linux), **`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`** (WebFetch-Cache-TTL, Default unverändert 15 Minuten) und die Gateway-Einstellung **`forward_user_identity`** (Nutzeridentität als Header für Kostenzuordnung pro Nutzer). npm-Registry: `latest` = `next` = **2.1.233**, **`stable` von 2.1.223 auf 2.1.224 angehoben** (dritte stable-Bewegung binnen fünf Tagen); letzter Registry-Schreibzugriff 14.08. 22:19 UTC; offizielles `CHANGELOG.md` (oberster Eintrag 2.1.233) gegenbestätigt — alle drei Kanäle konsistent, **kein v2.1.234 und kein Release vom 15.08.**; **v2.1.230 bleibt übersprungen**. What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest — der Digest zur abgelaufenen Woche 33 mit v2.1.228–v2.1.233 steht damit weiterhin aus; `code.claude.com` im ersten WebFetch-Versuch erreichbar). Blog-Announcements Top unverändert **13.08.** („Claude Tag now reads even more of the room", darunter 12.08. Claude in Chrome / Cowork und 11.08. Compliance API) — **kein Eintrag vom 14. oder 15.08.**; die Quelle antwortet weiterhin mit HTTP 200. Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`, darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — weiterhin **kein Eintrag vom 12.–15.08.**, die längste Pause dieser Quelle in der laufenden Serie. Alle vier Quellen gegengeprüft. — Vorheriger Crawl 14.08. 18:00 UTC: **keine neuen Einträge** seit dem Crawl am 14.08. um 12:00 UTC. **v2.1.232** (13.08., 23:29 UTC GitHub-Release / 21:30 UTC npm-Publish) ist weiterhin die neueste CLI-Version — gegenbestätigt über GitHub-Releases-API (Top-Release unverändert v2.1.232, darunter v2.1.231/13.08. und v2.1.229/12.08.; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = 2.1.232, `stable` unverändert **2.1.223**; letzter Registry-Schreibzugriff unverändert 13.08. 23:29 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.232, unverändert die 50 bekannten Punkte) — **kein v2.1.233 und kein Release vom 14.08.** What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest — der Digest zur laufenden Woche 33 mit v2.1.228–v2.1.232 steht damit weiterhin aus; `code.claude.com` im ersten WebFetch-Versuch erreichbar). Blog-Announcements Top unverändert **13.08.** („Claude Tag now reads even more of the room", darunter 12.08. Claude in Chrome / Cowork und 11.08. Compliance API) — **kein Eintrag vom 14.08.**; die Quelle antwortet weiterhin mit HTTP 200, der 302-Redirect vom 13.08. bleibt eine einmalige Störung. Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`, darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — weiterhin **kein Eintrag vom 12.–14.08.**, die längste Pause dieser Quelle in der laufenden Serie. Alle vier Quellen gegengeprüft. — Vorheriger Crawl 14.08. 12:00 UTC: **v2.1.232 ist erschienen** (13.08., 23:29 UTC GitHub-Release / 21:30 UTC npm-Publish) — mit **50 Changelog-Punkten das umfangreichste Release der laufenden Serie**; unten als eigene Sektion dokumentiert. **Nachtrag zum ausgefallenen 06:00-Crawl:** Der Lauf vom 14.08. 06:00 UTC hatte die Einträge zu v2.1.232 sowie den Blog-Eintrag zu **Claude Tag** (13.08.) bereits vollständig geschrieben, kam aber nicht mehr zum Commit — sie werden mit diesem Commit nachgereicht. Bei diesem Crawl wurden die 50 Changelog-Punkte Punkt für Punkt gegen das offizielle `CHANGELOG.md` abgeglichen: **alle sind abgedeckt**, keine Lücke. Schwerpunkte von v2.1.232: **Subagent-Forking ist jetzt Standard** (`subagent_type: "fork"` erbt Konversation *und* Prompt-Cache, Nicht-Teammitglied-Agenten laufen in interaktiven Sessions im Hintergrund); **Cross-Session-Messaging wird alltagstauglich** (`@`-Mention im Prompt, `SendMessage` per bloßem Namen, maschinenweit eindeutige Session-Namen, `/config`-Zeile mit annehmen/zurückhalten/ablehnen); **GitLab-Support** (Secret-Redaction für neun Token-Familien plus vollständige Redaktion routbarer `glpat-`/`gldt-`-Tokens, `glab`-Config-Store wie `gh` geschützt, Plugin-Marketplaces inklusive verschachtelter Subgruppen); **Sicherheits-Fixes und Härtungen** (PowerShell-Bypass über `$PSDefaultParameterValues`, Cygwin-Symlinks in Git Bash, verschachtelte Git-Repos erben kein Trust mehr, Bash-Eingabeumleitungen `< datei` permission-geprüft, `/tmp`-Socket-Verzeichnis und Linux-Dateisystem-Sandbox gehärtet, `sandbox.ripgrep` nicht mehr aus Projekt-Settings überschreibbar); sowie **sieben Remote-Control-Fixes**, darunter der schwerwiegendste Fund der Serie: Eine **Bridge innerhalb einer Cloud-Session erbte deren Transkript und Credentials**. npm-Registry: `latest` = `next` = **2.1.232**, `stable` unverändert **2.1.223**; letzter Registry-Schreibzugriff 13.08. 23:29 UTC; offizielles `CHANGELOG.md` (oberster Eintrag 2.1.232) gegenbestätigt — alle drei Kanäle konsistent, **kein v2.1.233 und kein Release vom 14.08.** (v2.1.230 bleibt übersprungen). What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest; `code.claude.com` im ersten WebFetch-Versuch erreichbar). **Blog-Quelle wieder erreichbar:** `claude.com/blog-category/announcements` liefert wieder HTTP 200 — der beim 18:00-Crawl am 13.08. notierte 302-Redirect auf `claude.ai` war vorübergehend; Top-Eintrag jetzt **13.08.** („Claude Tag now reads even more of the room", darunter 12.08. Claude in Chrome / Cowork und 11.08. Compliance API), kein Eintrag vom 14.08. Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`, darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — weiterhin kein Eintrag vom 12.–14.08. Alle vier Quellen gegengeprüft. — Vorheriger Crawl 13.08. 18:00 UTC: **keine neuen Einträge** seit dem Crawl am 13.08. um 12:00 UTC. **v2.1.231** (13.08., 08:38 UTC) ist weiterhin die neueste CLI-Version — gegenbestätigt über GitHub-Releases-API (Top-Release unverändert v2.1.231, darunter v2.1.229/12.08. und v2.1.228/11.08.; **v2.1.230 fehlt weiterhin** — die Version wurde übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = 2.1.231, `stable` unverändert 2.1.223; letzter Registry-Schreibzugriff unverändert 13.08. 08:38 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.231, weiterhin nur der eine MCP-OAuth-Fix) — kein v2.1.232 und kein zweites Release am 13.08. What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest; `code.claude.com` im ersten WebFetch-Versuch erreichbar). Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`, darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — weiterhin kein Eintrag vom 12. oder 13.08. **Quellen-Ausfall: `claude.com/blog-category/announcements` war bei diesem Crawl nicht abrufbar** — die Seite antwortet mit **HTTP 302 auf `claude.ai/?redirect=claude.com`** (per curl dahinter HTTP 403); `claude.com/blog` und die Variante mit Trailing Slash verhalten sich identisch. Die deutschsprachige Spiegelung `claude.com/de/blog-category/announcements` liefert zwar HTTP 200, aber einen **veralteten, lokalisierten Teilbestand** (Top-Eintrag 12.05.2026) und taugt **nicht** als Ersatz. Als Gegenprobe für größere Ankündigungen wurde `anthropic.com/news` herangezogen: jüngster Eintrag dort ist der **07.08.** — kein Hinweis auf eine Ankündigung vom 12./13.08. Der Blog-Stand bleibt damit der vom 06:00-Crawl notierte: Top **12.08.** (Claude in Chrome / Cowork). Beim nächsten Crawl erneut versuchen; hält der Redirect an, ist `claude.com` per curl mit Browser-User-Agent weiterhin erreichbar (`sitemap.xml` liefert HTTP 200). Die übrigen drei Quellen gegengeprüft. — Vorheriger Crawl 13.08. 12:00 UTC: **v2.1.231 ist erschienen** (13.08., 08:38 UTC GitHub-Release / 08:27 UTC npm-Publish) — ein **einziger Fix**: Der MCP-OAuth-Login scheiterte an einem **Redirect-URI-Mismatch bei Servern mit vorregistriertem OAuth-Client** (Beispiel Slack). Das ist ein **Regressions-Fix zu v2.1.229**, wo die Redirect-URI von `localhost` auf `127.0.0.1` umgestellt wurde; unten als eigener Eintrag dokumentiert. **v2.1.230 wurde übersprungen** — die Version existiert weder auf GitHub noch auf npm noch im CHANGELOG. npm-Registry: `latest` = `next` = **2.1.231**, **`stable` von 2.1.221 auf 2.1.223 angehoben** (zweite stable-Bewegung binnen zwei Tagen, nach der ersten seit dem 25.07.); letzter Registry-Schreibzugriff 13.08. 08:38 UTC; offizielles `CHANGELOG.md` (oberster Eintrag 2.1.231) gegenbestätigt, alle drei Kanäle konsistent. **Nachtrag zum ausgefallenen 06:00-Crawl:** Der Lauf vom 13.08. 06:00 UTC hatte die Einträge zu **v2.1.229** (12.08., 20:56 UTC, 33 Changelog-Punkte) sowie zwei Nicht-CLI-Einträge (**Claude in Chrome: Seitenleiste ist jetzt eine Cowork-Session**, Blog 12.08.; **Response-Header `anthropic-workspace-id`**, Platform-Note 11.08.) bereits geschrieben, lief aber vor dem Commit in ein Timeout (HTTP 504) — sie werden mit diesem Commit nachgereicht. Schwerpunkte von v2.1.229: **Absturz-Serie** (nicht-textuelle Tool-Call-Werte und RangeError in schmalen Terminals machten Sessions auch beim `--resume` unbrauchbar, Windows-UNC/`\\?\`-Pfade), **Auto-Mode-Blocker** kurz vor dem Standard-Rollout am 14.08., **stiller CI-Ausfall** des `/install-github-app`-Review-Workflows, **Prompt-Cache-Staffelung** bei Workflow-Fan-outs, **Sandbox-IPv6 fail-closed** und **`/commit-push-pr`** genehmigt `--force`/`--amend`/`--no-verify` nicht mehr automatisch. What's-New Top weiterhin **Week 32** (03.–07.08.; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest; `code.claude.com` im ersten WebFetch-Versuch erreichbar). Blog-Announcements Top jetzt **12.08.** (Claude in Chrome / Cowork, darunter 11.08. Compliance API), Platform-Release-Notes Top unverändert **11.08.** — kein Eintrag vom 12. oder 13.08.; der Abruf direkt über `…/release-notes/overview` funktionierte wie beim letzten Crawl notiert. Alle vier Quellen gegengeprüft. — Vorheriger Crawl 12.08. 18:00 UTC: **keine neuen Einträge** seit dem Crawl am 12.08. um 12:00 UTC. **v2.1.228** (11.08., 19:50 UTC) ist weiterhin die neueste CLI-Version — gegenbestätigt über GitHub-Releases-API (Top-Release unverändert v2.1.228, darunter v2.1.227/10.08. und v2.1.226/08.08.), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = 2.1.228, `stable` = 2.1.221; letzter Registry-Schreibzugriff unverändert 11.08. 19:55 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.228) — kein v2.1.229+ und kein Release vom 12.08. What's-New Top weiterhin **Week 32** (03.–07.08., darunter Week 30; **Week 31 weiterhin unveröffentlicht**, kein Week-33-Digest; `code.claude.com` war diesmal im ersten WebFetch-Versuch erreichbar, kein ESERVFAIL). Blog-Announcements Top weiterhin **11.08.** (Compliance API), Platform-Release-Notes Top weiterhin **11.08.** (darunter 10.08. Sonnet-5-Dauerpreis und 07.08.) — kein Eintrag vom 12.08. Hinweis: `platform.claude.com/docs/en/release-notes` liefert in WebFetch inzwischen konsistent 404 (serverseitig 307-Redirect) — abrufbar ist nur noch `…/release-notes/overview`; künftige Crawls sollten direkt die overview-URL nutzen. Alle vier Quellen gegengeprüft. — Ältere Crawl-Historie in den Git-Commits.)
