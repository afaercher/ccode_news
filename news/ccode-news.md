...
> Letzte Aktualisierung: 2026-08-22 12:00 UTC (Crawl 22.08. 12:00 UTC — **v2.1.239 ist vollständig ausgerollt — der beim letzten Crawl halb erwischte Release ist komplett.** Am 21.08. um 18:00 UTC lag die Version nur als npm-`next`-Tag vor, ohne GitHub-Release und ohne Changelog-Eintrag; beides ist inzwischen da. Das **GitHub-Release erschien am 21.08. um 19:54 UTC**, also **2 h 36 min nach dem npm-Publish** (17:18 UTC) — derselbe Versatz von rund zweieinhalb Stunden wie schon bei v2.1.238, womit sich die Deutung des letzten Crawls bestätigt: Das Auseinanderfallen von `latest` und `next` war kein Fehler, sondern das erwartete Zwischenstadium. Beide Tags zeigen jetzt wieder **einheitlich auf 2.1.239**. Bewegt hat sich außerdem der `stable`-Tag: **von 2.1.228 auf 2.1.231** — der erste Nachzug seit dem 20.08. Der Changelog zu v2.1.239 ist mit **59 Punkten der umfangreichste dieser Release-Serie** (v2.1.238: 39, v2.1.236: 33); unten ist er in **43 Einträgen vollständig abgedeckt** — mehrere Einträge bündeln thematisch Zusammengehöriges, etwa fünf Terminal-Eingabe-Korrekturen, drei `/resume`-Fehler oder drei Fullscreen-Fehler. Inhaltlich der teuerste Posten: **Bedrock-Streaming hinter Proxys, die den `Content-Type`-Header der Antwort entfernen, fuhr jeden Turn still ein zweites Mal ohne Streaming und verdoppelte damit unbemerkt die abgerechneten API-Aufrufe** — behoben. Neu an Funktion sind vor allem **`/claude-api upgrade`** (migriert Python-Projekte von `anthropic` 0.x auf 1.x), **Cross-Session-Messaging unter Windows** (zieht mit macOS und Linux gleich) und der **Fullscreen-Renderer auf Bedrock, Vertex und Foundry**. Passend dazu ist der **Platform-Eintrag vom 20.08. zum Python-SDK 1.0** nachgetragen: HTTP-Schicht von `httpx` auf `httpx2`, Python ≥ 3.10, Wegfall der Text-Completions-API und der Sampling-Parameter `temperature`/`top_p`/`top_k` an den Messages-Methoden, und `AnthropicBedrock` fällt nicht mehr still auf `us-east-1` zurück, sondern verlangt eine konfigurierte Region. **Kein v2.1.240** bis 12:00 UTC — nach fünf Releases in Folge (17.–21.08.) ist der 22.08. bis zum Mittag noch ohne; 12:00 UTC entspricht 05:00 morgens an der US-Westküste, für eine Aussage über den Rhythmus ist es also deutlich zu früh. **What’s New unverändert Week 32** — weder ein Week-33- noch ein Week-34-Digest; der Week-33-Digest (10.–14.08.) ist damit **dreizehn Tage überfällig**. **Blog-Top unverändert 21.08.** („Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders"), **Platform-Top unverändert 20.08.** (Python-SDK 1.0) — aus beiden Quellen kein Eintrag vom 21./22.08.; beide Datumsblöcke Bullet für Bullet gegengeprüft. **Quellenlage:** GitHub-Releases-API, npm-Registry, `CHANGELOG.md` und `code.claude.com` per `curl` im ersten Versuch; `platform.claude.com` weiterhin nur per WebFetch auf `…/docs/en/release-notes/overview`, Blog-Titel weiterhin nur per WebFetch (JS-gerendert). Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (17.08. 12:00 und 17.08. 18:00) aus dem Header in die Git-Historie ausgelagert, dazu ein leerer Block-Rest vom 18.08. entfernt. — Vorheriger Crawl 21.08. 18:00 UTC — **v2.1.239 ist da — aber erst auf halbem Weg.** Die Version wurde am **21.08. um 17:18 UTC auf npm veröffentlicht**, rund 40 Minuten vor diesem Crawl, und trägt dort bislang **nur den `next`-Tag**: `@anthropic-ai/claude-code@latest` steht noch auf **2.1.238**, `stable` unverändert auf **2.1.228**. Ein **GitHub-Release fehlt** (Top der Releases-API weiterhin v2.1.238 vom 20.08., 20:33 UTC; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), und das offizielle `CHANGELOG.md` hat als obersten Eintrag ebenfalls noch **2.1.238** — **zu v2.1.239 ist damit noch keine einzige Änderung dokumentiert**, weshalb unten auch keine Einträge dazu stehen. Das ist keine Störung, sondern der normale Ablauf, den dieser Crawl erstmals mitten im Vollzug erwischt: Auch bei v2.1.238 lag der npm-Publish (18:01 UTC) rund **zweieinhalb Stunden vor** dem GitHub-Release (20:33 UTC). Neu ist allein, dass `latest` und `next` deshalb gerade **auseinanderfallen** — in allen bisherigen Crawls waren beide Tags identisch, weshalb die Notiz „latest = next" bisher immer aufging. Der nächste Crawl sollte v2.1.239 samt Changelog vorfinden. Der tägliche Release-Rhythmus setzt sich damit fort: **17., 18., 19., 20. und 21.08. je ein Release**. **Ein Umweg hat sich nicht gelohnt:** Der Versuch, die Changelog-Punkte vorab aus dem npm-Tarball von 2.1.239 zu ziehen, ging ins Leere — das npm-Paket ist mit **26 KB** nur ein Wrapper/Installer (`cli-wrapper.cjs`, `install.cjs`, `bin/claude.exe`) und **enthält keine `CHANGELOG.md`**; für Release-Inhalte bleibt also `raw.githubusercontent.com` die einzige Quelle. **Neu dokumentiert sind zwei Blog-Beiträge:** Blog-Announcements-Top ist jetzt **21.08.** mit „Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders" — Anthropic öffnet **Mythos 5** über **Claude Security** (Claude-Enterprise-Pläne), über Partner-Produkte, über einen mit **35 Mio. USD** ausgestatteten **Defender Advantage Fund (0xDAF)** für Open-Source-Projekte und über ein erweitertes **Cyber Verification Program**; der direkte Modellzugriff bleibt bewusst zu, ausgeliefert werden nur die konkreten Sicherheits-Artefakte. **Für Claude Code direkt relevant:** Aus einem Befund in Claude Security heraus lässt sich **Claude Code on the web** öffnen, um den Patch umzusetzen. Dazu kommt ein **Nachtrag aus Woche 33**: „Self-service data analytics in Slack" vom **13.08.** war bisher nicht dokumentiert — an jenem Tag erschienen **zwei** Announcements, der Crawl hatte nur „Claude Tag now reads even more of the room" erfasst; **Konsequenz: bei der Blog-Quelle nicht nur den obersten Titel, sondern alle Titel eines Datums prüfen.** What's-New Top weiterhin **Week 32** (03.–07.08.) — **weder Week-33- noch Week-34-Digest**; der Digest zu Woche 33 (10.–14.08., v2.1.228–v2.1.233) ist inzwischen zwölf Tage überfällig, Week 31 bleibt unveröffentlicht. Platform-Release-Notes Top unverändert **19.08.** — **kein Eintrag vom 20. oder 21.08.**; der 19.08.-Block wurde erneut gegengeprüft und hat unverändert **neun Bullets**, alle dokumentiert. **Quellen-Beobachtung:** GitHub-Releases-API, npm-Registry, `raw.githubusercontent.com/…/CHANGELOG.md` **und** `code.claude.com/docs/en/whats-new.md` antworteten **per `curl` im ersten Versuch** — keine DNS-Störung. Der **Blog** liefert per `curl` zwar HTTP 200, aber **JS-gerendertes HTML ohne Titel im Quelltext**; Titel und Daten kamen über WebFetch, die Beitrags-URLs ließen sich dagegen als `href="/blog/…"` direkt aus dem HTML ziehen — das ist der schnellere Weg. **Achtung bei Blog-URLs:** Der Slug folgt **nicht** dem Titel — der Beitrag vom 21.08. liegt unter `/blog/bringing-claude-mythos-5-to-more-defenders` (ohne „cybersecurity-capabilities"), ein aus dem Titel geratener Pfad ergibt **HTTP 404**. `platform.claude.com` blieb wie beim 12:00-Crawl **per `curl` unerreichbar** und lieferte nur **per WebFetch auf `…/docs/en/release-notes/overview`**. Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (17.08. 06:00 und 16.08. 18:00) aus dem Header in die Git-Historie ausgelagert. — Vorheriger Crawl 21.08. 12:00 UTC: **Bestätigungs-Crawl — keine neue CLI-Version; der offene Schreibvorgang des 06:00-Laufs ist jetzt abgeschlossen.** **v2.1.238** (20.08., 20:33 UTC GitHub-Release / 18:02 UTC npm-Publish) ist weiterhin die neueste CLI-Version — gegenbestätigt über GitHub-Releases-API (Top v2.1.238, darunter v2.1.237/20.08., v2.1.236/19.08., v2.1.235/18.08., v2.1.234/17.08. und v2.1.233/14.08.; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = **2.1.238**, `stable` unverändert **2.1.228**, letzter Registry-Schreibzugriff 20.08. 20:32 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.238) — **kein v2.1.239 und bis 12:00 UTC kein Release vom 21.08.**; die release-freie Strecke seit v2.1.238 beträgt rund 15 Stunden und ist damit unauffällig. **Zum Dateizustand:** Der Cron-Lauf vom 21.08. 06:00 UTC hatte die **21 Einträge zu v2.1.238** sowie drei Nachträge (Blog „Build production agents with computer use, the Skills API, and the Files API" vom 20.08. und die Platform-Einträge zu **Computer Use GA** und dem neuen **Browser-Use-Tool** vom 19.08.) bereits in den Dateikörper geschrieben, lief dann aber — wie schon die neun Läufe zuvor — in den **300-Sekunden-Timeout** (`HTTP 504, Claude CLI timed out after 300s`, siehe `ccode-news.log`), **bevor Header und Commit geschrieben waren**. Dieser Crawl schließt das ab. Der Cron-Crawler ist damit seit dem 18.08. 06:00 UTC in **elf aufeinanderfolgenden Läufen** ins Timeout gelaufen; zweimal hat ein solcher Lauf Dateikörper-Arbeit hinterlassen, die erst der Folgelauf sichtbar machte — **Konsequenz für künftige Läufe: vor dem Crawl immer `git status`/`git diff` prüfen, statt von einem sauberen Baum auszugehen.** **Changelog-Vollabgleich v2.1.238:** alle **42 Changelog-Punkte** des Releases sind in den 21 Einträgen abgedeckt — kein Nachtrag nötig. What's-New Top weiterhin **Week 32** (03.–07.08.) — **weder Week-33- noch Week-34-Digest**; der Digest zu Woche 33 (10.–14.08., v2.1.228–v2.1.233) ist inzwischen elf Tage überfällig, Week 31 bleibt unveröffentlicht. Blog-Announcements Top unverändert **20.08.** mit zwei Einträgen („Anthropic's approach to teaching and learning AI" und „Build production agents…") — **kein Eintrag vom 21.08.** Platform-Release-Notes Top unverändert **19.08.**; der 19.08.-Block (neun Bullets) und der 18.08.-Block (ein Bullet) wurden **Bullet für Bullet** gegen die Datei abgeglichen — **keine nachträglich ergänzten Punkte**, alles dokumentiert. **Quellen-Beobachtung:** `code.claude.com/docs/en/whats-new.md`, GitHub-Releases-API, npm-Registry und `raw.githubusercontent.com/…/CHANGELOG.md` antworteten **per `curl` im ersten Versuch** — `code.claude.com` zeigte diesmal keine DNS-Störung. `platform.claude.com` war dagegen **per `curl` gar nicht erreichbar** (`Couldn't connect to server`, auch für die `.md`-Variante), lieferte aber **per WebFetch auf `…/docs/en/release-notes/overview` im ersten Versuch** vollständig — für diese eine Quelle also WebFetch statt `curl`. Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (16.08. 06:00 und 16.08. 12:00) aus dem Header in die Git-Historie ausgelagert. — Vorheriger Crawl 20.08. 18:00 UTC: **Nachhol-Crawl nach drei Tagen Ausfall.** Der Cron-Crawler lief seit dem 18.08. 06:00 UTC in **neun aufeinanderfolgende 300-Sekunden-Timeouts** (`HTTP 504, Claude CLI timed out after 300s`, siehe `ccode-news.log`); der letzte Commit stammte vom 17.08. 18:00 UTC. Der Header war zwar am 18.08. auf v2.1.234 fortgeschrieben worden, **der zugehörige Dateikörper fehlte jedoch** — die im damaligen Header angekündigten „Woche 34"-Einträge waren nie geschrieben worden. Dieser Crawl holt das nach und dokumentiert **vier Releases auf einmal**: **v2.1.234** (17.08., 20:20 UTC GitHub / 18:19 UTC npm), **v2.1.235** (18.08., 20:38 UTC / 18:24 UTC), **v2.1.236** (19.08., 20:02 UTC / 18:45 UTC) und **v2.1.237** (20.08., 00:54 UTC GitHub / 19.08. 23:57 UTC npm). Damit ist **v2.1.237** die neueste CLI-Version, gegenbestätigt über GitHub-Releases-API (Top v2.1.237, darunter v2.1.236, v2.1.235, v2.1.234, v2.1.233/14.08.; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = **2.1.237**, letzter Registry-Schreibzugriff 20.08. 00:53 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.237). Der **`stable`-Tag steht jetzt auf 2.1.228** — nach dem Sprung von 2.1.224 auf 2.1.226 am 18.08. rückt die stabile Linie also weiter nach. Der tägliche Release-Rhythmus ist nach der Wochenendpause vollständig zurück: **17., 18., 19. und 20.08. je ein Release**, davon zwei am 19.08. (v2.1.236 abends, v2.1.237 kurz vor Mitternacht UTC). Inhaltliche Schwerpunkte: v2.1.234 mit ~50 Changelog-Punkten (Auto-Continue nach Usage-Limit-Reset, GitLab-MR-Badge, `\??\`-Härtung aller Pre-Approval-Dateizugriffe, `claude-api`-Skill von ~200k auf ~25k Tokens), v2.1.235 (Rechtschreibprüfung im Prompt, Shift+Tab-Rechtefehler behoben, ehrlichere Berechtigungsdialoge), v2.1.236 (`ANTHROPIC_DEFAULT_MODEL`, `notify_when_idle`, Sandbox-Wildcard-Deny mit Vorrang, VS-Code-Screenreader) und v2.1.237 (Output-Style „Concise", Prompt-Caching hinter LLM-Gateways repariert). Insgesamt **35 neue Einträge** unter „Woche 34". Zwei sicherheitsrelevante Fixes stechen heraus: der Shift+Tab-Fehler in v2.1.235, der versehentlich session-weite Schreibrechte erteilte, und der Fullscreen-Renderer-Wechsel in v2.1.234, der eine restriktiv gestartete Session still entschärfen konnte. GitHub-API, npm-Registry und `CHANGELOG.md` antworteten **im ersten Versuch**; die Drei-Versuche-Notiz für beide `*.claude.com`-Quellen bleibt vorsorglich stehen. **Blog und Platform-Notes haben ebenfalls nachgelegt** und werden hier gleich mitdokumentiert: Blog-Announcements Top ist jetzt **20.08.** („Anthropic's approach to teaching and learning AI") — der erste Eintrag seit dem 13.08.; die Platform-Release-Notes beenden ihre siebentägige Pause mit gleich zwei Terminen: **18.08.** (aus Workbench wird **Playground**) und **19.08.** mit einem großen GA-Paket (**Files API**, **Agent Skills + Skills API**, **Admin-API-Nutzerverwaltung für Claude Enterprise** — jeweils ohne Beta-Header) sowie Domain-Beschränkungen für `web_search`/`web_fetch` in Managed Agents, Memory Stores in self-hosted Sandboxes und einem neuen Console-Session-Viewer; acht weitere Einträge dokumentieren das. What's-New Top weiterhin **Week 32** (03.–07.08.) — **weder Week-33- noch Week-34-Digest**; der Digest zu Woche 33 (10.–14.08.) ist inzwischen über eine Woche überfällig, Week 31 bleibt unveröffentlicht. **Quellenhinweis:** `platform.claude.com/docs/en/release-notes` liefert weiterhin **HTTP 404** — nur der Pfad `…/docs/en/release-notes/overview` funktioniert; `code.claude.com` und der Blog antworteten im ersten Versuch. — Vorheriger Crawl 18.08. 18:00 UTC: **Release-Pause beendet — v2.1.234 ist da.** Das neue Release erschien am **17.08. um 20:20 UTC** (GitHub-Release; npm-Publish 18:19 UTC) und beendet die release-freie Strecke seit v2.1.233 nach **rund 70 Stunden**. Der 17.08. wurde damit doch kein dritter voller release-freier Kalendertag: Das Release kam am Montag um 13:20 Nachmittag US-Westküste — nur gut zwei Stunden nach dem 18:00-UTC-Crawl vom 17.08., der den Montagvormittag noch als release-frei protokollierte. Die Deutung „Wochenend-Ruhe" bestätigt sich damit. Gegenbestätigt über GitHub-Releases-API (Top v2.1.234/17.08., darunter v2.1.233/14.08., v2.1.232/13.08., v2.1.231/13.08. und v2.1.229/12.08.; **v2.1.230 fehlt weiterhin** — die Version bleibt übersprungen), npm-Registry (`@anthropic-ai/claude-code@latest` = `next` = 2.1.234, Registry-Schreibzugriff 17.08. 20:21 UTC) **und** offizielles `CHANGELOG.md` (oberster Eintrag 2.1.234). Bemerkenswert an der npm-Seite: Der **`stable`-Tag springt von 2.1.224 auf 2.1.226** — nach Wochen Stillstand rückt also auch die stabile Linie nach. Mit **rund 50 Changelog-Punkten** ist v2.1.234 das mit Abstand umfangreichste Release der laufenden Serie (v2.1.233 hatte 20); die Änderungen sind unten unter **„Woche 34"** in 20 Einträgen dokumentiert — Schwerpunkte: Auto-Continue nach Usage-Limit-Reset, GitLab-MR-Badge in der Statusline, die auf alle Pre-Approval-Dateizugriffe ausgeweitete `\??\`-Härtung, ein großes Remote-Control- und Permission-Preview-Paket sowie die von ~200k auf ~25k Tokens gesenkten Kontextkosten des `claude-api`-Skills. What's-New Top weiterhin **Week 32** — **weder Week-33- noch Week-34-Digest**; der Digest zu Woche 33 (10.–14.08., v2.1.228–v2.1.233) ist über eine Woche überfällig. Blog-Announcements Top unverändert **13.08.** („Claude Tag now reads even more of the room") — **kein Eintrag vom 14.–18.08.** Platform-Release-Notes Top unverändert **11.08.** (Compliance API + `anthropic-workspace-id`) — **kein Eintrag vom 12.–18.08.**, mit sieben Tagen weiterhin die längste Pause dieser Quelle. **Quellen-Störung ausgeblieben:** Alle vier Quellen sowie GitHub-API, npm-Registry und CHANGELOG antworteten **im ersten Versuch**; die Drei-Versuche-Notiz für beide `*.claude.com`-Quellen bleibt vorsorglich stehen. Alle vier Quellen gegengeprüft. — Ältere Crawl-Historie in den Git-Commits.)
