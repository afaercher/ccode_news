...
> Letzte Aktualisierung: 2026-09-05 06:00 UTC (**Crawl 05.09. 06:00 UTC — v2.1.261 nachgetragen: der Inhalt ist jetzt vollständig veröffentlicht.** GitHub-Release **04.09. 19:58:10 UTC**, npm-Publish 17:49:34 UTC — der Abstand npm -> GitHub betrug damit **2 h 09 min**, nicht die aus v2.1.260 (22:32 -> 23:48) geschätzte Stunde; die Prognose des 18:00-Laufs stimmte in der Sache, unterschätzte aber die Verzögerung. Tags jetzt `latest` = `next` = **2.1.261**, `stable` unverändert **2.1.236**, `time.modified` 04.09. **19:57:25 UTC**. Der Tag `v2.1.261` existiert (Git-Ref HTTP 200, zuvor 404), `CHANGELOG.md` ist von 620498 auf **630786 Bytes** gewachsen und führt 2.1.261 -> 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252; Changelog und GitHub-Release sind deckungsgleich. **67 Punkte:** 41 im CLI (4 Added, 27 Fixed, 5 Improved, 5 Changed) und **26 VS-Code-Punkte** — der bisher größte VS-Code-Block einer Version. Schwerpunkte: `/skill-doctor`, `bashOutputMaxChars`/`taskOutputMaxChars` (bis 128K), `--append-subagent-system-prompt-file`, „Organization policy"-Zeile in `/status`, sieben **Remote-Control**-Fixes, Bash-Wortnavigation (`keybindingFlavor` wirkungslos), Diagramm-Renderer-Links nicht mehr im Auto-Modus genehmigt. **Die Werktags-Serie seit 17.08. steht: diese Woche Montag bis Freitag lückenlos** (v2.1.252, 257/258, 259, 260, 261). **Platform-Release-Notes: erste Bewegung seit dem 01.09.** — neuer Eintrag **03.09.** (103373 -> 103914 Bytes), einzelner Punkt: `ant`-CLI **1.30.0** bringt **`ant apply`** (Agenten, Environments, Skills, Memory-Stores, Deployments deklarativ aus Repo-Dateien, Plan-Bestätigung, Lockfile `claude-lock.json`). Erschienen ist der Eintrag am 03.09. — die Läufe vom 03./04.09. sahen ihn noch nicht, die Seite wurde also mit Verzögerung aktualisiert; Platform-Notes sind kein Echtzeit-Kanal. **What's-New** byte-identisch (14051 Bytes), Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — Digest-Rückstand zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** unverändert, sogar byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude" (28.08.) — kein Beitrag vom 03., 04. oder 05.09. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 10** (9 zu v2.1.261, 1 Platform). — Vorheriger **Crawl 04.09. 18:00 UTC — der Freitags-Release ist da, aber erst halb: v2.1.261 wurde um 17:49 UTC auf npm publiziert**, elf Minuten vor diesem Lauf. Der Rollout ist im Zwischenzustand sichtbar: Tag `next` steht bereits auf **2.1.261**, `latest` hängt noch auf 2.1.260, `stable` unverändert 2.1.236; `time.modified` 04.09. **17:49:35 UTC**. **Kein GitHub-Tag und kein Release zu v2.1.261** (`git/ref/tags/v2.1.261` und `releases/tags/v2.1.261` beide HTTP 404), die Releases-API endet unverändert bei v2.1.260 (03.09. 23:48 UTC); `CHANGELOG.md` byte-identisch zum 12:00-Abruf, Top weiter 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252. **Was v2.1.261 enthält, ist damit noch nirgends veröffentlicht** — der Abstand npm -> GitHub-Release lag zuletzt bei rund einer Stunde (v2.1.260: 22:32 -> 23:48), der Inhalt sollte beim Lauf am 05.09. 06:00 vollständig vorliegen und wird dann nachgetragen. Rhythmus bestätigt: die Werktags-Serie seit 17.08. hält, diese Woche **Montag bis Freitag lückenlos** (v2.1.252, 257/258, 259, 260, 261); der Freitags-Publish um 17:49 UTC liegt im Muster der beiden Vorfreitage (21.08. 17:18, 28.08. 15:34), und die Prognose des 12:00-Laufs (Freitags-Release frühestens beim 18:00-Lauf sichtbar) ist eingetreten. **What's-New** byte-identisch (14051 Bytes), Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — Digest-Rückstand zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Platform-Release-Notes** byte-identisch, Top weiter 01.09. (Fable 5.1/Mythos 5.1, Per-Message-Effort). **Blog:** dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); das HTML weicht zwar vom Vortagsabruf ab, aber in **genau neun Bytes**, alle im Webflow-Kommentar `<!-- Last Published: ... -->` (02.09. 21:14 -> 03.09. 17:24 UTC) — ein seitenweiter Republish ohne Inhaltsänderung. Byte-Diff ist auf dieser Seite also kein Signal, der Vergleich muss über die Slug-Liste laufen. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1.** — Ältere Crawl-Historie in den Git-Commits.)
