...
> Letzte Aktualisierung: 2026-09-05 12:00 UTC (**Crawl 05.09. 12:00 UTC — Bestätigungslauf am Samstag: keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.261** (npm-Publish 04.09. 17:49:34 UTC, GitHub-Release 19:58:10 UTC); bis 12:00 UTC gibt es **kein v2.1.262** — der Git-Ref `v2.1.262` antwortet HTTP **404**, `time.modified` steht weiterhin auf 04.09. **19:57:25 UTC**, die Tags sind still (`latest` = `next` = **2.1.261**, `stable` **2.1.236**). `CHANGELOG.md` ist byte-identisch zum 06:00-Abruf (**630786 Bytes**, 2.1.261 -> 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252), die Releases-API listet deckungsgleich v2.1.261 an der Spitze. **Rhythmus — eine Korrektur an der naheliegenden Annahme:** Heute ist Samstag, und die Werktags-Serie seit 17.08. legt nahe, das Wochenende sei grundsätzlich leer. Das stimmt so nicht. Seit dem 01.07. fallen **fünf Publishes auf ein UTC-Wochenende**, davon liegen **drei auch nach US-Pazifikzeit am Wochenende**: 2.1.215 (19.07. 00:53 UTC = Sa 17:53 PT), 2.1.240 (22.08. 13:03 UTC = **Sa 06:03 PT**) und 2.1.241 (22.08. 23:58 UTC = Sa 16:58 PT) — der 22.08. war ein Samstag mit gleich zwei Publishes. 2.1.214 und 2.1.226 sind dagegen nur Scheintreffer: 00:13 bzw. 01:53 UTC sind Freitagabend an der Westküste. Die beiden letzten Wochenenden (23./24.08. und 29./30.08.) blieben leer. Ein Samstags-Release ist also unwahrscheinlich, aber belegt vorgekommen — der 18:00-Lauf prüft weiter, ein leerer Vormittag ist kein Beweis. **Fund dieses Laufs — eine Falle in der Quellenliste:** Die dokumentierte URL `platform.claude.com/docs/en/release-notes` liefert die Platform-Release-Notes **nicht mehr**. Sie antwortet mit **HTTP 307** und leitet auf `/docs/en/release-notes/system-prompts/overview` weiter — den Änderungsverlauf der **System-Prompts**, eine völlig andere Seite, die anschließend brav HTTP 200 und plausibel aussehenden Inhalt zurückgibt (**3858 Bytes** statt 103914). Die Weiterleitung gilt für die HTML-Seite **und** die `.md`-Variante. Ein naiver Abruf der URL aus der Quellenliste hätte damit stillschweigend die falsche Seite als „Platform Release Notes“ protokolliert — kein Fehlercode, keine leere Antwort, nur der falsche Inhalt. Die funktionierende Adresse ist `/docs/en/release-notes/overview.md`; aufgefallen ist der Tausch hier nur über die **Größenplausibilität** (3858 statt 103914 Bytes). Für künftige Läufe: Platform-Notes immer über `…/release-notes/overview.md` holen und die Bytezahl gegen den Vorlauf prüfen. Über die richtige URL ist die Seite **byte-identisch** (103914 Bytes), Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`) — der beim 06:00-Lauf erstmals gesehene Eintrag ist bestätigt, danach kam nichts. **What's-New** byte-identisch (14051 Bytes), Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — der Digest-Rückstand bleibt bei zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der Webflow-Stempel `Last Published` steht unverändert auf **03.09. 17:24 UTC**, seit Donnerstag also nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 0** — geändert wurde nur der Crawl-Header. — Vorheriger **Crawl 05.09. 06:00 UTC — v2.1.261 nachgetragen: der Inhalt ist jetzt vollständig veröffentlicht.** GitHub-Release **04.09. 19:58:10 UTC**, npm-Publish 17:49:34 UTC — der Abstand npm -> GitHub betrug damit **2 h 09 min**, nicht die aus v2.1.260 (22:32 -> 23:48) geschätzte Stunde; die Prognose des 18:00-Laufs stimmte in der Sache, unterschätzte aber die Verzögerung. Tags jetzt `latest` = `next` = **2.1.261**, `stable` unverändert **2.1.236**, `time.modified` 04.09. **19:57:25 UTC**. Der Tag `v2.1.261` existiert (Git-Ref HTTP 200, zuvor 404), `CHANGELOG.md` ist von 620498 auf **630786 Bytes** gewachsen und führt 2.1.261 -> 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252; Changelog und GitHub-Release sind deckungsgleich. **67 Punkte:** 41 im CLI (4 Added, 27 Fixed, 5 Improved, 5 Changed) und **26 VS-Code-Punkte** — der bisher größte VS-Code-Block einer Version. Schwerpunkte: `/skill-doctor`, `bashOutputMaxChars`/`taskOutputMaxChars` (bis 128K), `--append-subagent-system-prompt-file`, „Organization policy"-Zeile in `/status`, sieben **Remote-Control**-Fixes, Bash-Wortnavigation (`keybindingFlavor` wirkungslos), Diagramm-Renderer-Links nicht mehr im Auto-Modus genehmigt. **Die Werktags-Serie seit 17.08. steht: diese Woche Montag bis Freitag lückenlos** (v2.1.252, 257/258, 259, 260, 261). **Platform-Release-Notes: erste Bewegung seit dem 01.09.** — neuer Eintrag **03.09.** (103373 -> 103914 Bytes), einzelner Punkt: `ant`-CLI **1.30.0** bringt **`ant apply`** (Agenten, Environments, Skills, Memory-Stores, Deployments deklarativ aus Repo-Dateien, Plan-Bestätigung, Lockfile `claude-lock.json`). Erschienen ist der Eintrag am 03.09. — die Läufe vom 03./04.09. sahen ihn noch nicht, die Seite wurde also mit Verzögerung aktualisiert; Platform-Notes sind kein Echtzeit-Kanal. **What's-New** byte-identisch (14051 Bytes), Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — Digest-Rückstand zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** unverändert, sogar byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude" (28.08.) — kein Beitrag vom 03., 04. oder 05.09. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 10** (9 zu v2.1.261, 1 Platform). — Ältere Crawl-Historie in den Git-Commits.)
