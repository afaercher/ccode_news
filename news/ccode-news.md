...
> Letzte Aktualisierung: 2026-09-07 12:00 UTC (**Crawl 07.09. 12:00 UTC — fünfter Lauf in Folge ohne Bewegung, Montagvormittag UTC.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — die npm-Version antwortet HTTP **404**, ebenso die Git-Refs `v2.1.264` und `v2.1.265`, `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den vier Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze mit dem bekannten 60-Byte-Platzhalter-Body. `v2.1.262` bleibt HTTP **404**. Die Lücke seit dem letzten Publish beträgt **33,9 Stunden**. Der Zeitpunkt erklärt den Leerlauf weiterhin vollständig: 07.09. 12:00 UTC ist **Montag, 05:00 Uhr US-Pazifikzeit** — das prognostizierte Fenster für v2.1.264 (Montag **17:00–22:30 UTC**) beginnt erst in fünf Stunden, **die Prognose ist also weiterhin weder bestätigt noch widerlegt**; entschieden wird sie beim 18:00-Lauf beziehungsweise spätestens Dienstag 06:00. **Fund dieses Laufs — Changelog, npm und GitHub decken sich nicht, und zwar in beide Richtungen:** Ein vollständiger Abgleich der **511 npm-Publishes** gegen die **387 Versionsabschnitte** in `CHANGELOG.md` zeigt **132 Versionen, die publiziert, aber nie im Changelog beschrieben wurden** (25,8 %), und umgekehrt **8 Changelog-Abschnitte zu Versionen, die es auf npm nie gab** (0.2.21, 0.2.26, 0.2.63, 0.2.75, 0.2.82, 1.0.97, **2.1.43**, **2.1.46**) — letztere mit echtem, detailliertem Inhalt, aber ohne Git-Tag und ohne Release, also **dokumentiert und trotzdem nicht installierbar**. Die Disziplin hat über die Serien hinweg stark zugenommen: **0.2.x 66 %** undokumentierte Publishes, **1.0.x 33 %**, **2.0.x 16 %**, **2.1.x nur noch 8 %** (18 von 225), in den letzten 60 Publishes lediglich **v2.1.213** (17.07.) und **v2.1.242** (24.08.). Beide haben weder Changelog-Abschnitt noch Git-Tag noch Release — sie existieren ausschließlich in der npm-Registry. Der praktisch wichtigste Fall ist aber **v2.1.243** (24.08. 23:10 UTC): Git-Tag **HTTP 200**, Changelog-Abschnitt vorhanden, aber **kein GitHub-Release** — ein Watcher, der nur die Releases-API abfragt, hätte diese Version mit ihrem vollen Inhalt komplett übersehen. Dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08., v2.1.234–v2.1.239) weiterhin oben; `2026-w35`, `-w36` und `-w37` antworten 404, der reale Digest-Rückstand bleibt bei **zwei Wochen** (W35, W36). **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **siebten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Deckungslücken zwischen Changelog, npm und GitHub). — Vorheriger **Crawl 07.09. 06:00 UTC — vierter Lauf in Folge ohne Bewegung, Montagmorgen UTC.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — die Git-Refs `v2.1.264` und `v2.1.265` antworten HTTP **404**, `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den drei Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze mit dem bekannten 60-Byte-Platzhalter-Body. `v2.1.262` bleibt HTTP **404** und damit dauerhaft übersprungen. Die Lücke seit dem letzten Publish beträgt **27,9 Stunden**. Der Zeitpunkt erklärt den Leerlauf vollständig: 07.09. 06:00 UTC ist **Sonntag, 23:00 Uhr US-Pazifikzeit** — nach dem im 18:00-Lauf belegten Muster ist der Sonntag ein echter Leertag, und das prognostizierte Fenster für v2.1.264 (Montag **17:00–22:30 UTC**) beginnt erst in elf Stunden. **Die Prognose ist zu diesem Zeitpunkt also weder bestätigt noch widerlegt.** **Fund dieses Laufs — der wöchentliche Digest hat genau ein Loch, und das ist kein Versäumnis der Doku:** Die Übersicht unter `whats-new` listet lückenlos Week 13 bis Week 34 — **mit einer einzigen Ausnahme: Week 31 (27.–31.07.2026) fehlt.** Der Sprung von Week 32 direkt auf Week 30 steht so sowohl auf der Übersichtsseite als auch im Doku-Index `llms.txt`, und `whats-new/2026-w31.md` antwortet HTTP **404** — sechs Wochen später ist nichts nachgetragen worden. Die Ursache ist in der npm-Registry nachweisbar: In **ISO-Woche 31 gab es null Publishes**. Die 239-Stunden-Lücke zwischen v2.1.220 (24.07. 23:11 UTC) und v2.1.221 (03.08. 22:16 UTC) deckt exakt diese Kalenderwoche ab. **Der Digest ist also release-getrieben, nicht kalendergetrieben** — ohne Releases keine Wochenseite, und die Woche wird auch nicht nachgereicht. Daraus folgt zugleich eine **Korrektur an den letzten fünf Crawl-Blöcken**: Der dort wiederholt genannte „Digest-Rückstand von drei Wochen“ war zu hoch gegriffen. `2026-w37` antwortet 404, weil **Woche 37 heute erst beginnt** — das ist kein Rückstand. Der echte Rückstand beträgt **zwei Wochen**, W35 und W36, und die sind alles andere als ereignisarm: **8 Publishes** in W35 (v2.1.242–v2.1.251) und **7** in W36 (v2.1.252–v2.1.263), zusammen die dichtesten Wochen der gesamten Serie. Dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08., v2.1.234–v2.1.239) weiterhin oben. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **sechsten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API, Git-Refs und der Doku-Index `llms.txt` per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Digest-Lücke Week 31 und korrigierter Rückstand). — Ältere Crawl-Historie in den Git-Commits.)
