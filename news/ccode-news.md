...
> Letzte Aktualisierung: 2026-08-23 06:00 UTC (Crawl 23.08. 06:00 UTC — **Zwei Neuigkeiten, die gegenläufiger kaum sein könnten: Die Changelogs verstummen — und die seit zwei Wochen überfälligen Wochen-Digests sind auf einen Schlag alle da.** **Zuerst die CLI: v2.1.241 ist erschienen** — npm-Publish am **22.08. um 23:58 UTC**, GitHub-Release am **23.08. um 00:52 UTC**. Damit ist der 22.08. der **erste Tag dieser Serie mit zwei npm-Releases** (2.1.240 um 13:03, 2.1.241 um 23:58 UTC); der tägliche Rhythmus hält seit **sieben Tagen** (17.–23.08.). Der Versatz zwischen npm und GitHub schrumpft weiter und liegt jetzt bei **54 Minuten** — nach 2 h 36 min (v2.1.239) und 1 h 42 min (v2.1.240) der dritte Schritt nach unten in Folge. Ausgerollt ist alles: `latest` und `next` einheitlich **2.1.241**, `stable` unverändert **2.1.231**. **Der Changelog schweigt zum zweiten Mal.** Zu v2.1.241 steht in `CHANGELOG.md` wie im GitHub-Release-Body wieder nur der eine Punkt „Bug fixes and reliability improvements". Der letzte Crawl hatte ausdrücklich offengelassen, ob der ebenso leere Abschnitt zu **v2.1.240** nachträglich gefüllt wird — **er wurde nicht gefüllt**, beide Quellen zeigen ihn unverändert einzeilig. Damit sind **zwei aufeinanderfolgende Versionen ohne jede Aufschlüsselung** ausgeliefert; unten steht dazu erneut **genau ein Eintrag** und kein geratener Inhalt. Zum Vergleich der Serie davor: v2.1.239 hatte 59 Punkte, v2.1.238 39, v2.1.236 33. **Die eigentliche Bewegung kommt aus What's New:** Der Digest-Rückstand ist beendet — **Week 33 (10.–14.08., v2.1.225–v2.1.233) und Week 34 (17.–21.08., v2.1.234–v2.1.239) sind beide veröffentlicht**, der Week-33-Digest war zuletzt vierzehn Tage überfällig. Beide bringen je drei Schwerpunkt-Features und dazu elf bzw. dreizehn „Other wins". Der Abgleich Punkt für Punkt gegen die Datei zeigt: **Der Großteil war über die Changelogs längst dokumentiert** — `/goal`-Check-ins, `CLAUDE_CODE_TOOL_MEMORY_LIMIT`, `additionalMarketplaces`, Concise, `spellcheck`, `ANTHROPIC_DEFAULT_MODEL`, `notify_when_idle`, Fork-Modus, GitLab-`glab`, Markdown in eigenen Prompts und weitere standen schon drin. **Neu sind drei Dinge**, die in keinem Changelog-Punkt so auftauchten und deshalb unten als eigene Einträge stehen: die Research Preview **`/design`** (Claude Designs Artboard-Workflow in CLI und Desktop, ab v2.1.233), **Auto-Continue nach dem Nutzungslimit** (Häkchen auf der Limit-Karte in Claude Code Desktop, dazu die Zeile „Continue automatically at usage limit" in `/config` der CLI) und die **Gerätekarte im Code-Tab der Claude-App**, mit der Remote Control zugleich **die Research Preview verlässt**. **Week 31 bleibt weiterhin unveröffentlicht** — die Liste springt von Week 32 auf Week 30. **Blog und Platform stehen still.** Blog-Top unverändert **21.08.** („Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders"); alle **fünfzehn** im HTML gelisteten Slugs wurden geprüft, der einzige noch nicht per Slug wiedergefundene (`bringing-claude-code-and-claude-cowork-to-government`) entpuppte sich beim Nachfassen als Beitrag vom **07.07.**, der längst dokumentiert ist — **kein Beitrag vom 22./23.08.** **Platform-Top unverändert 20.08.** (Python-SDK 1.0) — kein Eintrag vom 21.–23.08. **Quellenlage:** GitHub-Releases-API, npm-Registry und `code.claude.com` (Übersicht plus beide Wochen-Seiten als `.md`) per `curl` im ersten Versuch; `raw.githubusercontent.com/…/CHANGELOG.md` lieferte im ersten Versuch **leer** und erst mit `-L --retry 3` den Inhalt — **Konsequenz: bei dieser Quelle die leere Antwort nicht als „unverändert" lesen, sondern mit Retry gegenprüfen.** `platform.claude.com` weiterhin nur per WebFetch auf `…/docs/en/release-notes/overview`, Blog-Titel und -Daten weiterhin nur per WebFetch (JS-gerendert), Slug-Liste per `curl` aus dem HTML. Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (21.08. 18:00 und 21.08. 12:00) aus dem Header in die Git-Historie ausgelagert. — Vorheriger Crawl 22.08. 18:00 UTC — **v2.1.240 ist da — und bricht mit der Dokumentationspraxis der ganzen Serie.** Der 22.08. ist damit doch kein release-freier Tag: Die Version wurde **um 13:03 UTC auf npm veröffentlicht** und **um 14:45 UTC als GitHub-Release** nachgezogen. Beide Zeitpunkte sind bemerkenswert. Erstens liegt der Versatz zwischen npm und GitHub bei nur **1 h 42 min** statt der rund zweieinhalb Stunden von v2.1.238 und v2.1.239. Zweitens kam der Publish **rund vier Stunden früher am Tag** als bei den Vorgängern (13:03 statt 17–18 UTC) — 06:03 morgens an der US-Westküste. Der 12:00-Crawl hatte den Tag noch als release-frei protokolliert und dabei ausdrücklich vermerkt, für eine Rhythmus-Aussage sei es zu früh; das war richtig, das Release folgte gut eine Stunde später. Der **tägliche Rhythmus hält damit seit sechs Tagen** (17.–22.08.). Ausgerollt ist die Version bereits vollständig — `latest` und `next` zeigen einheitlich auf **2.1.240**, ein Auseinanderfallen wie beim 21.08.-Crawl gibt es diesmal nicht; `stable` bleibt unverändert bei **2.1.231**. **Der eigentliche Befund dieses Crawls ist aber der Changelog — genauer: sein Fehlen.** Sowohl der Abschnitt `## 2.1.240` in der offiziellen `CHANGELOG.md` als auch der Body des GitHub-Releases enthalten **einen einzigen Punkt**: „Bug fixes and reliability improvements". Keine Aufschlüsselung, keine benannten Fehler, keine neue Funktion. Das ist **das erste Mal in dieser Release-Serie**, dass eine Version ohne Detailangaben ausgeliefert wird — v2.1.239 hatte 59 Punkte, v2.1.238 39, v2.1.236 33. Beide Quellen wurden gegengeprüft und stimmen überein, es handelt sich also nicht um einen noch nicht nachgezogenen Text wie beim halb ausgerollten v2.1.239 am 21.08., sondern um den veröffentlichten Stand. **Konsequenz für die Datei:** Unten steht dazu **genau ein Eintrag**, der den Sachverhalt benennt — geratene Inhalte gibt es nicht. Ob der Changelog nachträglich noch gefüllt wird, sollte der nächste Crawl prüfen. **Die drei übrigen Quellen stehen still.** **What’s New unverändert Week 32** (03.–07.08.) — weder Week-33- noch Week-34-Digest; der Week-33-Digest (10.–14.08.) ist damit **dreizehn Tage überfällig**, Week 31 bleibt unveröffentlicht. **Blog-Top unverändert 21.08.** („Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders"); alle fünfzehn im HTML gelisteten Beitrags-Slugs wurden gegen die Datei geprüft — **kein undokumentierter Beitrag**, insbesondere kein Eintrag vom 22.08. Damit greift die beim letzten Crawl gezogene Konsequenz („nicht nur den obersten Titel prüfen") erstmals systematisch. **Platform-Top unverändert 20.08.** (Python-SDK 1.0) — kein Eintrag vom 21./22.08. **Quellenlage:** GitHub-Releases-API, npm-Registry, `CHANGELOG.md` und `code.claude.com` per `curl` im ersten Versuch; `platform.claude.com` weiterhin nur per WebFetch auf `…/docs/en/release-notes/overview`, Blog-Titel und -Daten weiterhin nur per WebFetch (JS-gerendert), die Slug-Liste dagegen per `curl` direkt aus dem HTML. Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (20.08. 18:00 und 18.08. 18:00) aus dem Header in die Git-Historie ausgelagert. — Vorheriger Crawl 22.08. 12:00 UTC — **v2.1.239 ist vollständig ausgerollt — der beim letzten Crawl halb erwischte Release ist komplett.** Am 21.08. um 18:00 UTC lag die Version nur als npm-`next`-Tag vor, ohne GitHub-Release und ohne Changelog-Eintrag; beides ist inzwischen da. Das **GitHub-Release erschien am 21.08. um 19:54 UTC**, also **2 h 36 min nach dem npm-Publish** (17:18 UTC) — derselbe Versatz von rund zweieinhalb Stunden wie schon bei v2.1.238, womit sich die Deutung des letzten Crawls bestätigt: Das Auseinanderfallen von `latest` und `next` war kein Fehler, sondern das erwartete Zwischenstadium. Beide Tags zeigen jetzt wieder **einheitlich auf 2.1.239**. Bewegt hat sich außerdem der `stable`-Tag: **von 2.1.228 auf 2.1.231** — der erste Nachzug seit dem 20.08. Der Changelog zu v2.1.239 ist mit **59 Punkten der umfangreichste dieser Release-Serie** (v2.1.238: 39, v2.1.236: 33); unten ist er in **43 Einträgen vollständig abgedeckt** — mehrere Einträge bündeln thematisch Zusammengehöriges, etwa fünf Terminal-Eingabe-Korrekturen, drei `/resume`-Fehler oder drei Fullscreen-Fehler. Inhaltlich der teuerste Posten: **Bedrock-Streaming hinter Proxys, die den `Content-Type`-Header der Antwort entfernen, fuhr jeden Turn still ein zweites Mal ohne Streaming und verdoppelte damit unbemerkt die abgerechneten API-Aufrufe** — behoben. Neu an Funktion sind vor allem **`/claude-api upgrade`** (migriert Python-Projekte von `anthropic` 0.x auf 1.x), **Cross-Session-Messaging unter Windows** (zieht mit macOS und Linux gleich) und der **Fullscreen-Renderer auf Bedrock, Vertex und Foundry**. Passend dazu ist der **Platform-Eintrag vom 20.08. zum Python-SDK 1.0** nachgetragen: HTTP-Schicht von `httpx` auf `httpx2`, Python ≥ 3.10, Wegfall der Text-Completions-API und der Sampling-Parameter `temperature`/`top_p`/`top_k` an den Messages-Methoden, und `AnthropicBedrock` fällt nicht mehr still auf `us-east-1` zurück, sondern verlangt eine konfigurierte Region. **Kein v2.1.240** bis 12:00 UTC — nach fünf Releases in Folge (17.–21.08.) ist der 22.08. bis zum Mittag noch ohne; 12:00 UTC entspricht 05:00 morgens an der US-Westküste, für eine Aussage über den Rhythmus ist es also deutlich zu früh. **What’s New unverändert Week 32** — weder ein Week-33- noch ein Week-34-Digest; der Week-33-Digest (10.–14.08.) ist damit **dreizehn Tage überfällig**. **Blog-Top unverändert 21.08.** („Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders"), **Platform-Top unverändert 20.08.** (Python-SDK 1.0) — aus beiden Quellen kein Eintrag vom 21./22.08.; beide Datumsblöcke Bullet für Bullet gegengeprüft. **Quellenlage:** GitHub-Releases-API, npm-Registry, `CHANGELOG.md` und `code.claude.com` per `curl` im ersten Versuch; `platform.claude.com` weiterhin nur per WebFetch auf `…/docs/en/release-notes/overview`, Blog-Titel weiterhin nur per WebFetch (JS-gerendert). Alle vier Quellen gegengeprüft; die beiden ältesten Crawl-Blöcke (17.08. 12:00 und 17.08. 18:00) aus dem Header in die Git-Historie ausgelagert, dazu ein leerer Block-Rest vom 18.08. entfernt. — Ältere Crawl-Historie in den Git-Commits.)
