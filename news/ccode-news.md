...
> Letzte Aktualisierung: 2026-09-05 18:00 UTC (**Crawl 05.09. 18:00 UTC — dritter Lauf des Samstags, erneut keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.261** (npm-Publish 04.09. 17:49:34 UTC, GitHub-Release 19:58:10 UTC); ein **v2.1.262 gibt es auch um 18:00 UTC nicht** — der Git-Ref `v2.1.262` antwortet HTTP **404**, `time.modified` steht weiterhin auf 04.09. **19:57:25 UTC**, die Tags sind still (`latest` = `next` = **2.1.261**, `stable` **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den beiden Vorläufen (**630786 Bytes**), die Releases-API listet deckungsgleich v2.1.261 an der Spitze. **Rhythmus — der leere Samstag ist statistisch unauffällig:** Seit dem letzten Publish sind **24,2 Stunden** vergangen. Unter den 57 Publishes seit dem 01.07. gibt es 56 Abstände, und dieser liegt damit erst auf **Rang 20** — völlig gewöhnlich. Aussagekräftiger sind die Spitzen: Die drei größten Lücken **innerhalb** des normalen Betriebs sind allesamt Freitag→Montag und liegen bei **71–74 Stunden** (2.1.251 -> 2.1.252 mit 73,6 h, 2.1.201 -> 2.1.202 mit 71,8 h, 2.1.233 -> 2.1.234 mit 71,5 h); käme die nächste Version am Montagabend, wäre die Lücke rund 72 h und träfe das Muster exakt. Der Ausreißer bleibt der Sommerstillstand 2.1.220 -> 2.1.221 mit **239 h** (24.07. bis 03.08.). **Zum Zeitfenster:** 18:00 UTC ist 11:00 Uhr Samstagvormittag an der US-Westküste. Von den beiden echten Samstags-Publishes lag 2.1.240 bei **06:03 PT** — dieser Vormittagsslot ist heute ungenutzt verstrichen; 2.1.241 kam dagegen um **16:58 PT**, also 23:58 UTC. Ein Samstags-Release wäre ab jetzt nur noch in diesem Abendfenster möglich und damit erst beim **Lauf am Sonntag 06:00 UTC** sichtbar. **Fund dieses Laufs — der `stable`-Kanal hängt planmäßig zwei Wochen zurück:** Der Tag `stable` zeigt auf **2.1.236**, und diese Version wurde am **19.08. um 18:45 UTC** publiziert — sie ist heute **17 Tage alt**. Zwischen `stable` und `latest` liegen **19 publizierte Versionen** (2.1.237 bis 2.1.261). Der Zeiger selbst steht dabei nicht fest: Er ist am **03.09. von 2.1.231 auf 2.1.236 gesprungen** — von einer Version vom 13.08. auf eine vom 19.08., also ein Nachrücken um sechs Tage, das den Abstand zu `latest` gerade nicht verkleinert hat. `stable` ist damit erkennbar kein „letztes gutes Release“, sondern ein bewusst **rund zwei Wochen zurückgehaltener Kanal** — relevant für alle, die per `@stable` pinnen und sich fragen, warum ein am Vortag veröffentlichter Fix bei ihnen nicht ankommt. Dazu unten ein eigener Eintrag. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über die korrekte Adresse `…/release-notes/overview.md` — die beim 12:00-Lauf gefundene 307-Weiterleitung der dokumentierten URL besteht fort, die Größenprüfung greift; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`). **What's-New** byte-identisch (14051 Bytes), Week 34 oben; `whats-new/2026-w35`, `-w36` und nun auch `-w37` antworten **HTTP 404** — der Digest-Rückstand wächst damit rechnerisch auf drei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der Webflow-Stempel `Last Published` steht den dritten Lauf in Folge auf **03.09. 17:24 UTC** — seit Donnerstag nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (der `stable`-Kanal). — Vorheriger **Crawl 05.09. 12:00 UTC — Bestätigungslauf am Samstag: keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.261** (npm-Publish 04.09. 17:49:34 UTC, GitHub-Release 19:58:10 UTC); bis 12:00 UTC gibt es **kein v2.1.262** — der Git-Ref `v2.1.262` antwortet HTTP **404**, `time.modified` steht weiterhin auf 04.09. **19:57:25 UTC**, die Tags sind still (`latest` = `next` = **2.1.261**, `stable` **2.1.236**). `CHANGELOG.md` ist byte-identisch zum 06:00-Abruf (**630786 Bytes**, 2.1.261 -> 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252), die Releases-API listet deckungsgleich v2.1.261 an der Spitze. **Rhythmus — eine Korrektur an der naheliegenden Annahme:** Heute ist Samstag, und die Werktags-Serie seit 17.08. legt nahe, das Wochenende sei grundsätzlich leer. Das stimmt so nicht. Seit dem 01.07. fallen **fünf Publishes auf ein UTC-Wochenende**, davon liegen **drei auch nach US-Pazifikzeit am Wochenende**: 2.1.215 (19.07. 00:53 UTC = Sa 17:53 PT), 2.1.240 (22.08. 13:03 UTC = **Sa 06:03 PT**) und 2.1.241 (22.08. 23:58 UTC = Sa 16:58 PT) — der 22.08. war ein Samstag mit gleich zwei Publishes. 2.1.214 und 2.1.226 sind dagegen nur Scheintreffer: 00:13 bzw. 01:53 UTC sind Freitagabend an der Westküste. Die beiden letzten Wochenenden (23./24.08. und 29./30.08.) blieben leer. Ein Samstags-Release ist also unwahrscheinlich, aber belegt vorgekommen — der 18:00-Lauf prüft weiter, ein leerer Vormittag ist kein Beweis. **Fund dieses Laufs — eine Falle in der Quellenliste:** Die dokumentierte URL `platform.claude.com/docs/en/release-notes` liefert die Platform-Release-Notes **nicht mehr**. Sie antwortet mit **HTTP 307** und leitet auf `/docs/en/release-notes/system-prompts/overview` weiter — den Änderungsverlauf der **System-Prompts**, eine völlig andere Seite, die anschließend brav HTTP 200 und plausibel aussehenden Inhalt zurückgibt (**3858 Bytes** statt 103914). Die Weiterleitung gilt für die HTML-Seite **und** die `.md`-Variante. Ein naiver Abruf der URL aus der Quellenliste hätte damit stillschweigend die falsche Seite als „Platform Release Notes“ protokolliert — kein Fehlercode, keine leere Antwort, nur der falsche Inhalt. Die funktionierende Adresse ist `/docs/en/release-notes/overview.md`; aufgefallen ist der Tausch hier nur über die **Größenplausibilität** (3858 statt 103914 Bytes). Für künftige Läufe: Platform-Notes immer über `…/release-notes/overview.md` holen und die Bytezahl gegen den Vorlauf prüfen. Über die richtige URL ist die Seite **byte-identisch** (103914 Bytes), Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`) — der beim 06:00-Lauf erstmals gesehene Eintrag ist bestätigt, danach kam nichts. **What's-New** byte-identisch (14051 Bytes), Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — der Digest-Rückstand bleibt bei zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der Webflow-Stempel `Last Published` steht unverändert auf **03.09. 17:24 UTC**, seit Donnerstag also nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 0** — geändert wurde nur der Crawl-Header. — Ältere Crawl-Historie in den Git-Commits.)
