...
> Letzte Aktualisierung: 2026-09-06 18:00 UTC (**Crawl 06.09. 18:00 UTC — dritter Lauf des Sonntags, keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — der Git-Ref `v2.1.264` antwortet HTTP **404**, `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zum 06:00- und 12:00-Lauf (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze mit einem 60-Byte-Body — der bekannten Platzhalter-Zeile. Auch `v2.1.262` bleibt HTTP **404** und damit dauerhaft übersprungen. Die Lücke seit dem letzten Publish beträgt **15,9 Stunden**. **Fund dieses Laufs — der Sonntag ist der einzige Tag, an dem Claude Code praktisch nie ausgeliefert wird:** Über die **gesamte** Publish-Historie des npm-Pakets — **511 Versionen** seit Mai 2025 — fallen nach US-Pazifikzeit nur **8 auf einen Sonntag (1,6 %)** gegenüber 19 auf einen Samstag (3,7 %) und 87 bis 107 pro Werktag. Im laufenden Jahr 2026 sind es **2 von 226 (0,9 %)**, und der letzte Sonntags-Publish überhaupt war **v2.1.159 am 31.05.2026** — zum Crawl **97 Tage** her. Noch schärfer ist das Anschluss-Muster: Von den sechs Wochenend-Publishes seit Mai wurde **kein einziger** am Sonntag nachgefasst. Entweder folgte die nächste Version noch am selben Samstag (2.1.240 -> 2.1.241 nach 10,9 h) oder erst am **Montag zwischen 11:03 und 13:19 Uhr PT** (Lücken von 42,6 / 43,3 / 43,4 / 49,2 h). Damit ist der heutige Lauf statistisch der erwartbar leere: **Der Sonntag ist kein verzögerter Samstag, sondern ein echter Leertag.** Daraus folgt eine belastbare Prognose für v2.1.264 — dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08., v2.1.234–v2.1.239) weiterhin oben; `whats-new/2026-w35`, `-w36` und `-w37` antworten alle **HTTP 404**, der Digest-Rückstand bleibt bei rechnerisch drei Wochen. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der große Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **fünften Lauf in Folge** auf **03.09. 17:24 UTC** — seit Donnerstag nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Sonntags-Muster und Prognose für v2.1.264). — Vorheriger **Crawl 06.09. 06:00 UTC — Wochenend-Release: v2.1.263 ist da, v2.1.262 ist übersprungen.** Der Samstag ist nicht leer geblieben: **v2.1.263** wurde am **06.09. um 02:07:58 UTC** auf npm publiziert — das ist **Samstag, 19:07 Uhr US-Pazifikzeit**. Der 18:00-Lauf hatte genau dieses Abendfenster als einzige verbliebene Möglichkeit benannt (2.1.241 kam an einem Samstag um 16:58 PT); die Prognose ist eingetreten. Das GitHub-Release folgte **46 Minuten** später um **02:54:20 UTC** — deutlich schneller als die 2h09min bei v2.1.261. Die Tags stehen jetzt auf `latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**; `time.modified` **06.09. 02:53:41 UTC**; der Git-Ref `v2.1.263` antwortet HTTP **200**. Die Lücke zum Vorgänger betrug **32,3 Stunden**. **Der Inhalt ist allerdings nicht veröffentlicht:** `CHANGELOG.md` (630786 -> **630840 Bytes**, +54) enthält für 2.1.263 genau eine Zeile — „Bug fixes and reliability improvements“ — und das GitHub-Release ist wortgleich. Was in dieser Version steckt, ist von außen nicht feststellbar; der npm-Wrapper gibt nichts preis (7 Dateien, **182844 Bytes**, exakt wie v2.1.261, der Code liegt in den plattformspezifischen `optionalDependencies`). **Fund dieses Laufs — zwei Muster, die zusammengehören:** Erstens ist **v2.1.262 übersprungen** und bleibt es: keine npm-Version, kein `time`-Eintrag, kein Git-Tag (HTTP 404), kein Changelog-Abschnitt. Das ist kein Einzelfall — im Nummernraum **2.1.200 bis 2.1.263** (64 Nummern) sind nur **56 publiziert**, **acht fehlen** (230, 244, 249, 253, 254, 255, 256, 262), darunter ein Block von vier aufeinanderfolgenden. Übersprungene Nummern tauchen **nie nachträglich** auf, weder auf npm noch im Changelog. Praktische Folge: Aus einer Versionslücke lässt sich **nicht** schließen, dass ein Abruf etwas verpasst hat. Zweitens korrelieren die inhaltslosen Platzhalter-Releases mit der Uhrzeit: Von den **10 Versionen**, deren Changelog nur aus „Bug fixes and reliability improvements“ besteht (2.1.165, 167, 168, 190, 220, 226, 240, 241, 250, 263), wurden **acht nach 15:00 Uhr PT oder am Wochenende** publiziert, und **alle drei Wochenend-Publishes seit August** (2.1.240, 2.1.241, 2.1.263) sind Platzhalter. Ein Release außerhalb der Kernzeit bringt also mit hoher Wahrscheinlichkeit **keine dokumentierten Neuerungen** — für einen Crawler heißt das: Wochenend-Version einsammeln, aber keinen Feature-Ertrag erwarten. Dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 weiterhin oben; `whats-new/2026-w35`, `-w36` und `-w37` antworten alle **HTTP 404** — der Digest-Rückstand bleibt bei rechnerisch drei Wochen. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`). **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der `Last Published`-Stempel steht den vierten Lauf in Folge auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 2** (v2.1.263 und das Muster aus Nummernlücken und Platzhaltern). — Ältere Crawl-Historie in den Git-Commits.)
