...
> Letzte Aktualisierung: 2026-09-08 18:00 UTC (**Crawl 08.09. 18:00 UTC — neunter Lauf in Folge ohne inhaltliche Bewegung; erstmals seit elf Läufen meldet der Blog einen Byte-Diff, und er ist ohne jeden Inhalt.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es weiterhin nicht** — die npm-Version antwortet HTTP **404**, ebenso die Git-Refs `v2.1.264`, `v2.1.265` und `v2.1.266`; `time.modified` steht weiter auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**), `CHANGELOG.md` ist byte-identisch bei **630 840 Bytes**, und die Releases-API zeigt deckungsgleich denselben 60-Byte-Platzhalter-Body an der Spitze (Autor `ashwin-ant`). Die Lücke seit dem letzten Publish beträgt **63,9 Stunden Kalenderzeit** — nach dem Maßstab des Vorlaufs aber erst **11,0 Arbeitsstunden**, da die Uhr nach Wochenende und Labor Day erst Dienstag 00:00 PT wieder anlief. **Zur Prognose:** Der Lauf liegt bei 11:00 PT und damit **45 Minuten innerhalb** des erwarteten Dienstags-Fensters (17:15–20:37 UTC); der Schwerpunkt 12:17 PT = **19:17 UTC** ist noch eine Stunde und siebzehn Minuten voraus — die Prognose ist damit **weder bestätigt noch widerlegt**, die Entscheidung fällt beim Lauf am Mittwoch 06:00. **FUND DES LAUFS — der Blog hat sich geändert, aber an keiner Stelle, die zählt:** Er kam mit **763 786 statt 763 730 Bytes** und neuem `Last Published`-Stempel (**08.09. 16:11:45 UTC** statt 03.09. 17:24:14), der erste Stempelwechsel seit elf Läufen — der Diff besteht jedoch ausschließlich aus einem neuen CSS-Bundle (`claude-brand.shared.a080fda7e.min.css` statt `…01acb1de2`, mit neuem `integrity`-Hash) und einem durch einen Formatierer gelaufenen Inline-Skript; der **sichtbare Text ist byte-identisch** (19 726 Zeichen, gleicher Hash), die **Artikelliste ebenfalls** (dieselben 15 Slugs). Über 14 Snapshots und 13 Übergänge seit dem 01.09. schlug der **Roh-Hash 3-mal** an, der **`Last Published`-Stempel 3-mal**, die **Dateigröße 2-mal** — echt war jeweils **genau ein** Fall (02.09., `claude-for-commerce-agents` rein, `think-through-hard-problems-in-voice-mode` raus); Trefferquote **33 %** bzw. **50 %**, während der **Slug-Mengenvergleich 1-mal** anschlug und **100 %** traf. Falle dabei: Die **Anzahl** der Slugs ist konstant **15**, weil das Kartenraster feste Größe hat — bei der echten Änderung rotierte einer heraus; wer zählt statt Mengen zu vergleichen, sieht nie etwas. Ursache ist die Signaldichte: nur **2,6 %** der Seite sind sichtbarer Text, **97,4 %** sind Markup, Inline-Skripte und Asset-Verweise, die am Deploy-Zyklus hängen. Gegenprobe an den Markdown-Quellen im selben Zeitraum: `CHANGELOG.md` **4** Hash-Wechsel in 14 Übergängen, `overview.md` **2** in 13, `whats-new.md` **0** in 12 — **alle 6 echt**, jedes Mal mit gewachsener Datei, **null Fehlalarme**. Konsequenz: Wo eine `.md`- oder JSON-Repräsentation existiert, diese abrufen; für den Blog als einzige reine HTML-Quelle die Slug-Menge vergleichen (`grep -o 'href="/blog/[a-z0-9-]*"' | sort -u`, dann `diff`), niemals Größe, Roh-Hash oder Stempel. Im Übrigen unverändert: **What's New** byte-identisch (**14 051 Bytes**), Week 34 oben, `w35`/`w36`/`w37` weiterhin HTTP 404, realer Digest-Rückstand zwei Wochen (W35, W36); **Platform-Release-Notes** byte-identisch (**103 914 Bytes**) über `…/release-notes/overview.md`, oben weiter der 03.09. (`ant`-CLI 1.30.0), darunter der Block vom 01.09. zu Fable 5.1/Mythos 5.1; **Blog** inhaltlich unverändert, fünfzehn Slugs stabil. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Byte-Diff ohne Inhalt — wie man eine HTML-Quelle richtig überwacht). — Vorheriger **Crawl 08.09. 12:00 UTC — achter Lauf in Folge ohne Bewegung; gemessen in Arbeitszeit ist die Pause aber erst fünf Stunden alt.** Alle vier Quellen sind byte-identisch zum Vorlauf. Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — die npm-Version antwortet HTTP **404**, ebenso die Git-Refs `v2.1.264`, `v2.1.265` und `v2.1.266`; `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den sieben Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze (60-Byte-Platzhalter-Body, `ashwin-ant`). `v2.1.262` bleibt HTTP **404**. Die Lücke seit dem letzten Publish beträgt **57,9 Stunden**. **Fund dieses Laufs — Kalenderzeit ist das falsche Lineal für diese Pause:** Rechnet man die 57,9 Stunden in *Arbeitszeit* um, also nur Stunden auf US-Werktagen (Wochenende und Bundesfeiertage herausgenommen), bleiben davon **5,0 Stunden** übrig — v2.1.263 kam Samstagabend 19:07 PT, danach folgten Sonntag, der Labor-Day-Montag und erst seit Dienstag 00:00 PT läuft die Uhr wieder. Nach Kalenderzeit liegt die aktuelle Pause im oberen Zehntel (**26 von 225** Lücken des Jahres 2026 sind ≥ 57,9 h, **12 %**), nach Arbeitszeit ist sie **unauffälliger als 88 %** aller Lücken (199 von 225 sind länger als 5,0 h). Der Maßstabswechsel schrumpft die Streuung drastisch: Kalender-Lücken 2026 haben Median **22,5 h** und p90 **68,0 h** (Faktor 3,0), Arbeitszeit-Lücken Median **20,2 h** und p90 **26,5 h** (Faktor 1,3). **Praktisch entscheidend ist die Alarmschwelle:** Von den **28 Kalender-Lücken ≥ 48 h** des Jahres 2026 überschreiten nur **3** auch in Arbeitszeit 48 Stunden — eine 48-Stunden-Schwelle auf der Wanduhr feuert also 28-mal im Jahr und trifft in **11 %** der Fälle etwas Echtes, dieselbe Schwelle auf Arbeitszeit feuert **3-mal** und jedes Mal zu Recht (Weihnachtspause 22.12.–06.01. mit 192,5 Arbeitsstunden, die leere Woche 31 vom 24.07.–03.08. mit 143,1, und 20.–24.03. mit 48,8). Die übrigen 25 sind reine Wochenend- und Feiertagsartefakte; in ihnen stecken im Median **24,0 Arbeitsstunden**, also genau eine gewöhnliche Lücke. **Prognose, diesmal mit getrennt geschätzter Ausfallrate:** Von den 9 Dienstagen seit dem 01.07.2026 hatten **8 einen Publish** (89 %, leer blieb nur der 28.07. — mitten in der leeren Woche 31), und der jeweils erste Publish des Dienstags lag ausnahmslos zwischen **10:15 und 13:37 PT**, Median **12:17 PT**. Erwartet wird v2.1.264 also heute zwischen **17:15 und 20:37 UTC**, Schwerpunkt **19:17 UTC** — zum jetzigen Lauf (12:00 UTC = **05:00 PT**) ist das Fenster noch **fünf Stunden entfernt**, die Prognose damit weder bestätigt noch widerlegt; die Entscheidung fällt beim 18:00-Lauf, sicher sichtbar erst am Mittwoch 06:00. **What's New** ist byte-identisch (**14051 Bytes**), oben weiterhin **Week 34**; `2026-w35`, `-w36` und `-w37` antworten HTTP **404**, der reale Digest-Rückstand bleibt **zwei Wochen** (W35, W36). **Platform** ist byte-identisch (**103914 Bytes**) über `…/release-notes/overview.md`, oben unverändert der **03.09.** (`ant`-CLI 1.30.0), darunter der Block vom 01.09. zu Fable 5.1/Mythos 5.1. **Blog** ist byte-identisch (**763730 Bytes**), fünfzehn Slugs unverändert, der `Last Published`-Stempel steht den **zehnten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Kalenderzeit gegen Arbeitszeit als Maßstab für Release-Pausen). — Ältere Crawl-Historie in den Git-Commits.)

---

## Neueste Änderungen

### Woche 37 (8. September 2026) — Beobachtung zur Überwachung von HTML-Quellen

#### Der Blog hat sich zum ersten Mal seit elf Läufen geändert — und zwar an keiner einzigen Stelle, die zählt

- **Was:** Die Announcements-Übersicht auf `claude.com` kam in diesem Lauf mit **763 786 statt 763 730 Bytes** und einem neuen `Last Published`-Stempel: **08.09.2026 16:11:45 UTC** statt **03.09. 17:24:14** — der erste Stempelwechsel seit **elf aufeinanderfolgenden Läufen**. Redaktionell ist trotzdem **nichts** passiert. Der Diff besteht aus genau zwei Dingen: Das CSS-Bundle heißt jetzt `claude-brand.shared.a080fda7e.min.css` statt `…01acb1de2.min.css` (mit neuem `integrity`-Hash), und ein eingebettetes GSAP-Skript wurde umformatiert — aus `if (text) { tl.from(…); }` in einer Zeile wurden drei Zeilen mit ausgeklappten Klammern. Der **sichtbare Text ist byte-identisch**: 19 726 Zeichen vorher wie nachher, gleicher Hash. Die **Artikelliste ist identisch**: dieselben 15 Slugs, dieselbe Reihenfolge, dieselben Daten. Über alle 14 Snapshots dieser Serie (01.09.–08.09., **13 Übergänge**) ergibt sich damit ein klares Bild, welcher Prüfschritt taugt: Der **Roh-Hash** der Seite schlug **3-mal** an, der **`Last Published`-Stempel** ebenfalls **3-mal**, die **Dateigröße 2-mal** — echt war davon jeweils **genau ein Fall**, der 02.09., als `claude-for-commerce-agents` in die Liste rückte und `think-through-hard-problems-in-voice-mode` herausfiel. Trefferquote also **33 %** für Hash und Stempel, **50 %** für die Größe. Der **Slug-Vergleich** dagegen schlug **1-mal** an und lag richtig: **100 %**, ohne einen einzigen verpassten Fall. Dabei steckt eine Falle im Detail: Die **Anzahl** der Slugs ist über alle Snapshots konstant **15**, weil die Seite ein Raster fester Größe zeigt — bei der echten Änderung rotierte ein Artikel heraus, während ein neuer hereinkam. Wer zählt statt zu vergleichen, sieht **nie** etwas. Der Grund für die Störanfälligkeit ist eine Frage der Signaldichte: Von 763 786 Bytes sind **19 726 Zeichen sichtbarer Text — 2,6 %**. Die übrigen **97,4 %** sind Markup, Inline-Skripte und Asset-Verweise, die sich bei jedem Deploy der Website ändern können, ohne dass ein Artikel dazukommt. Zum Vergleich die drei Markdown-Quellen im selben Zeitraum: `CHANGELOG.md` wechselte in 14 Übergängen **4-mal** den Hash, `release-notes/overview.md` in 13 Übergängen **2-mal**, `whats-new.md` **kein einziges Mal** — und **alle 6 Wechsel waren echte inhaltliche Zuwächse**, erkennbar schon daran, dass die Datei jedes Mal wuchs (604 880 → 610 185 → 620 498 → 630 786 → 630 840 Bytes beim Changelog). **Null Fehlalarme bei den Textquellen, zwei von drei bei der HTML-Quelle.**
- **Einsatz:** Wo es eine Text- oder JSON-Repräsentation gibt, diese abrufen und nicht die HTML-Seite — für die drei anderen Quellen also `raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md`, `code.claude.com/docs/en/whats-new.md` und `platform.claude.com/docs/en/release-notes/overview.md` (jeweils mit `.md`-Endung), dazu `registry.npmjs.org/@anthropic-ai/claude-code` als JSON. Nur der Blog hat keine solche Entsprechung; dort nicht die Seite hashen, sondern die Artikelmenge extrahieren und **als Menge** vergleichen: `curl -sL https://claude.com/blog-category/announcements | grep -o 'href="/blog/[a-z0-9-]*"' | sort -u > heute.txt`, danach `diff gestern.txt heute.txt` — `comm -13` zeigt die Neuzugänge allein. Als zweite, gröbere Sicherung eignet sich ein Hash über den entblößten Text (Skripte, Styles und Kommentare entfernen, Tags durch Leerzeichen ersetzen, Whitespace normalisieren); der fängt zusätzlich Änderungen an Artikelüberschriften und -daten ab, reagiert aber auch auf Fußzeilen und Sprachumschalter. **Nicht** verwenden: Dateigröße, Roh-Hash, `Last Published`-Stempel, `ETag`, `Last-Modified` — und keine Slug-Anzahl.
- **Mehrwert:** Dies ist der Lauf, der die schon länger notierte Faustregel „der Blog täuscht Byte-Diffs vor" mit Zahlen belegt, und die Zahlen sind deutlicher als erwartet: **Zwei von drei Alarmen dieser Quelle sind falsch.** Bei einem Sechs-Stunden-Raster bedeutet das ein paar Fehlalarme pro Woche — genug, um die Meldung nach zwei Wochen zu ignorieren, und damit genau die Konstellation, in der man den echten Artikel dann übersieht. Wichtiger als die Prozentzahl ist aber die Einsicht dahinter, und sie gilt für jede gecrawlte Website: **Eine gerenderte HTML-Seite ist kein Dokument, sondern ein Build-Artefakt.** Ihr Byte-Inhalt hängt am Deploy-Zyklus des CMS, an Bundler-Hashes, an Formatierern und an A/B-Varianten — alles Dinge, die sich bewegen, während der Inhalt stillsteht. Wer eine solche Seite überwacht, muss vorher entscheiden, **welche Teilmenge der Bytes die Nachricht trägt**, und darf nur diese vergleichen; hier sind es 2,6 %, und die kleinste ausreichende Projektion ist die Liste der verlinkten Artikel. Praktisch heißt das für dieses Projekt: Der `Last Published`-Stempel wandert ab sofort in die Rubrik „interessant, aber kein Signal", und die Aussage „Blog unverändert" stützt sich auf den Slug-Vergleich. Am Rande zeigt der Vorfall noch, dass sich hinter dem Deploy vom 08.09. 16:11 UTC eine reine Tooling-Änderung verbirgt — ein neues CSS-Bundle und ein durch einen Formatierer gelaufenes Skript. Das ist kein Vorbote einer Ankündigung, sondern Bauarbeiten am Fundament.
- **Version:** Beobachtung zum Stand v2.1.263 (14 Blog-Snapshots 01.–08.09.2026, 13 Übergänge; Vergleich gegen 3 Markdown-Quellen, Stand 08.09.2026 18:00 UTC)

### Woche 37 (8. September 2026) — Beobachtung zur Messgröße

#### Kalenderzeit ist das falsche Lineal für Release-Pausen — in Arbeitszeit gemessen schrumpfen 57,9 Stunden auf 5

- **Was:** Seit v2.1.263 (Samstag, 05.09. 19:07 PT) sind zum Crawl **57,9 Stunden** ohne Publish vergangen — nach Kalenderzeit eine der längeren Pausen des Jahres, im oberen Zehntel: **26 von 225** Lücken des Jahres 2026 sind so lang oder länger (12 %). Rechnet man dieselbe Spanne aber in **Arbeitszeit** um — also nur Stunden, die auf einen US-Werktag fallen, Wochenenden und Bundesfeiertage herausgenommen —, bleiben davon **5,0 Stunden**: Auf den Samstagabend folgten Sonntag und der Labor-Day-Montag, die Uhr läuft erst seit Dienstag 00:00 PT wieder. Nach diesem Maßstab ist die Pause **unauffälliger als 88 %** aller Lücken des Jahres (199 von 225 sind länger als 5,0 Arbeitsstunden). Der Maßstabswechsel bringt die Verteilung erst zum Sprechen: Kalender-Lücken 2026 haben Median **22,5 h** bei p90 **68,0 h** (Streuungsfaktor 3,0), Arbeitszeit-Lücken Median **20,2 h** bei p90 **26,5 h** (Faktor 1,3) — die Auslieferung läuft also deutlich regelmäßiger, als die Wanduhr suggeriert, und fast die gesamte scheinbare Unregelmäßigkeit ist Wochenende. Am schärfsten zeigt sich das an der Alarmschwelle: Von den **28 Kalender-Lücken ≥ 48 Stunden** des Jahres 2026 überschreiten nur **3** auch in Arbeitszeit 48 Stunden — die Weihnachtspause 22.12.–06.01. (192,5 Arbeitsstunden), die leere Woche 31 vom 24.07.–03.08. (143,1) und die Spanne 20.–24.03. (48,8). Die übrigen **25 sind reine Wochenend- oder Feiertagsartefakte** und enthalten im Median gerade **24,0 Arbeitsstunden**, also eine völlig gewöhnliche Lücke.
- **Einsatz:** Die Wartezeit eines Watchers nicht auf der Wanduhr messen, sondern auf dem Kalender, dem der Prozess folgt. In Python etwa: `pip install holidays`, dann die Spanne tageweise durchlaufen und nur Tage zählen, für die `d.weekday() < 5 and d not in holidays.US()` gilt (Zeitzone `America/Los_Angeles`, nicht UTC — die Tagesgrenze entscheidet). Die Publish-Zeitstempel als Datenbasis liefert `npm view @anthropic-ai/claude-code time --json`. Für eine Warnung „hier stimmt etwas nicht" die Schwelle auf diese Arbeitszeit legen: **48 Arbeitsstunden** ist ein brauchbarer Wert, **26,5** entspricht dem p90 des normalen Betriebs.
- **Mehrwert:** Das ist der Unterschied zwischen einem Alarm, den man abschaltet, und einem, dem man glaubt. Eine 48-Stunden-Schwelle auf der Wanduhr feuert 2026 **28-mal** und liegt in **25 dieser Fälle** daneben — Trefferquote **11 %**, praktisch jedes Wochenende ein Fehlalarm, und nach drei Monaten ignoriert man die Meldung. Dieselbe Schwelle auf Arbeitszeit feuert **3-mal im Jahr** und jedes Mal zu Recht: Trefferquote **100 %** bei identischer Erkennungsleistung, denn alle drei echten Ausfälle bleiben drin. Der Nutzen kostet nichts außer einer Feiertagsliste. Zugleich ist es die Fortsetzung der methodischen Lehre des Vorlaufs: Dort ging die Prognose schief, weil die Ausfallrate nicht getrennt geschätzt wurde; hier verzerrt dieselbe Sorte Denkfehler die Diagnose — man misst eine Wartezeit auf einer Uhr, die weiterläuft, während der beobachtete Prozess steht. **Wer ein Ereignis beobachtet, das einem Arbeitskalender folgt, muss auch in dessen Zeit rechnen.** Konkret heißt das für den aktuellen Stand: Es liegt kein Hinweis auf eine Störung vor, es ist schlicht noch früh am Dienstag.
- **Version:** Beobachtung zum Stand v2.1.263 (225 Publish-Lücken des Jahres 2026, Kalender- gegen Arbeitszeit, Stand 08.09.2026 12:00 UTC)



### Woche 37 (8. September 2026) — Beobachtung zum Freigabe-Kalender

#### An US-Feiertagen wird nicht ausgeliefert — und warum die Montags-Prognose danebenlag

- **Was:** Der 07.09.2026 war **Labor Day**, ein US-Bundesfeiertag — und blieb komplett ohne Publish, obwohl im Lauf zuvor genau für diesen Montag ein Release erwartet worden war. Der Abgleich aller **20 auf einen Werktag fallenden US-Bundesfeiertage seit Februar 2025** gegen die npm-Publish-Historie zeigt, dass das kein Zufall ist: **17 davon (85 %) blieben ohne einen einzigen Publish**, darunter Memorial Day, Juneteenth, Independence Day, Columbus Day, Veterans Day, Thanksgiving samt Folgetag, MLK Day und Labor Day 2026. Zum Vergleich die Grundrate: Von den **178 Werktagen** des Jahres 2026 bis zum 07.09. blieben nur **20 leer (11 %)**. Die **längste Lücke der gesamten Historie — 336,5 Stunden** zwischen v2.0.76 und v2.0.77 — beginnt am 22.12.2025 und deckt exakt Weihnachten bis Neujahr ab; auch der Thanksgiving-Block 2025 (120,7 h) und die Juli-4-Woche gehören zu den größten Lücken. Nur drei Werktags-Feiertage hatten überhaupt einen Publish: Labor Day 2025 (1.0.100), Presidents Day 2026 (2.1.44) und der auf Freitag vorgezogene 4. Juli 2026 (2.1.201). **Der zweite Teil des Befunds betrifft die eigene Fehlprognose.** Sie war ein Lehrbuchfall von Selektionsbias: Ausgewertet wurden nur die Uhrzeiten jener Montage, an denen tatsächlich publiziert wurde (stets 10:07–15:16 PT) — dass der **Montag zugleich der Werktag mit der höchsten Ausfallquote** ist, blieb außen vor. In 2026 blieben **7 von 36 Montagen (19 %)** ohne Publish, gegenüber **2 von 35 (6 %)** bei Dienstag und Mittwoch; auch in der Gesamtzahl ist der Montag mit **34 Publishes** der schwächste Werktag (Di 44, Mi 40, Do 49, Fr 43). **Drei der sieben leeren Montage 2026 sind Feiertage** (MLK 19.01., Memorial Day 25.05., Labor Day 07.09.), einer fällt in die publish-freie Woche 31.
- **Einsatz:** Vor einer Release-Erwartung den US-Feiertagskalender gegenprüfen, nicht nur den Wochentag — in Python etwa mit `pip install holidays` und `import holidays; d in holidays.US()`; die eigene Publish-Historie dazu über `npm view @anthropic-ai/claude-code time --json` und Umrechnung nach `America/Los_Angeles`. Für einen Crawler heißt das: an Werktags-Feiertagen dasselbe entspannte Raster fahren wie am Wochenende, statt bei ausbleibendem Release die Abrufkette zu prüfen.
- **Mehrwert:** Das Freigabeverhalten folgt sichtbar dem Arbeitskalender eines US-Teams, nicht einer Automatik — Wochenende, Feiertag und Feiertagsbrücke sind reale Pausen. Wer das nicht mitrechnet, zieht aus einem stillen Tag zwei falsche Schlüsse: entweder „mein Watcher ist kaputt“ oder „hier kommt gleich etwas Großes“. Praktisch wichtiger ist aber die methodische Lehre, die über dieses Projekt hinausgeht: **Eine Statistik über die Uhrzeiten stattgefundener Ereignisse sagt nichts darüber, ob das Ereignis überhaupt stattfindet.** Genau diesen Fehler hat die Montags-Prognose gemacht — sie war in ihrer Bedingung korrekt (*wenn* montags publiziert wird, dann nachmittags PT) und in ihrer Aussage trotzdem falsch. Wer aus Zeitreihen Erwartungen ableitet, sollte die Ausfallrate immer separat schätzen. **Korrigierte Erwartung für v2.1.264:** Nach einem leeren Feiertags-Montag kam der Anschluss dreimal am **Dienstag** (14.10.2025 10:18 PT, 20.01.2026 14:37 PT, 26.05.2026 14:38 PT), einmal am Mittwoch, einmal erst in der Folgewoche. Der erste Dienstags-Publish liegt seit 01.07. ausnahmslos zwischen **10:15 und 13:37 PT** (17:15–20:37 UTC), im Median 11:50 PT — erwartet wird v2.1.264 also am **Dienstag 08.09. zwischen etwa 17:00 und 21:00 UTC**, diesmal mit der ausdrücklichen Einschränkung, dass ein Anschluss erst am Mittwoch oder später ebenfalls im Rahmen liegt.
- **Version:** Beobachtung zum Stand v2.1.263 (511 npm-Publishes seit Feb. 2025, 20 Werktags-Feiertage, Stand 08.09.2026 06:00 UTC)
### Woche 37 (7. September 2026) — Beobachtung zur Veröffentlichungskette

#### npm läuft der GitHub-Releases-Seite immer voraus — im Median zweieinhalb Stunden

- **Was:** Vergleicht man für die letzten **100 GitHub-Releases** (v2.1.142 bis v2.1.263) den npm-Publish-Zeitstempel mit dem `published_at` des zugehörigen Releases, ergibt sich in **100 von 100 Fällen dieselbe Reihenfolge: erst npm, dann GitHub** — keine einzige Ausnahme. Der Verzug ist dabei erheblich und schwankt stark: **Median 144,5 Minuten** (2 h 25), Minimum **8 Minuten** (v2.1.258 am 01.09.), Maximum **739 Minuten = 12,3 Stunden** (v2.1.200 am 03.07.). Nur **8 von 100** Releases standen binnen einer halben Stunde auf GitHub, **35 von 100** brauchten länger als drei Stunden, und in **19 von 100** Fällen fiel der GitHub-Eintrag auf einen anderen UTC-Kalendertag als der npm-Publish. Die Kette wird jedoch sichtbar straffer: Der Monatsmedian sinkt von **194 Minuten im Mai** über 169 (Juni), 146 (Juli) und **121 (August)** auf **59 Minuten im September**; über die letzten zwanzig Releases liegt er bei 99 Minuten.
- **Einsatz:** Für „ist die Version draußen?" die npm-Registry abfragen (`npm view @anthropic-ai/claude-code version` oder `curl -sL https://registry.npmjs.org/@anthropic-ai/claude-code | jq -r '."dist-tags".latest'`), erst danach den Inhalt aus `CHANGELOG.md` bzw. dem GitHub-Release nachziehen. Wer eine Benachrichtigung an GitHub hängt, sollte die npm-Zeit als Referenz mitloggen, um den eigenen Verzug zu kennen: `curl -sL https://registry.npmjs.org/@anthropic-ai/claude-code | jq -r '.time | to_entries | last'`.
- **Mehrwert:** Die GitHub-Releases-Seite ist der naheliegendste Watch-Punkt — RSS/Atom-Feed vorhanden, Web-UI, Benachrichtigungen per Klick. Genau dieser Weg ist aber der langsamste: Im Median vergehen zweieinhalb Stunden, in denen `npm install -g @anthropic-ai/claude-code@latest` die neue Version längst liefert, während der Feed noch schweigt; im schlechtesten beobachteten Fall waren es zwölf Stunden. Bei einem Prüfraster von sechs Stunden verschiebt das ein Release regelmäßig um einen kompletten Zyklus. Zusammen mit der Beobachtung, dass vier Versionen aus diesem Bereich (2.1.182, 2.1.213, 2.1.242, 2.1.243) **gar kein** Release haben, gilt: **npm beantwortet „ist es draußen", GitHub nur „was steht drin".**
- **Version:** Beobachtung zum Stand v2.1.263 (100 gepaarte Releases v2.1.142–v2.1.263, Stand 07.09.2026 18:00 UTC)


### Woche 37 (7. September 2026) — Beobachtung zur Quellenlage

#### Changelog, npm und GitHub decken sich nicht — in beide Richtungen

- **Was:** Ein vollständiger Abgleich der drei Kanäle gegeneinander zeigt, dass keiner von ihnen die Versionsgeschichte vollständig abbildet. Die npm-Registry kennt **511 publizierte Versionen**, `CHANGELOG.md` enthält **387 Versionsabschnitte** — und die beiden Mengen sind nicht ineinander enthalten. **132 publizierte Versionen (25,8 %) haben keinen Changelog-Abschnitt**, wurden also ausgeliefert, ohne je beschrieben zu werden. Umgekehrt gibt es **8 Changelog-Abschnitte zu Versionen, die auf npm nie erschienen sind**: 0.2.21, 0.2.26, 0.2.63, 0.2.75, 0.2.82, 1.0.97 sowie **2.1.43** und **2.1.46**. Diese Abschnitte sind kein Formatierungsunfall, sondern tragen echten Inhalt — 2.1.43 beschreibt drei Fixes (AWS-Auth-Refresh-Timeout, Warnungen für Nicht-Agent-Markdown in `.claude/agents/`, Structured-Outputs-Header auf Vertex/Bedrock), 2.1.46 zwei Einträge (verwaiste Prozesse nach Terminal-Trennung unter macOS, claude.ai-MCP-Connectors) — aber weder Git-Tag noch Release noch npm-Paket, also **dokumentiert und trotzdem nicht installierbar**. Die Dokumentationsdisziplin hat über die Serien hinweg deutlich zugenommen: **0.2.x 66 %** undokumentierte Publishes, **1.0.x 33 %**, **2.0.x 16 %**, **2.1.x nur noch 8 %** (18 von 225); in den letzten 60 Publishes bleiben genau zwei übrig, **v2.1.213** (17.07.2026) und **v2.1.242** (24.08.2026) — beide ohne Changelog-Abschnitt, ohne Git-Tag und ohne GitHub-Release, sie existieren nur in der npm-Registry. Der praktisch folgenreichste Fall ist ein vierter Zustand: **v2.1.243** (24.08. 23:10 UTC) hat einen Changelog-Abschnitt und einen Git-Tag (`git/ref/tags/v2.1.243` -> HTTP **200**), aber **kein GitHub-Release** (`releases/tags/v2.1.243` -> HTTP **404**). Im Bereich der letzten 100 Releases (v2.1.142–v2.1.263) fehlen der Releases-API insgesamt vier npm-Versionen: 2.1.182, 2.1.213, 2.1.242 und 2.1.243.
- **Einsatz:** Für einen verlässlichen Versions-Watcher die npm-Registry als führende Quelle nehmen (`npm view @anthropic-ai/claude-code time --json` oder `curl -sL https://registry.npmjs.org/@anthropic-ai/claude-code`), Inhalte aus `raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md` ziehen und die Existenz einer Version über `api.github.com/repos/anthropics/claude-code/git/ref/tags/v<version>` prüfen — **nicht** über `releases/tags/…`; den Abgleich selbst reproduziert man mit einem Mengenvergleich von `time`-Keys gegen `^## \d+\.\d+\.\d+`-Überschriften
- **Mehrwert:** Wer die GitHub-Releases-Seite als Kanon behandelt — der naheliegendste Weg, weil sie am schönsten formatiert ist — verpasst reale Auslieferungen: v2.1.243 kam mit vollem Changelog-Inhalt heraus und taucht dort schlicht nicht auf. Wer umgekehrt aus dem Changelog auf Installierbarkeit schließt, greift bei 2.1.43 und 2.1.46 ins Leere. Für die Fehlersuche im Alltag ist die erste Richtung die wichtigere: Meldet jemand einen Bug „seit dem Update auf 2.1.242", findet man dazu **nirgends** eine Beschreibung — das ist kein Zeichen für eine erfundene Version, sondern der Normalfall bei acht Prozent der Publishes. Und für die Beurteilung der Quellenlage insgesamt: Die drei Kanäle sind nicht redundant, sondern ergänzen sich; erst npm plus Changelog plus Git-Tags ergibt ein vollständiges Bild. Das Muster reiht sich in die beiden Vorbefunde ein — übersprungene Versionsnummern und fehlende Digest-Wochen — und ergänzt sie um die Gegenrichtung: **Nicht nur fehlen Nummern, es gibt auch Beschreibungen ohne zugehörige Auslieferung.**
- **Version:** Beobachtung zum Stand v2.1.263 (Abgleich 511 npm-Publishes / 387 Changelog-Abschnitte / 100 GitHub-Releases, Stand 07.09.2026 12:00 UTC)


### Woche 37 (7. September 2026) — Beobachtung zur Dokumentation

#### Der wöchentliche Digest hat genau ein Loch — und dahinter steckt eine Regel

- **Was:** Die Seite `whats-new` sammelt seit **Week 13** (23.–27.03.2026) für jede Kalenderwoche einen Digest. Prüft man die Reihe Woche für Woche nach, ist sie bis Week 34 lückenlos — **mit einer einzigen Ausnahme: Week 31 (27.–31.07.2026) existiert nicht.** Die Übersichtsseite springt von Week 32 direkt auf Week 30, der Doku-Index `llms.txt` listet die Woche ebenfalls nicht, und `whats-new/2026-w31.md` antwortet HTTP **404**. Sechs Wochen später ist nichts nachgetragen worden. Der Grund lässt sich aus der npm-Registry belegen: **In ISO-Woche 31 wurde keine einzige Version publiziert.** Die bekannte 239-Stunden-Lücke zwischen **v2.1.220** (24.07. 23:11 UTC) und **v2.1.221** (03.08. 22:16 UTC) deckt genau diese Kalenderwoche ab. Zum Vergleich die Nachbarwochen: W30 fünf Publishes, W32 sechs, W33 sechs, W34 acht. Der Digest folgt also den Releases, nicht dem Kalender: **keine Releases, keine Wochenseite** — und die Woche wird nicht nachgereicht, sondern bleibt dauerhaft leer.
- **Einsatz:** Vorhandensein einer Wochenseite direkt prüfen mit `curl -sI https://code.claude.com/docs/en/whats-new/2026-w31.md`; die vollständige Liste der tatsächlich veröffentlichten Digests steht im Doku-Index `https://code.claude.com/docs/llms.txt` (nach `whats-new` filtern) — verlässlicher als das Hochzählen von Wochennummern
- **Mehrwert:** Wer den Digest als lückenloses Wochenprotokoll liest, zieht aus einer fehlenden Seite den falschen Schluss — „hier ist etwas verlorengegangen" oder „die Doku hinkt hinterher". Tatsächlich ist eine fehlende Woche im Regelfall die ehrliche Auskunft, dass nichts ausgeliefert wurde. Das ist dasselbe Muster wie bei den übersprungenen Versionsnummern: **Aus einer Lücke in einer Nummern- oder Wochenreihe lässt sich nicht auf einen verpassten Abruf schließen.** Für einen Crawler heißt das konkret: nicht auf eine Woche warten, die nie kommt, sondern gegen die Release-Historie gegenprüfen. Zugleich **korrigiert dieser Befund die eigene Buchführung der letzten fünf Crawl-Blöcke**: Der dort wiederholt genannte „Rückstand von drei Wochen" war zu hoch angesetzt, denn `2026-w37` antwortet nur deshalb mit 404, weil **Woche 37 am 07.09. erst beginnt**. Der reale Rückstand sind **zwei Wochen** — und der ist inhaltlich schwer: In **W35** (24.–28.08.) erschienen **8 Versionen** (v2.1.242–v2.1.251), in **W36** (31.08.–04.09.) **7** (v2.1.252–v2.1.263). Beide gehören damit zu den dichtesten Wochen der gesamten Serie, W36 enthält allein v2.1.261 mit 67 Changelog-Punkten. Hier fehlt also tatsächlich Redaktion, und wer auf den Digest wartet, um die letzten beiden Wochen einzuordnen, wartet auf die inhaltsreichsten Seiten seit Wochen — bis dahin bleiben `CHANGELOG.md` und die GitHub-Releases die einzige Quelle.
- **Version:** Beobachtung zum Stand v2.1.263 (Digest-Reihe Week 13–Week 34, npm-Publishes je ISO-Woche 2026)


### Woche 36 (6. September 2026) — Beobachtung zum Wochen-Rhythmus

#### Der Sonntag ist der einzige echte Leertag — und wann v2.1.264 zu erwarten ist
- **Was:** Über die vollständige Publish-Historie des npm-Pakets `@anthropic-ai/claude-code` hinweg — **511 Versionen** seit Mai 2025 — verteilen sich die Veröffentlichungen nach US-Pazifikzeit sehr ungleich über die Woche: **87 bis 107 pro Werktag**, **19 auf Samstag (3,7 %)** und nur **8 auf Sonntag (1,6 %)**. Für das laufende Jahr 2026 fällt der Sonntag noch weiter ab: **2 von 226 Publishes (0,9 %)**, und der jüngste davon war **v2.1.159 am 31.05.2026** — inzwischen über drei Monate her. Noch aussagekräftiger als der Anteil ist, **was auf ein Wochenend-Release folgt**: Von den sechs Wochenend-Publishes seit Mai 2026 (2.1.159, 168, 185, 215, 240, 241, 263) wurde **kein einziger am Sonntag nachgefasst**. Der Anschluss kam entweder noch am selben Samstag — 2.1.240 (06:03 PT) -> 2.1.241 (16:58 PT) nach 10,9 Stunden — oder erst am **Montag zwischen 11:03 und 13:19 Uhr PT**, mit Abständen von 42,6 / 43,3 / 43,4 / 49,2 Stunden. Passend dazu die Montage insgesamt: Seit dem 01.07. liegt der **erste** Publish eines Montags stets zwischen **10:07 und 15:16 Uhr PT** (17:07–22:16 UTC), im Median gegen **13:56 Uhr PT**.
- **Einsatz:** Publish-Zeitstempel selbst auswerten mit `npm view @anthropic-ai/claude-code time --json`; für einen Crawler oder ein Update-Skript daraus die Abfragefenster ableiten, statt an Wochenenden im Stundentakt zu pollen
- **Mehrwert:** Zusammen mit dem Platzhalter-Muster vom Morgen ergibt das ein brauchbares Bild des Freigabe-Rhythmus. **Der Sonntag ist kein verzögerter Samstag, sondern ein echter Leertag** — wer sonntags nichts findet, hat nichts verpasst und braucht keine Fehlersuche in seiner Abrufkette zu starten. Und für die nächste Version lässt sich daraus eine konkrete Erwartung ableiten: Rechnet man die typischen Wochenend-Anschlusslücken von 42–49 Stunden auf den Publish von v2.1.263 (06.09. 02:07:58 UTC) hoch, landet **v2.1.264 am Montag, 07.09., etwa zwischen 17:00 und 22:30 UTC** (10:00–15:30 PT). Praktisch heißt das: Ein 18:00-UTC-Abruf am Montag erwischt bestenfalls den Anfang dieses Fensters, verlässlich sichtbar wird die Version erst beim Lauf am **Dienstag 06:00 UTC**. Für Teams, die Updates manuell einspielen, ist der Montagnachmittag PT also der sinnvolle Zeitpunkt zum Nachsehen — nicht der Montagmorgen.
- **Version:** Beobachtung zum Stand v2.1.263 (Auswertung aller 511 npm-Publishes seit Mai 2025)


### Woche 36 (6. September 2026) — v2.1.263

#### v2.1.263: Samstagabend-Release ohne veröffentlichten Inhalt
- **Was:** **v2.1.263** wurde am **06.09. um 02:07:58 UTC** auf npm publiziert — nach US-Pazifikzeit **Samstag, 19:07 Uhr**, also ein echtes Wochenend-Release. Das GitHub-Release folgte **46 Minuten** später (02:54:20 UTC), spürbar schneller als der Abstand von 2h09min bei v2.1.261. Der Changelog-Eintrag besteht aus **einer Zeile**: „Bug fixes and reliability improvements“ — mehr gibt weder `CHANGELOG.md` (630786 -> 630840 Bytes) noch das GitHub-Release her. Der npm-Wrapper ist mit **7 Dateien und 182844 Bytes byte-genau so groß wie v2.1.261**; der eigentliche Code steckt in den plattformspezifischen `optionalDependencies` und ist aus den Metadaten nicht lesbar. **v2.1.262 wurde übersprungen** und existiert nirgends: kein npm-Publish, kein Git-Tag, kein Changelog-Abschnitt. Tags nach dem Release: `latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**.
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Ein Update ohne Beschreibung ist schwer einzuordnen — hier hilft der Kontext: Wochenend-Releases dieser Serie sind durchweg Platzhalter (siehe den folgenden Eintrag), also mit hoher Wahrscheinlichkeit ein Nachschieben von Korrekturen zu der ungewöhnlich dichten Woche vom 31.08. bis 04.09. (fünf Releases an fünf Werktagen, v2.1.261 allein mit 67 Changelog-Punkten). Wer auf v2.1.261 sitzt und kein konkretes Problem hat, verpasst nichts Dokumentiertes; wer nach dem Freitags-Update auf ein neues Fehlverhalten gestoßen ist, sollte v2.1.263 ausprobieren, bevor er einen Bugreport schreibt.
- **Version:** v2.1.263

### Woche 36 (6. September 2026) — Beobachtung zum Versions- und Freigabe-Rhythmus

#### Versionslücken sind normal, und Releases außerhalb der Kernzeit bringen selten Inhalt
- **Was:** Zwei zusammenhängende Muster, die sich über die npm-Registry belegen lassen. **Erstens: Versionsnummern werden regelmäßig übersprungen.** Im Nummernraum **2.1.200 bis 2.1.263** liegen 64 mögliche Nummern, publiziert sind aber nur **56**. Es fehlen **2.1.230, 2.1.244, 2.1.249 sowie 2.1.253 bis 2.1.256 und 2.1.262** — darunter also ein Block von vier aufeinanderfolgenden Nummern. Diese Versionen erscheinen **nie nachträglich**: kein npm-Publish, kein Eintrag in `time`, kein Git-Tag, kein Changelog-Abschnitt. **Zweitens: die inhaltslosen Releases häufen sich in den Randzeiten.** Zehn Versionen tragen als kompletten Changelog nur den Satz „Bug fixes and reliability improvements“ (2.1.165, 167, 168, 190, 220, 226, 240, 241, 250, 263). **Acht davon** wurden nach **15:00 Uhr US-Pazifikzeit oder am Wochenende** publiziert, und **alle drei Wochenend-Publishes seit August** — 2.1.240 (Sa 06:03 PT), 2.1.241 (Sa 16:58 PT) und 2.1.263 (Sa 19:07 PT) — sind Platzhalter. Umgekehrt gilt das nicht: Wochenend-Releases waren früher durchaus ausführlich dokumentiert (2.1.215 vom 18.07. etwa), das Muster ist also eine Beobachtung der letzten Wochen, keine Regel seit jeher.
- **Einsatz:** Aktuellen Stand aller Kanäle prüfen mit `npm view @anthropic-ai/claude-code dist-tags`; die vollständige Publish-Historie mit Zeitstempeln liefert `npm view @anthropic-ai/claude-code time --json`
- **Mehrwert:** Beides räumt mit einer naheliegenden Fehlannahme auf. Wer eine fehlende Versionsnummer sieht, vermutet schnell einen verpassten Abruf, ein zurückgezogenes Release oder eine stillschweigend entfernte Version — tatsächlich ist die Lücke der Normalfall und bedeutet schlicht, dass ein interner Build nie ausgeliefert wurde. Für Skripte, die Versionen durchzählen, heißt das: **niemals von `n+1` auf die nächste Version schließen**, sondern die Tag- bzw. Registry-Liste auswerten. Und die Randzeit-Beobachtung spart Arbeit: Ein Update, das Samstagabend erscheint, wird seinen Inhalt aller Wahrscheinlichkeit nach nicht dokumentieren — es lohnt nicht, auf Release Notes zu warten, die nicht kommen, und es lohnt umgekehrt auch nicht, ein solches Update für belanglos zu halten: Genau die Fehlerkorrekturen zu einer dicht gepackten Woche landen hier.
- **Version:** Beobachtung zum Stand v2.1.263 (Nummernraum 2.1.200–2.1.263)


### Woche 36 (5. September 2026) — Beobachtung zum Freigabe-Kanal

#### Der `stable`-Tag ist kein „letztes gutes Release“, sondern ein Zwei-Wochen-Puffer
- **Was:** Das npm-Paket `@anthropic-ai/claude-code` führt drei Dist-Tags: `latest`, `next` und `stable`. Beim Crawl am 05.09. zeigen `latest` und `next` beide auf **v2.1.261** (publiziert 04.09.), `stable` dagegen auf **v2.1.236** — eine Version vom **19.08. 18:45 UTC**, zum Crawl-Zeitpunkt **17 Tage alt**. Dazwischen liegen **19 publizierte Versionen** (2.1.237 bis 2.1.261). Der `stable`-Zeiger ist dabei nicht eingefroren: Er stand seit mindestens dem 23.08. auf 2.1.231 und ist am **03.09. auf 2.1.236 nachgerückt** — von einer Version vom 13.08. auf eine vom 19.08. Das Nachrücken holte also sechs Tage auf, während `latest` im selben Zeitraum um 25 Versionen weiterlief; der Abstand blieb praktisch konstant.
- **Einsatz:** `npm i -g @anthropic-ai/claude-code@stable` für den zurückgehaltenen Kanal, `@latest` für die aktuelle Version; den aktuellen Stand aller drei Kanäle zeigt `npm view @anthropic-ai/claude-code dist-tags`
- **Mehrwert:** Der Name legt nahe, `stable` sei schlicht die zuletzt als solide befundene Version — die Zahlen sagen etwas anderes: Es ist ein Kanal, der bewusst **rund zwei Wochen** hinter der Entwicklung bleibt und in Sprüngen nachzieht. Praktisch heißt das zweierlei. Wer auf `@stable` pinnt, bekommt einen erprobten Stand, aber eben auch **keinen** Fix, der gestern veröffentlicht wurde — der glibc-2.44-Absturz etwa war in v2.1.245 (25.08.) behoben und ist über `stable` bis heute nicht ausgeliefert. Und wer einen Bugreport schreibt, sollte die Kanalwahl mitnennen: „läuft auf der aktuellen Version“ meint auf `@stable` einen zweieinhalb Wochen alten Build. Für CI-Pins ist der Kanal dagegen genau richtig, weil er die Kadenz von zuletzt fünf Releases pro Woche auf wenige Sprünge im Monat glättet.
- **Version:** Beobachtung zum Stand v2.1.261 / `stable` = v2.1.236
...
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Damit ist die Woche vom 31.08. bis 04.09. an jedem Werktag mit einem Release belegt (v2.1.252, 257/258, 259, 260, 261) und die Serie seit dem 17.08. ungebrochen. Praktisch relevant sind vor allem `/skill-doctor` gegen aufgeblähten Kontext und die neuen Output-Limits — beides betrifft direkt, wie viel Kontext eine Session verbrennt.
- **Version:** v2.1.261

#### `/skill-doctor`: welche Skills ungenutzt bleiben und was sie an Kontext kosten
- **Was:** Der neue Befehl **`/skill-doctor`** listet die geladenen Skills auf, zeigt, **welche davon in der Session nie benutzt wurden**, und beziffert, **wie viel Kontext jeder einzelne kostet** — als Grundlage zum Ausmisten.
- **Einsatz:** `/skill-doctor`
- **Mehrwert:** Jeder installierte Skill belegt mit seiner Beschreibung dauerhaft Platz im System-Prompt, auch wenn er nie aufgerufen wird. Wer über die Zeit ein Dutzend Plugins gesammelt hat, zahlt diesen Aufschlag in jedem einzelnen Request. Bisher war das nur zu erahnen; jetzt gibt es eine Zahl pro Skill und damit eine Entscheidungsgrundlage, was raus kann.
- **Version:** v2.1.261

#### `bashOutputMaxChars` und `taskOutputMaxChars`: mehr Kommando-Ausgabe direkt im Kontext
- **Was:** Zwei neue Settings heben die Grenze an, **wie viel Ausgabe eines Bash-Kommandos bzw. einer Hintergrund-Task Claude inline zu sehen bekommt**, bevor der Rest in eine Datei ausgelagert wird — **bis zu 128.000 Zeichen**. Dazu kommt **`--append-subagent-system-prompt-file`**: Der System-Prompt-Zusatz für Subagenten wird aus einer **Datei** gelesen, für Prompts, die zu groß für die Kommandozeile sind.
- **Einsatz:** In `settings.json`: `"bashOutputMaxChars": 128000`, `"taskOutputMaxChars": 128000`; auf der Kommandozeile `claude --append-subagent-system-prompt-file ./subagent-prompt.md`
- **Mehrwert:** Der Auslagerungs-Mechanismus ist gut gemeint, kostet aber einen zusätzlichen Lese-Schritt — bei einem abgeschnittenen Test-Log muss Claude erst die Datei öffnen, um den eigentlichen Fehler zu finden. Wer lange Build- oder Testausgaben fährt und Kontext übrig hat, spart mit einem höheren Limit eine Werkzeugrunde pro Kommando. Das Datei-Flag löst ein handfestes Problem: `--append-subagent-system-prompt` scheiterte bei umfangreichen Prompts an der Längenbegrenzung der Shell.
- **Version:** v2.1.261

#### „Organization policy"-Zeile in `/status` und `claude doctor`
- **Was:** `/status` und `claude doctor` zeigen jetzt eine Zeile **„Organization policy"**, die benennt, **warum** die Richtlinie der Organisation nicht geladen werden konnte — etwa weil ein Proxy den Endpunkt nicht durchlässt. Passend dazu die geänderte Fehlermeldung: Ein **403 des Claude-Apps-Gateways** beim Laden der Managed Settings (beim Start oder nach `/login`) sagt jetzt, dass Claude Code für die Organisation womöglich **gar nicht freigeschaltet** ist, statt zu einer neuen Anmeldung zu raten.
- **Einsatz:** `/status`; `claude doctor`
- **Mehrwert:** In Firmennetzen mit TLS-Inspection war der Ausfall der Organisations-Policy bisher unsichtbar — Claude Code lief scheinbar normal, nur ohne die zentral gesetzten Regeln. Der irreführende „bitte neu anmelden"-Hinweis schickte Admins zusätzlich auf die falsche Fährte. Beide Änderungen zusammen machen aus einem stillen Fehlverhalten eine benennbare Ursache.
- **Version:** v2.1.261

#### Remote Control: sieben Fixes für Handy-, Browser- und claude.ai-Sessions
- **Was:** Der größte zusammenhängende Fix-Block der Version betrifft **Remote Control**: Der Permission-Modus wurde **veraltet angezeigt**, wenn sich Handy, Browser oder die claude.ai-App an eine Terminal-Session hängten oder der Modus im Terminal wechselte. Sessions erschienen nach einem vom Handy gestoppten Turn oder nach einem lokalen `/clear` **weiterhin als arbeitend** (hängender Spinner, Stop-Button). Eine per **`/teleport`** geholte Session wurde in die verbundene Session **hochgeladen** und wirkte auf Handy und Web an die ursprüngliche angehängt. Der eingehende **Event-Stream scheiterte hinter TLS-inspizierenden Firmen-Proxys** auf nativem Windows. Der **Effort-Level** wurde auf claude.ai als Standardwert angezeigt, obwohl er aus den Settings kam. **`SendMessage`** an eine Offline-Remote-Control-Session auf einem anderen Rechner meldete **„zugestellt"** — die Antwort sagt jetzt, dass die Zustellung wartet, bis der Rechner wieder verbindet. Und SDK-/Cloud-Sessions **ignorierten ein Stop oder Interrupt**, das kurz nach dem ersten Prompt kam, bevor der Turn begonnen hatte; der Turn lief bis zum Ende durch.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die falsche „zugestellt"-Meldung von `SendMessage` war die gefährlichste davon — wer eine Nachricht an einen Rechner schickt, der gerade offline ist, plante bisher mit einer Zustellung, die nie stattfand. Der Stop-Fix beendet das Ärgernis, dass ein sofortiger Abbruch nach dem Absenden wirkungslos war und den vollen Turn kostete.
- **Version:** v2.1.261

#### Fixes v2.1.261: Tastatureingabe, Bedrock-Wizard, Resume, CPU-Last, Feature-Flags
- **Was:** Weitere wichtige Korrekturen: Getippte oder eingefügte Zeichen landeten bei **schneller Eingabe oder Tastenwiederholung** gelegentlich **in falscher Reihenfolge oder fielen weg**. Der **Bedrock-Setup-Assistent** hing endlos, wenn AWS oder ein Credential-Helper nie antwortete (jetzt Timeout mit klarer Meldung), und seine Modellprüfungen scheiterten hinter TLS-inspizierendem Proxy. Das **Fortsetzen einer Session verlor Hook-Ausgaben und Kontext rund um parallele Tool-Calls**, wodurch sich der wiederaufgenommene Request änderte. `claude -p --resume <datei>` übernahm eine **fehlerhafte Session-ID** aus dem Transcript und setzt jetzt unter frischer ID fort. **Dauerhaft hohe CPU-Last**, wenn ein Hintergrund-Agent nicht fortgesetzt werden konnte und sein Aufwecken in einer engen Schleife wiederholt wurde. **Feature-Flags einer neueren Version** griffen gelegentlich auf eine ältere, parallel auf derselben Maschine laufende Claude-Code-Installation über. Dazu: `/add-dir <unterverzeichnis>` meldete fälschlich „couldn't be resolved" bei `/net`-Automounts; `/usage` und das VS-Code-Usage-Panel ließen modellspezifische Wochenlimit-Zeilen weg, wenn der Endpunkt rate-limitiert war; das Terminal-Fortschrittssymbol (iTerm2, Ghostty, ConEmu) zeigte die Session als fertig, während noch ein Workflow lief; claude.ai-Connectors blieben nach einem Timeout beim Start die ganze Session weg (jetzt Retry im Hintergrund); `gcpAuthRefresh` öffnete beim Start unnötig den Browser; Cloud-Sessions verwarfen ein von claude.ai synchronisiertes Plugin; das Zeichen direkt vor einem `[Image #N]`-Chip ließ sich nicht löschen; Teammates in Agent-Teams **wiederholten im zweiten Turn ihre Erstturn-Ankündigungen** und verfehlten dadurch den Prompt-Cache.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der Eingabe-Fix und der CPU-Fix betreffen jede Session. Der Resume-Fix ist der stillste und teuerste: Ein wiederaufgenommenes Gespräch schickte einen anderen Request los als das Original — sichtbar wird so etwas erst an unerklärlich abweichendem Verhalten nach `--resume`. Und der Agent-Teams-Fix schlägt direkt auf die Rechnung durch, weil ein geänderter Request-Präfix den kompletten Cache-Vorteil des Turns kostet.
- **Version:** v2.1.261

#### Verhaltensänderungen v2.1.261: Bash-Wortnavigation, `forceLoginMethod`, Diagramm-Links
- **Was:** Die **Wort-Bearbeitungstasten im Prompt folgen jetzt Bash**: **Ctrl+W** löscht zurück bis zum Leerzeichen, **Alt+F** und **Alt+D** halten am Wortende, **Satzzeichen trennen Wörter** — die Einstellung **`keybindingFlavor` hat keine Wirkung mehr**. Maschinen, deren Managed Settings `forceLoginMethod: "gateway"` setzen, **ignorieren einen übrig gebliebenen API-Key oder claude.ai-Login** und verlangen `/login` (Bedrock, Vertex AI und Foundry sind nicht betroffen). Der **Auto-Modus** behandelt einen Link, der Inhalte in die **URL eines öffentlichen Diagramm-Renderers** packt, als **Upload zu dieser Seite** — nicht mehr automatisch genehmigt, außer man hat ausdrücklich danach gefragt. **`/context`** zählt Tokens **lokal geschätzt**, wenn die Token-Counting-API nicht erreichbar ist, statt zusätzliche Requests an ein kleines Modell zu schicken.
- **Einsatz:** Automatisch aktiv; `keybindingFlavor` kann aus den Settings entfernt werden
- **Mehrwert:** Die Diagramm-Renderer-Regel ist die wichtigste: Ein Mermaid- oder PlantUML-Link kodiert den kompletten Diagramminhalt in der URL — Claude hätte damit im Auto-Modus unbemerkt Code oder interne Architektur an einen fremden Server geschickt. Bei den Tastenbelegungen lohnt ein kurzer Test: Wer `keybindingFlavor` gesetzt hatte, bekommt ab jetzt Bash-Verhalten, ohne dass die Einstellung noch etwas ändert.
- **Version:** v2.1.261

#### Verbesserungen v2.1.261: Modellnamen statt Gateway-IDs, Vertex-Start, `rm -rf`-Schutz
- **Was:** Der **`/model`-Picker und die VS-Code-Modell-Pille zeigen den Namen eines Modells** statt seiner rohen Bedrock-, Vertex-AI- oder Gateway-ID, sofern Claude Code sie erkennt. Der **Start auf Google Vertex AI** mit gesetztem `GOOGLE_APPLICATION_CREDENTIALS` wiederholt die Projekt-Ermittlung nicht mehr und startet keine zusätzlichen `gcloud`-Prozesse. Die **Streaming-Performance** steigt, weil bereits gerenderte Blöcke nicht bei jedem Update erneut vom Layout geprüft werden. Der **Sicherheits-Prompt für gefährliche `rm`-Aufrufe** erkennt jetzt auch `rm -rf` auf **Positionsparametern** und **innerhalb doppelt gequoteter `sh -c`-Skripte**. Und wenn die API **gar keine Response-Header** schickt, wartet der Retry bis zu `API_TIMEOUT_MS` (standardmäßig 10 Minuten) statt weiterer 3 Minuten, mit einer Meldung, die sagt, was zu ändern ist.
- **Einsatz:** Automatisch aktiv; `/model`
- **Mehrwert:** Der `rm -rf`-Fix schließt eine echte Lücke — ein `sh -c "rm -rf $1"` rutschte bisher am Sicherheitsdialog vorbei. Die lesbaren Modellnamen sparen Enterprise-Nutzern das Entziffern von IDs wie `us.anthropic.claude-…-v1:0` im Picker.
- **Version:** v2.1.261

#### VS Code v2.1.261: 26 Punkte — MCP-Verwaltung, Faltbare Prompts, Session-Archiv
- **Was:** Der bislang größte VS-Code-Block einer Version. **Neu:** ein Formular zum **Hinzufügen und Entfernen von MCP-Servern** direkt im MCP-Dialog, ohne die IDE zu verlassen; ein **Falt-Button an Permission- und Frage-Dialogen**, damit man das Gespräch dahinter lesen kann, ohne sie wegzuklicken (der Bereich neben dem Prompt scrollt jetzt die Konversation); **„Archive session"** im Kontextmenü der Session-Liste; eine **„Build a custom style"-Anleitung** im Output-Styles-Menü, die die Datei gleich schreibt und den Stil sofort listet; und ein **hohler Ring** in der Session-Liste für Sessions, die in einem Terminal, einem anderen VS-Code-Fenster oder Claude Desktop offen sind — bisher sahen sie geschlossen aus. **Geändert:** Der Modell-Picker ist eine **flache Liste aller Modelle**, ältere Schreibweisen stehen am Ende. Dazu 20 Fixes, überwiegend an der Session-Liste, an Tab-Zuordnung und Gruppen, am wiederkehrenden Frage-Dialog nach Panel-Reload, an der Usage-Anzeige in der Seitenleiste sowie am Modell-Picker, der von der Organisation deaktivierte Modelle bis zum zweiten Reload weiter anbot.
- **Einsatz:** VS-Code-Erweiterung aktualisieren
- **Mehrwert:** Der Falt-Button behebt eine tägliche Reibung: Bisher musste man einen Permission-Dialog beantworten oder verwerfen, um überhaupt lesen zu können, worauf er sich bezieht — also blind zustimmen oder blind ablehnen. Und die MCP-Verwaltung im Dialog erspart den Umweg über `claude mcp add` im Terminal.
- **Version:** v2.1.261

#### v2.1.261 auf npm — Freitags-Release im Rollout, Inhalt noch undokumentiert
- **Was:** **v2.1.261** wurde am 04.09. um **17:49 UTC** auf npm publiziert und schließt die Werktags-Serie dieser Woche zu einer lückenlosen Montag-bis-Freitag-Kette (v2.1.252, 257/258, 259, 260, 261). Der Release ist zum Zeitpunkt dieses Crawls erst **halb draußen**: Der npm-Tag **`next` zeigt auf 2.1.261**, **`latest` steht noch auf 2.1.260**, `stable` unverändert auf 2.1.236. Auf GitHub existiert **weder ein Tag `v2.1.261` noch ein Release** (beide API-Abfragen HTTP 404), und `CHANGELOG.md` enthält **keinen Abschnitt 2.1.261** — die Datei ist byte-identisch zum Abruf sechs Stunden zuvor. **Welche Features und Fixes in v2.1.261 stecken, ist damit noch nirgends veröffentlicht.** Das npm-Wrapper-Paket selbst gibt nichts preis: 7 Dateien, 182844 Bytes entpackt, exakt wie bei v2.1.260 — der Code liegt in den plattformspezifischen `optionalDependencies` (`@anthropic-ai/claude-code-linux-x64` usw., alle auf 2.1.261 gezogen).
- **Einsatz:** Wer jetzt schon testen will: `npm i -g @anthropic-ai/claude-code@next`. Für den regulären Kanal `claude update` bzw. `@latest` — das liefert bis zur Tag-Umstellung weiterhin v2.1.260.
- **Mehrwert:** Der Zwischenzustand ist die eigentliche Beobachtung: Anthropic schiebt eine neue Version zuerst unter `next` auf npm und zieht `latest`, GitHub-Tag, Release-Notes und Changelog erst danach nach — beim Vorgänger lag zwischen npm-Publish (22:32) und GitHub-Release (23:48) rund eine Stunde. Wer `@next` fährt, bekommt eine Version, zu der es noch keine Release-Notes gibt; wer `claude update` nutzt, merkt von dem Fenster nichts. Ein Upgrade blind auf `@next` lohnt hier nicht — der Inhalt sollte am 05.09. früh vollständig dokumentiert sein und wird dann hier nachgetragen.
- **Version:** v2.1.261

---

### Woche 36 (3. September 2026) — Platform-Release-Notes

#### Platform 03.09.: `ant apply` — Agenten, Skills und Deployments als Code
- **Was:** Version **1.30.0 der `ant`-CLI** bringt **`ant apply`**: **Agenten, Environments, Skills, Memory-Stores und Deployments** werden aus **Dateien im eigenen Repository** angelegt und aktualisiert. Man beschreibt jede Ressource in einer Datei, ruft `ant apply` auf und bestätigt den ausgegebenen Plan. Dabei entsteht eine **Lockfile `claude-lock.json`**, die man mitcommittet — damit fassen spätere Läufe, auf dem eigenen Rechner oder in der CI, **dieselben Ressourcen an, statt neue anzulegen**.
- **Einsatz:** `ant apply` (ab `ant` 1.30.0); `claude-lock.json` ins Repository committen. Dokumentation: [Manage resources as code with ant apply](https://platform.claude.com/docs/en/cli-sdks-libraries/cli/apply)
- **Mehrwert:** Das ist das Terraform-Muster für die Claude-Platform: deklarative Dateien, ein Plan zur Bestätigung, eine Lockfile für Identität über Läufe hinweg. Ohne sie erzeugte jeder CI-Durchlauf Duplikate — genau der Fallstrick, an dem imperative Setup-Skripte scheitern. Wer Agenten und Skills bisher per Console oder Ad-hoc-Skript pflegte, bekommt damit Review-fähige Änderungen, reproduzierbare Umgebungen und einen sicheren Weg, dieselbe Konfiguration in Staging und Produktion auszurollen.
- **Version:** Keine CLI-Version — Platform-Feature (`ant` CLI 1.30.0; Platform-Release-Note vom 03.09.2026)
...
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Wer v2.1.259 installiert hat und seitdem über unerklärliche Permission-Prompts bei `npm run build` oder `cd … && grep` stolpert, bekommt hier den Revert. Dazu kommen das lang erwartete Diff-Panel und mehrere Fable-5.1-Korrekturen, die direkt Kosten sparen.
- **Version:** v2.1.260

#### `/diff`: Diff-Panel neben der Konversation im Fullscreen-Modus
- **Was:** Im Fullscreen-Modus öffnet **`/diff`** ein Panel neben dem Gespräch, das die **nicht committeten Änderungen** des Arbeitsverzeichnisses live anzeigt, während Claude editiert. Erneutes `/diff` schließt es wieder. Passend dazu: `ctrl+l` / `cmd+k` leert im Fullscreen-Modus jetzt die Transcript-Ansicht wie ein Terminal-`clear`; ältere Nachrichten bleiben per Scrollen erreichbar.
- **Einsatz:** `/diff` in einer Fullscreen-Session (`/fullscreen` bzw. `fullscreen: true` in den Settings)
- **Mehrwert:** Bisher musste man für den Überblick in ein zweites Terminal mit `git diff` wechseln oder auf die IDE-Erweiterung ausweichen. Jetzt sieht man im selben Fenster, was Claude gerade anfasst — besonders im Auto-Modus ein Gewinn an Kontrolle.
- **Version:** v2.1.260

#### `/cost` nennt die wahrscheinliche Ursache von Prompt-Cache-Misses
- **Was:** `/cost` und das Statusline-Feld **`prompt_cache`** zeigen jetzt eine **vermutete Ursache** für Cache-Misses, etwa geänderte Tool-Definitionen, geänderter System-Prompt oder Leerlauf über die TTL hinaus. Ergänzend: `/effort` auf Claude Fable 5.1 invalidiert den Prompt-Cache beim Wechsel mitten in der Session nicht mehr, und auf Fable 5.1 wird der nach Tool-Results angehängte Kontext jetzt mitgecacht — er wurde bisher in jedem Tool-Call-Turn als ungecachter Input erneut gesendet.
- **Einsatz:** `/cost`; Statusline-Feld `prompt_cache`
- **Mehrwert:** Cache-Reads kosten auf Fable 5.1 nur 0,25 $/Mtok, ungecachter Input 10 $/Mtok. Ein unbemerkter Cache-Bruch pro Turn war damit ein Vierzigfach-Kostenfaktor; der Fix für Tool-Result-Kontext dürfte bei langen agentischen Sessions den Unterschied in der Monatsrechnung ausmachen.
- **Version:** v2.1.260

#### Revert: `Read()`-Deny-Regeln greifen nicht mehr auf Bash-Argumente
- **Was:** Die in v2.1.259 eingeführte Anwendung von **`Read()`-Deny-Regeln auf Bash-Argumente** wurde **zurückgenommen**. Sie blockierte `npm run build` unter einer Regel wie `Read(./**/build/**)` in jedem Modus und ließ `cd … && grep` selbst im Auto-Modus nachfragen. Weitere Permission-Korrekturen: `Edit`/`Write`/`Read`-Regeln mit **Klammern im Pfad** wurden als ungültig verworfen oder von der Bash-Sandbox ignoriert, wodurch „read-only"-Ordner beschreibbar blieben; eine einzige Regel mit **unkompilierbarem Muster** (etwa offenes `[`) ließ jede Dateiänderung mit `Invalid regular expression` scheitern — eine solche Deny-Regel schützt jetzt den wörtlichen Pfad; Regeln mit Text nach der schließenden Klammer (`Bash(ls) x`) werden als ungültige Settings gemeldet statt still ignoriert; zsh-Kommandos, die eine Command-Substitution in `REPORTTIME`/`REPORTMEMORY`/`DIRSTACKSIZE` verstecken, werden nicht mehr automatisch genehmigt.
- **Einsatz:** Automatisch aktiv; Windows-Nutzer mit Regeln wie `Edit(C:\dir\(name)\**)` bekommen einen Hinweis auf eine eindeutige Schreibweise
- **Mehrwert:** Wer nach dem Upgrade auf v2.1.259 Deny-Regeln gelockert hat, um wieder arbeiten zu können, sollte sie jetzt zurückdrehen. Der Klammer-Fix ist sicherheitsrelevant: Verzeichnisse mit Klammern im Namen (Next.js-Routen wie `(auth)`) waren trotz Regel beschreibbar.
- **Version:** v2.1.260

#### Fable 5.1: `/model`-Picker, `[1m]`-Pin, Auto-Compact vor 1M
- **Was:** Der **`/model`-Picker zeigt Fable 5.1** jetzt auch für berechtigte Organisationen an — bisher wurde das Modell nur getippt als `/model claude-fable-5-1` akzeptiert. Agenten mit `model: fable` ignorierten den **`[1m]`-Tag** eines `ANTHROPIC_DEFAULT_FABLE_MODEL`-Pins und liefen still mit 200K Kontext. **Auto-Compact** für 1M-Modelle: Opus- und Fable-Sessions komprimieren jetzt kurz vor der 1M-Grenze, und die Recovery-Compaction sehr großer Kontexte läuft nicht mehr nach zehn Minuten in den Timeout. Modellwechsel blieb nach einem fehlgeschlagenen Plugin-Hook-Load oder einem nicht ladbaren Org-Marketplace für den Rest der Session gesperrt — jetzt wird bei jedem Wechsel neu geprüft und die Ursache genannt.
- **Einsatz:** `/model`; `ANTHROPIC_DEFAULT_FABLE_MODEL=claude-fable-5-1[1m]`
- **Mehrwert:** Wer Fable 5.1 per Env-Pin mit 1M-Kontext nutzen wollte, hatte ohne Warnung nur ein Fünftel davon. Und ein Klick im Picker ersetzt das Auswendiglernen der Modell-ID.
- **Version:** v2.1.260

#### Fixes v2.1.260: Bedrock-CA, Managed Settings, `/rewind`, Agent-Teams
- **Was:** Weitere wichtige Fehlerbehebungen: **Bedrock**-Modellsuche, Token-Counting und AWS-SSO/STS scheiterten mit „unable to get local issuer certificate", wenn die Firmen-Root-CA nur im OS-Store lag. **Managed Settings** luden nicht für claude.ai-Enterprise/Team-Nutzer mit übrig gebliebenem API-Key aus einem früheren `/login`; `/status` markiert jetzt, welche der beiden Credentials tatsächlich aktiv ist. Managed `skillOverrides` per Alias (`checkup` für `/doctor`) griffen nicht, `Skill(name)`-Deny-Regeln deckten verschachtelte `<dir>:name`-Skills nicht ab. **`/rewind`** und `--rewind-files` meldeten Erfolg bei fehlenden Checkpoint-Dateien, obwohl nichts wiederhergestellt wurde, und hinterließen veraltetes File-Read-Tracking. `permissions.blockReadsOutsideWorkingDirectories` versteckte auf macOS die Git-Config und den eigenen Checkout worktree-isolierter Subagenten. **Agent-Teams:** Teammate-Transcripts verloren Nachrichten bei langen API-Retries; ein per SendMessage fortgesetzter Agent weckte den Aufrufer nie; Hintergrund-Sessions erschienen doppelt in ListAgents. Dazu: SDK-MCP-Server fehlten im ersten Turn; Emoji/Flaggen/Akzente brachen an Zeilenumbrüchen; Ctrl+Z im Fullscreen ließ die Shell auf dem Alternate Screen; `-p --resume` scheiterte dauerhaft nach verlorenen Worktree-Git-Metadaten; GitLab-Erkennung bei verschachtelten Subgroups und `owner/repo#123`-Links auf gitlab.com.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der Bedrock-CA-Fix ist für Enterprise-Setups hinter Proxy-CAs der Unterschied zwischen „läuft" und „läuft nicht". Der `/rewind`-Fix verhindert falsches Vertrauen in eine Wiederherstellung, die nie stattfand.
- **Version:** v2.1.260

#### Verhaltensänderungen v2.1.260: Bash-Modus außerhalb der Sandbox, Chrome-Admin-Schalter, Subagent-Zeitlimit weg
- **Was:** Kommandos am **`!`-Bash-Prompt** laufen jetzt **außerhalb der Sandbox**, auch bei striktem `sandbox.allowUnsandboxedCommands: false` — wie Tippen ins eigene Terminal. **Claude in Chrome** folgt dem Admin-Schalter der Organisation: Ist er aus, sind `--chrome`, `/chrome` und die Browser-Tools nicht verfügbar. Ein server-verwaltetes **`claudeMd`** löst den Sicherheitsdialog nicht mehr aus (Hooks, Shell-Kommandos, Sandbox und unsichere `env` weiterhin). **Hintergrund-Kommandos von Subagenten** haben kein Ein-Stunden-Limit mehr. Self-hosted-Runner: `--kill-session-after-min` gibt eine nur auf den Nutzer wartende Session frei statt sie als Fehler zu beenden. Neu: `/reload-plugins` und eine Textform von `/advisor` (`/advisor <model>`, `/advisor off`) in Headless-/Desktop-/Remote-Control-Sessions; `/ultrareview` wartet bis 45 Minuten; Gateway-Neuerungen `oidc.scope_on_refresh`, Desktop-Policy-Keys `userPluginMarketplacesEnabled`/`userPluginUploadsEnabled`, Bedrock-Token-Zählung per `bedrock:CountTokens`.
- **Einsatz:** `!`-Prompt; `/advisor <model>`; `/reload-plugins`
- **Mehrwert:** Die `!`-Änderung ist bewusst so gewollt, aber für Admins mit strikter Sandbox ein Punkt zum Wissen: Der interaktive Bash-Prompt ist jetzt eine Sandbox-Ausnahme. Wer Subagenten lange Testläufe starten lässt, wird nicht mehr nach einer Stunde abgeschnitten.
- **Version:** v2.1.260

---

### Woche 35 (26.–28. August 2026) — Blog-Ankündigungen ohne Claude-Code-Bezug (nachgetragen)

#### Blog: „Claude for Commerce Agents" (28.08.) und „Claude for Teachers" (26.08.)
- **Was:** Zwei Beiträge aus der Announcements-Kategorie, die die Läufe vom 27.08. und 03.09. wegen Zeitbudget nicht abgeglichen hatten: **„Claude for Commerce Agents"** vom 28.08.2026 und **„Claude for Teachers now available for schools and districts"** vom 26.08.2026. Beide betreffen Claude-Produkte außerhalb der CLI (Handels-/Einkaufsagenten bzw. das Bildungsangebot für Schulen und Bezirke) und enthalten keine Claude-Code-Funktionen, Flags oder Einstellungen.
- **Einsatz:** Nicht anwendbar
- **Mehrwert:** Nur zur Vollständigkeit der Quellenabdeckung aufgenommen, damit spätere Läufe die Slugs nicht erneut als „neu" melden. Die Blog-Liste steht damit weiterhin bei fünfzehn Beiträgen, Top ist der Commerce-Beitrag vom 28.08.
- **Version:** Keine CLI-Version — Blog-Beiträge (vom 26.08.2026 und 28.08.2026)
...
#### Überblick: v2.1.247 — 33 Punkte, die Serie geht weiter
- **Was:** Nach dem leeren Mittwochvormittag kam **v2.1.247** doch noch am 26.08.: **npm-Publish um 18:02 UTC** (11:02 Uhr US-Westküste), das **GitHub-Release fünf Stunden später um 23:06 UTC**. Damit hält die Werktags-Serie seit dem 17.08. ohne Unterbrechung. Der Changelog umfasst **33 Punkte**: **vier neue Funktionen, ein aktualisiertes Skill, zwei Verbesserungen, sieben Verhaltensänderungen und neunzehn Fehlerbehebungen**. Nach dem 61-Punkte-Aufräumrelease v2.1.246 ist das ein deutlich kleinerer, aber inhaltlich dichterer Release — mit dem `SendFeedback`-Tool und `/claude-api cost-optimize` sind zwei echte neue Funktionen dabei. **Dokumentationslage normal:** `CHANGELOG.md` und GitHub-Release stimmen Punkt für Punkt überein (beide 33 Einträge). Die älteren Lücken bleiben unverändert bestehen: Die `CHANGELOG.md` springt weiterhin 2.1.247 → 2.1.246 → 2.1.245 → 2.1.243 → 2.1.241, **v2.1.242 hat weiterhin keinen Abschnitt**, **v2.1.244 existiert nirgends**, und v2.1.242/243 haben nach wie vor kein GitHub-Release. `latest` und `next` zeigen beide auf **2.1.247**, `stable` bleibt seit über zwei Wochen bei **2.1.231**.
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Zwei Punkte machen das Update für Vielnutzer dringlich: Ein Hook oder Background-Agent, der megabyteweise Fehlerausgabe produziert, konnte die Session mit „Prompt is too long" endgültig lahmlegen — das ist behoben. Und wer Sonnet 5 auf dem 1M-Fenster fährt, bekommt rund 33.000 Token mehr Arbeitsraum vor dem Auto-Compact geschenkt. Wer Nix/home-manager oder GNU stow für seine Dotfiles nutzt, sollte ebenfalls zügig updaten — die Sandbox-Aufräumroutine löschte den `settings.json`-Symlink.
- **Version:** v2.1.247

#### Neues `SendFeedback`-Tool: Claude schreibt den Fehlerbericht selbst
- **Was:** Wenn in einer Session etwas schiefgeht, kann Claude jetzt **selbst einen Fehlerbericht entwerfen**. Der Entwurf landet in `/feedback`, wo man ihn **vor dem Absenden prüfen und bearbeiten** kann — abgeschickt wird nichts ohne Zutun. Abschaltbar über die Einstellung **`feedbackDrafts`**.
- **Einsatz:** Läuft automatisch; Entwürfe unter `/feedback` einsehen. Abschalten mit `"feedbackDrafts": false` in `settings.json`.
- **Mehrwert:** Der klassische Grund, warum Fehlerberichte nicht geschrieben werden, ist der Aufwand im falschen Moment: Man ist mitten in einer Aufgabe, etwas hakt, man arbeitet um das Problem herum und vergisst es. Claude hat den Kontext ohnehin schon — Was passiert ist, welcher Befehl lief, welche Fehlermeldung kam — und kann ihn ohne Zusatzarbeit zusammenschreiben. Wichtig ist die **Bestätigungsstufe**: Der Bericht geht durch die eigene Hand, bevor er Anthropic erreicht, was in Umgebungen mit heiklem Kontext der entscheidende Punkt ist. Wer das grundsätzlich nicht will, dreht `feedbackDrafts` ab.
- **Version:** v2.1.247

#### `/claude-api cost-optimize`: die API-Rechnung eines Projekts systematisch senken
- **Was:** Das `/claude-api`-Skill bekommt einen neuen Unterbefehl: **`cost-optimize`** analysiert die **Claude-API-Ausgaben eines bestehenden Projekts** und arbeitet die Kostenhebel **einzeln und gemessen** ab — Prompt-Caching, Token-Hygiene, Batch-Verarbeitung, Reasoning-Effort und Modellwahl. Statt alles auf einmal umzustellen, wird **eine Änderung nach der anderen** vorgenommen und ihre Wirkung überprüft.
- **Einsatz:** `/claude-api cost-optimize` im Projektverzeichnis
- **Mehrwert:** Kostenoptimierung an LLM-Integrationen scheitert in der Praxis selten am Wissen, sondern an der Methode: Man ändert vier Dinge gleichzeitig, die Rechnung sinkt um 30 %, und niemand weiß, welche der vier Änderungen es war — beim nächsten Refactoring fliegt die wirksame womöglich wieder raus. Das erzwungene „eine gemessene Änderung nach der anderen" ist genau die Disziplin, die man sich bei diesem Thema sonst selbst auferlegen müsste. Die genannten Hebel decken die üblichen Verdächtigen ab; Caching und Modellwahl sind erfahrungsgemäß die beiden mit der größten Spanne.
- **Version:** v2.1.247

#### `/claude-api`-Skill deckt jetzt auch die Admin API ab
- **Was:** Das `/claude-api`-Skill kennt jetzt zusätzlich die **Admin API**: Organisationsmitglieder, Einladungen, Workspaces, API-Keys, Rate-Limit-Berichte, Workload Identity Federation und kundenverwaltete Verschlüsselungs-Keys (CMEK).
- **Einsatz:** Automatisch aktiv beim Nutzen des `/claude-api`-Skills
- **Mehrwert:** Die passende Ergänzung zur Platform-Neuerung vom selben Tag, mit der die Admin API in die `ant`-CLI und sieben SDKs eingezogen ist (siehe eigener Eintrag): Wer jetzt Onboarding, Key-Rotation oder Rate-Limit-Anpassungen skriptet, bekommt die Referenz dafür direkt in der CLI, statt sie in der Dokumentation zusammensuchen zu müssen.
- **Version:** v2.1.247

#### Bash-Permission-Prompt weist auf den Auto-Modus hin — mit Ein-Tasten-Wechsel
- **Was:** Die Bestätigungsabfrage für Bash-Kommandos zeigt jetzt einen **Hinweis auf den Auto-Modus** und bietet die Option **„Yes, and switch to auto mode"** an — ein Tastendruck genehmigt das Kommando und stellt gleichzeitig auf den Auto-Modus um.
- **Einsatz:** Automatisch aktiv (erscheint im Bash-Permission-Prompt)
- **Mehrwert:** Der Auto-Modus wird genau dann interessant, wenn man zum fünften Mal in Folge dasselbe harmlose Kommando bestätigt — und genau in dem Moment steht der Hinweis jetzt da, statt in der Dokumentation. Zusammen mit dem neuen **Auto-Mode-Tab in `/permissions`** aus v2.1.246 wird der Modus damit spürbar zugänglicher: Man findet ihn dort, wo er weh tut, und kann anschließend nachsehen, welche Regeln er anwendet.
- **Version:** v2.1.247

#### `spinnerTipsOverride`: Organisationen können eigene Tipps rotieren lassen
- **Was:** Die Einstellung `spinnerTipsOverride` versteht jetzt strukturierte Einträge der Form **`{id, text, cooldownSessions, priority}`** sowie die neuen Felder **`tipsFile`** und **`label`**. Damit lassen sich eigene Tipps **neben den eingebauten** rotieren, mit Steuerung darüber, wie oft ein Tipp wiederkommt (`cooldownSessions`) und wie er gegenüber anderen gewichtet wird (`priority`).
- **Einsatz:** `spinnerTipsOverride` in den (verwalteten) Einstellungen; Tipp-Sammlung über `tipsFile` aus einer Datei laden
- **Mehrwert:** Die Wartezeile beim Nachdenken ist der einzige Ort in Claude Code, den wirklich jeder Nutzer täglich mehrfach ansieht — und bislang war sie entweder Anthropics oder komplett die eigene. Für Plattform-Teams ist die Mischung der interessante Teil: Man kann die eigenen Konventionen („Nutze `/review` vor dem Push", „Interne MCP-Server unter …") einstreuen, ohne die eingebauten Tipps zu verdrängen, und über `cooldownSessions` verhindern, dass derselbe Hinweis zum Rauschen wird.
- **Version:** v2.1.247

#### Fix: Hook oder Background-Agent mit riesiger Fehlerausgabe legte die Session lahm
- **Was:** Ein Hook oder ein Background-Agent, der **megabyteweise Fehlerausgabe** produzierte, konnte damit die Konversation überlaufen lassen — die Session steckte anschließend dauerhaft in **„Prompt is too long"** fest. Behoben. Zusätzlich behoben: **unbegrenztes Speicherwachstum**, wenn die Ausgabedatei eines Hooks oder Background-Tasks **nicht geschrieben werden konnte**; die Datei vermerkt jetzt, wo Ausgabe verlorenging.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Das war die unangenehmste Sorte Fehler: Ein einziger fehlkonfigurierter Hook — ein falscher Pfad, ein Linter, der auf ein generiertes Verzeichnis losgeht — und die laufende Session war nicht mehr zu retten, weil jeder weitere Turn am zu langen Prompt scheiterte. Wer Hooks produktiv einsetzt, hat das mit hoher Wahrscheinlichkeit mindestens einmal erlebt; die Ursache war von außen kaum zu erkennen.
- **Version:** v2.1.247

#### Fix: Sub-Agenten starben an einem 404 beim ersten Modellaufruf
- **Was:** Scheiterte der **erste Modellaufruf eines Sub-Agenten mit einem 404** (Modell nicht verfügbar), starb der Sub-Agent kommentarlos. Jetzt nutzt er die **Fallback-Modellkette der Session**, und die an den übergeordneten Agenten zurückgemeldete Fehlermeldung enthält **Fehlertyp, Status, Request-ID und Modellnamen**.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Trifft vor allem, wer über Bedrock, Vertex oder ein Gateway arbeitet und dort nicht jedes Modell freigeschaltet hat, oder wer Sub-Agenten mit einem `model:`-Eintrag im Frontmatter definiert hat, der auf der aktuellen Plattform nicht existiert. Bisher sah man nur, dass der Agent nichts lieferte; jetzt steht die Ursache samt Request-ID in der Rückmeldung — und meistens läuft er über die Fallback-Kette einfach weiter.
- **Version:** v2.1.247

#### Fix: Sandbox löschte den per Dotfiles verwalteten `settings.json`-Symlink
- **Was:** Die **Aufräumroutine der Bash-Sandbox** löschte nach einem Kommando eine **per Dotfiles verwaltete `~/.claude/settings.json`**, wenn diese ein **Symlink** ist, der aus dem beschreibbaren Bereich der Sandbox herauszeigt — also genau die Konstellation bei **Nix/home-manager** und **GNU stow**.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein Datenverlust-Fehler mit gutem Tarnanzug: Die eigene Konfiguration verschwand, aber weil sie ohnehin aus dem Dotfiles-Repo kommt, wirkte es wie ein Fehler des Dotfile-Managers — und ein `stow -R` bzw. `home-manager switch` stellte sie wieder her, bis zum nächsten Mal. Wer seine Claude-Konfiguration deklarativ verwaltet (was in dieser Nutzergruppe die Regel ist), sollte hier updaten.
- **Version:** v2.1.247

#### Fix: `/terminal-setup` überschrieb die komplette Zed-`keymap.json`
- **Was:** `/terminal-setup` schrieb die **gesamte `keymap.json` von Zed neu**, statt seine eine Tastenbelegung **einzufügen**. Jetzt wird gemergt.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Wer `/terminal-setup` in Zed ausgeführt hat, hat damit seine sämtlichen eigenen Tastenbelegungen verloren — ein Befehl, den man genau einmal ausführt und dessen Schaden man womöglich erst Tage später bemerkt. Ein Blick in die Versionsgeschichte der eigenen Dotfiles lohnt sich für Betroffene.
- **Version:** v2.1.247

#### Sonnet 5 kompaktiert jetzt erst bei rund 967K statt 934K Token
- **Was:** Das **Standard-Auto-Compact-Fenster von Sonnet 5** entspricht jetzt seinem **vollen 1M-Kontext**. Sessions auf dem 1M-Fenster kompaktieren dadurch erst bei etwa **967K Token** statt wie bisher bei rund **934K**.
- **Einsatz:** Automatisch aktiv (Sonnet 5 auf dem 1M-Kontextfenster)
- **Mehrwert:** Rund **33.000 Token zusätzlicher Arbeitsraum** vor der Kompaktierung — bei großen Refactorings oder Recherchen über viele Dateien kann das der Unterschied zwischen „läuft durch" und „verliert auf halber Strecke den Überblick" sein. Jede Kompaktierung kostet Kontexttreue; sie später auszulösen ist immer die bessere Variante.
- **Version:** v2.1.247

#### Cross-Session-Nachrichten kommen jetzt eingeklappt an
- **Was:** Nachrichten von anderen Sessions (Peer-Messaging) werden standardmäßig auf **eine Zeile eingeklappt**: `Message from @<sender>: <erste Zeile>`. Der vollständige Text lässt sich mit **Ctrl+O** aufklappen.
- **Einsatz:** Automatisch aktiv; Ctrl+O zum Aufklappen
- **Mehrwert:** Das Cross-Session-Messaging ist seit seiner Einführung nützlich, aber laut: Eine lange Nachricht einer Nachbar-Session schob den eigenen Arbeitsstand aus dem sichtbaren Bereich. Die Ein-Zeilen-Vorschau ist die richtige Standardeinstellung — man sieht, dass etwas kam und von wem, und entscheidet selbst, wann man hinschaut.
- **Version:** v2.1.247

#### Härtung: Terminal-Hyperlinks in gerendertem Markdown
- **Was:** Link-Ziele in gerendertem Markdown werden **als reiner Text dargestellt statt als anklickbarer Terminal-Hyperlink**, wenn sie auf einen **Netzwerk- oder Automounter-Pfad** zeigen, ein **Steuerzeichen** enthalten oder mit einem **unsichtbaren Zeichen** beginnen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Anklickbare Terminal-Hyperlinks sind bequem, aber sie zeigen nicht zwingend dorthin, wo der sichtbare Text hindeutet — bei Inhalten aus einer Websuche, einem MCP-Server oder einem fremden Repository ist der Linktext also kein Beleg für das Ziel. Der Fall „UNC-/Automounter-Pfad" ist dabei der interessanteste, weil ein Klick unter Windows allein durch den Zugriffsversuch Anmeldedaten preisgeben kann. Die Regel greift genau dort und lässt normale Links unberührt.
- **Version:** v2.1.247

#### Härtung: Plugin-Marktplätze mit unsichtbaren Zeichen im Namen
- **Was:** Marktplatz-**Namen mit Steuerzeichen oder unsichtbaren Zeichen werden abgelehnt**, und vom Marktplatz gelieferter Text in der Ausgabe von `/plugin` und `claude plugin` ist jetzt **escape-sicher**.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Fortsetzung der Plugin-Härtung der letzten Releases, an einer Stelle, die man leicht übersieht: Plugin-Marktplätze liefern Text, den Claude Code im Terminal ausgibt — mit ANSI-Escapes und unsichtbaren Zeichen lässt sich damit die Anzeige manipulieren, etwa um einen Marktplatz wie einen bekannten aussehen zu lassen. Wer Plugins aus fremden Quellen installiert, verlässt sich beim Prüfen genau auf diese Ausgabe.
- **Version:** v2.1.247

#### Bedrock, Vertex und Foundry: Claude erfährt jetzt, wenn ein MCP-Server nicht verbunden ist
- **Was:** In Sessions auf **Bedrock, Vertex und Foundry** — sowie überall dort, wo **Telemetrie deaktiviert** ist — wird Claude jetzt mitgeteilt, wenn ein konfigurierter **MCP-Server sich nicht verbinden konnte**. Bisher fehlte diese Information, und Claude schloss daraus, dass es die betreffenden Werkzeuge schlicht nicht gibt.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein besonders verwirrender Fehlerzustand: Claude behauptete, ein Werkzeug stehe nicht zur Verfügung, obwohl es in der `.mcp.json` konfiguriert war — und begann, die Aufgabe umständlich ohne dieses Werkzeug zu lösen, statt den Verbindungsfehler zu melden. Wer in Unternehmensumgebungen mit abgeschalteter Telemetrie arbeitet, hat dieses Verhalten regelmäßig gesehen, ohne die Ursache zu kennen.
- **Version:** v2.1.247

#### Cloud-Sessions: Container-Neustart zwischen zwei Turns verschluckte laufende Arbeit
- **Was:** Startete der Container einer **Cloud-Session** zwischen zwei Turns neu, während noch ein **Background-Agent, eine Shell oder ein Monitor lief**, verstummte die Session. Die wieder aufgenommene Session **meldet die verlorene Arbeit jetzt explizit**. Ebenfalls behoben: Cloud-Sessions zeigten mitunter den **vorherigen Permission-Modus**, wenn man direkt nach dem Absenden einer Nachricht umschaltete.
- **Einsatz:** Automatisch aktiv (Claude Code im Web, Desktop- und Mobil-App)
- **Mehrwert:** „Session verstummt und man weiß nicht, warum" ist der teuerste Fehler in einer Cloud-Umgebung, weil man nicht einmal entscheiden kann, ob man wartet oder neu startet. Dass der verlorene Arbeitsstand jetzt benannt wird, macht aus einem Rätsel eine Information.
- **Version:** v2.1.247

#### Terminal-Eingabe: Ctrl-Kürzel unter kyrillischem Layout, versprengte Maus-Codes
- **Was:** Zwei Eingabefehler behoben. (1) **Ctrl-Tastenkürzel** funktionierten in Terminals mit **Kitty-Protokoll** unter **nicht-lateinischen Tastaturlayouts** (etwa kyrillisch) nicht. (2) Traf ein **Maus-Report direkt nach dem Escape-Präfix aufgeteilt** über zwei Lesevorgänge ein, landete Text wie **`<35;150;7M`** im Eingabefeld.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der erste Punkt machte Claude Code für alle mit nicht-lateinischem Layout praktisch unbedienbar, sobald sie ein modernes Terminal (Kitty, Ghostty, WezTerm) nutzten — sämtliche Kürzel fielen aus. Der zweite ist der bekannte Fall, in dem beim Scrollen kryptische Zeichenfolgen im Prompt auftauchen, die man von Hand löschen muss.
- **Version:** v2.1.247

#### Fixes rund um Background- und benannte Sessions
- **Was:** Mehrere Fehler in Sessions abseits des Vordergrunds. (1) Eine Background-Session, deren **Terminal-Hostprozess gestorben** war, zeigte in `claude agents` **dauerhaft „opening…"**; die Zeile meldet jetzt innerhalb von Sekunden den Fehlschlag samt Grund, und **Enter startet sie neu**. (2) **Aus dem Vordergrund übernommene Shell-Kommandos** protokollierten beim Beenden in einer Background-Session einen internen Fehler oder die irreführende Zeile `[exited with code -1]`. (3) `/rename` **bestätigte stillschweigend**, wenn die Session-Registry nicht aktualisiert werden konnte — jetzt weist es darauf hin, dass andere Sessions weiter den alten Namen zeigen können. (4) **Self-hosted-Runner-Sessions** meldeten `running`, **bevor Claude Code gestartet war**, was in der Claude-Desktop-App eine verfrühte „Claude wartet auf deine Eingabe"-Benachrichtigung auslöste. (5) **Remote-Control-Sessions**, die per `/remote-control` gestartet wurden, meldeten den **Diff des Arbeitsverzeichnisses nicht** an verbundene Clients.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Background-Sessions und Remote Control sind die Bausteine, mit denen man Claude Code über mehrere Rechner und Terminals hinweg betreibt — und genau dort tut ein falscher Statusbericht am meisten weh, weil man den tatsächlichen Zustand nicht sehen kann. Besonders die endlose „opening…"-Zeile band Aufmerksamkeit an eine Session, die längst tot war.
- **Version:** v2.1.247

#### Fix: `/compact` und „Summarize from here" unter `--agent`
- **Was:** In Sessions, die mit **`--agent`** gestartet wurden, fassten `/compact` und „Summarize from here" den Verlauf **unter dem Standard-System-Prompt** zusammen statt unter dem **eigenen System-Prompt der Konversation**.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Wer mit einem spezialisierten Agenten arbeitet, hat dessen Rolle, Konventionen und Schwerpunkte im System-Prompt hinterlegt — und genau diese Perspektive ging bei der Kompaktierung verloren. Die Zusammenfassung betonte dann Dinge, die der Agent gar nicht als wichtig erachtet hätte; nach dem Compact arbeitete er faktisch mit einer fremden Sicht auf seine eigene Vorgeschichte.
- **Version:** v2.1.247

#### Fixes für Plugins, Anmeldung und Oberfläche
- **Was:** Sammelposten aus dem restlichen Changelog. (1) Das **Live-Cache-Verzeichnis eines versionslosen Marktplatz-Plugins** wurde bei einer **zweiten Installation in anderem Scope** gelöscht und neu angelegt, was eine laufende Session stören konnte. (2) **Schnelle Pfeiltasten-plus-Enter-Folgen** wirkten auf die **Zeile über** der angesteuerten — in der Verlaufssuche, in `/config`, `/mcp`, `/skills`, `/model` und der Background-Task-Liste. (3) `/install-github-app` **über SSH**: Die Kopier-Verknüpfung behauptet nicht mehr pauschal Erfolg, sondern nennt den tatsächlich verwendeten Weg, und die Anmelde-URL erscheint **sofort**, wenn kein Browser geöffnet werden kann. (4) Die **Ersteinrichtung** brach mit „Unable to connect to Anthropic services" ab, wenn verwaltete Einstellungen die Anmeldung über das Claude-Apps-Gateway vorschreiben und die Anthropic-Endpunkte **nicht erreichbar** sind. (5) Der **PR-Badge in der Prompt-Fußzeile** überspringt seine GitHub-Prüfung beim Zurückkehren ins Terminal, wenn die letzte Prüfung **unter einer Minute** her ist.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Punkt (2) ist der im Alltag ärgerlichste: Wer die Verlaufssuche oder `/model` zügig bedient, wählte gelegentlich den falschen Eintrag — ein Fehler, der sich nicht reproduzieren ließ, weil er nur bei hohem Tempo auftrat. Punkt (4) betrifft Unternehmensumgebungen, in denen die Ersteinrichtung hinter einem restriktiven Netzwerk stattfindet und die Fehlermeldung in die falsche Richtung wies.
- **Version:** v2.1.247

#### Anmeldung und Telemetrie: strengere Regeln in verwalteten Umgebungen
- **Was:** Vier Änderungen am Verhalten in verwalteten Installationen. (1) **Analytics bleiben ab dem Start abgeschaltet** — nicht erst nach der Anmeldung —, wenn verwaltete Einstellungen die Gateway-Anmeldung erzwingen oder ein eigener OAuth-Aufbau konfiguriert ist. (2) Anfragen zur **Gateway-Anmeldung weisen sich jetzt als Claude Code aus** (Parameter `surface=claude_code` bei der Geräteautorisierung, User-Agent `claude-code/<version>`). (3) Die **Durchsetzung der Organisationsanmeldung** beendet den Start, wenn die verwalteten Einstellungen der Administration **nicht gelesen werden können** — auch dann, wenn hostseitige oder nutzerbezogene Windows-Registry-Einstellungen vorliegen. (4) **OpenTelemetry-Ereignisse für aus claude.ai synchronisierte Plugins**: `plugin_id_hash` spiegelt jetzt den tatsächlichen Marktplatz, und `enabled_via` steht bei administrativ installierten Plugins auf `admin-install`.
- **Einsatz:** Automatisch aktiv (verwaltete Einstellungen)
- **Mehrwert:** Punkt (1) und (3) schließen dieselbe Lücke aus zwei Richtungen: Bisher konnte eine Installation, deren verwaltete Einstellungen nicht lesbar waren, faktisch ungeregelt weiterlaufen — jetzt fällt sie auf, statt still durchzurutschen. Punkt (2) ist für Administratoren praktisch, weil sich Claude-Code-Anmeldungen in den Gateway-Protokollen endlich von anderen Claude-Oberflächen unterscheiden lassen.
- **Version:** v2.1.247

---

### Woche 35 (26. August 2026) — Blog-Ankündigung

#### Cowork bekommt einen eigenen, eingebauten Browser
- **Was:** **Claude Cowork** in der Desktop-App hat ab sofort einen **eigenen Browser** — getrennt vom persönlichen Browser des Nutzers. Wenn eine Aufgabe Web-Interaktion erfordert, öffnet er sich in einer **Seitenleiste**; Claude navigiert dort selbstständig, füllt Formulare aus und liest Informationen aus. Anmeldedaten lassen sich **aus Chrome, Edge oder Firefox importieren**, damit Claude auf bestimmten Seiten angemeldet bleibt. **Verfügbarkeit:** Enterprise sofort (Freigabe durch die Administration in den Organisationseinstellungen), Pro/Max/Team im Lauf der Woche; **macOS, Windows und Linux** (Linux als Beta). Nach der Freischaltung ist der eingebaute Browser **standardmäßig aktiv**; wer stattdessen die Chrome-Erweiterung nutzen will, stellt das unter **Einstellungen → Cowork → bevorzugter Browser** um. **Grenzen:** **Banking-, E-Mail- und Single-Sign-on-Seiten sind standardmäßig ausgeschlossen**, solange man sie nicht ausdrücklich einbezieht; Tabs, Lesezeichen und Passwörter des eigenen Browsers bleiben unberührt.
- **Einsatz:** Claude-Desktop-App → Cowork; Browser-Wahl unter Einstellungen → Cowork → bevorzugter Browser. Kein Claude-Code-Befehl.
- **Mehrwert:** **Kein direkter Claude-Code-Bezug** — der Beitrag betrifft Cowork, und weder die CLI noch ihre Chrome-Anbindung sind berührt. Aufgenommen wird er hier aus demselben Grund wie die übrigen Cowork-Ankündigungen dieser Datei: Er ist das **Gegenstück zur GA von Claude in Chrome vom selben Tag** und macht deren Rollenverteilung erst verständlich. Anthropic bietet jetzt **zwei getrennte Wege**, Claude ins Web zu lassen — die Erweiterung, die **im eigenen Browser mit den eigenen Logins** arbeitet, und den eingebauten Browser, der **daneben** läuft und den eigenen unangetastet lässt. Für die Sicherheitsbetrachtung ist das der entscheidende Unterschied: Der eingebaute Browser hat eine **eigene, importierte Anmelde-Umgebung mit Ausschluss von Banking, E-Mail und SSO**, während die Erweiterung auf allem sitzt, wo man ohnehin angemeldet ist. Für Prompt-Injection gelten laut Anthropic **dieselben Abwehrmechanismen wie bei Claude in Chrome** — also die Kombination aus Probes und Safety-Classifier, deren Zahlen im Chrome-Beitrag offengelegt sind und die auch im Auto-Modus von Claude Code arbeitet. Die Empfehlung, mit vertrauenswürdigen Seiten anzufangen, gilt entsprechend.
- **Version:** Keine CLI-Version — Cowork-Feature (Blog-Announcement vom 26.08.2026)

---

### Woche 35 (26. August 2026) — Blog- & Platform-Ankündigungen

#### Claude in Chrome ist allgemein verfügbar — und darf jetzt ohne Rückfrage handeln
- **Was:** Die Chrome-Erweiterung **Claude in Chrome** verlässt die Pilotphase und ist ab sofort **auf jedem bezahlten Claude-Plan** verfügbar. Die eigentliche Neuerung steckt aber nicht in der Verfügbarkeit, sondern im Verhalten: Claude kann im Browser **Aktionen selbstständig ausführen**, statt für jeden Klick und jede Eingabe eine Bestätigung einzuholen. Ein **Safety-Classifier prüft jede Aktion vor der Ausführung** — er vergleicht sie mit dem ursprünglichen Auftrag und blockiert alles, was nicht dazu passt (etwa das Ansteuern einer fremden Website oder eine Texteingabe, die niemand verlangt hat). Anthropic benennt den Mechanismus ausdrücklich als **denselben, der im Auto-Modus von Claude Code arbeitet**. Wer das nicht will, schaltet die automatische Freigabe in den Einstellungen ab und bleibt beim manuellen Bestätigen. Der Anwendungsfall bleibt derselbe wie im Pilot: Claude erreicht damit interne Dashboards, Altsysteme und Lieferantenportale, für die es keinen Connector gibt, und arbeitet dort **mit den bereits im Browser vorhandenen Logins** — Seite lesen, Text tippen, Links klicken, navigieren, Formulare ausfüllen. **Grenzen:** kein Zugriff auf lokale Dateien oder andere Anwendungen (dafür weiterhin die Desktop-App), **keine anderen Chromium-Browser**, **kein Mobilgerät**.
- **Einsatz:** Erweiterung aus dem Chrome Web Store installieren; automatische Freigabe in den Einstellungen abschaltbar. Auf Enterprise-Plänen verwaltet die Administration die Erweiterung in den Organisationseinstellungen und kann sie **auf freigegebene Domains beschränken**.
- **Mehrwert:** Für Entwickler ist weniger die GA interessant als die Begründung dafür — Anthropic legt die **Prompt-Injection-Zahlen** offen, die den Schritt tragen, und die sind auch für den Auto-Modus in Claude Code aufschlussreich, weil dort dieselbe Klassifizierer-Technik läuft. Auf der aktuellen Evaluation (Angriffe von professionellen Red-Teamern) waren **ohne jede Schutzschicht** 17,6 % der Angriffe gegen Opus 4.5 und 3,8 % gegen Opus 5 erfolgreich; mit dem stärksten Schutz vom November 2025 lag Opus 4.5 immer noch bei 16,7 %. **Mit Probes plus Safety-Classifier** kam ab Opus 4.8 aufwärts **kein Angriff mehr durch** (Sonnet 5, Opus 5, Mythos 5 bei 0 %), bei Fable 5 blieben 0,3 % — sämtlich manuell als Fälle geringer Schwere verifiziert. Die ältere Cowork-Evaluation ist mit 0 % gesättigt und wurde stillgelegt. Praktische Lesart: Die Verteidigung besteht aus **zwei getrennten Schichten** — trainierte **Probes**, die Tool-Ergebnisse (Seiteninhalte, E-Mails) auf eingeschleuste Anweisungen absuchen und Claude zur Vorsicht mahnen, und der **Classifier**, der die geplante Aktion gegen den Auftrag hält. Erst beide zusammen ergeben die Null. Wer im eigenen Aufbau nur eine davon hat, sollte die 3,8-%-Zahl als Maßstab nehmen, nicht die 0 %.
- **Version:** Keine CLI-Version — Claude-in-Chrome/Cowork-Feature (Blog-Announcement vom 26.08.2026). **Nicht zu verwechseln** mit dem gleichnamigen Changelog-Punkt aus **v2.1.198** (1. Juli): Dort ging es um die CLI-seitige Chrome-Anbindung von Claude Code, hier um die Browser-Erweiterung als Produkt.

#### Platform: Compliance-API-Session-Endpunkte für Claude Code und Cowork verlassen die Beta
- **Was:** Die Endpunkte der **Compliance API zum Abruf von Session-Transkripten** sind für **Cowork- und Claude-Code-Sessions** jetzt **allgemein verfügbar** — der Beta-Vorbehalt entfällt.
- **Einsatz:** Compliance-API-Session-Endpunkte („Retrieve session transcripts") mit dem vorhandenen Compliance Access Key; kein Beta-Header mehr nötig.
- **Mehrwert:** Damit ist die am **11.08.** als Beta gestartete Funktion produktionsreif: Audit-, Records-Management- und eDiscovery-Pipelines lassen sich ohne Beta-Vorbehalt darauf aufbauen, was in regulierten Umgebungen oft die formale Voraussetzung für den Betrieb ist. Für Nutzer in Enterprise-Organisationen gilt unverändert: Die Transkripte ihrer Claude-Code-Sessions sind organisationsweit abrufbar — jetzt eben nicht mehr nur versuchsweise.
- **Version:** Keine CLI-Version — Platform-Feature (GA; Platform-Release-Note vom 26.08.2026)

#### Platform: Lokale Session-Endpunkte decken jetzt auch Claude Science und Claude für Microsoft 365 ab
- **Was:** Die Endpunkte für **lokale Sessions** der Compliance API (also Sessions auf den Rechnern der Nutzer) liefern zusätzlich die Transkripte von **Claude-Science-Sessions** (`product_surface` = `claude_science`) und von **Claude-für-Microsoft-365-Sessions** in **Excel, PowerPoint, Word und Outlook** (`product_surface` beginnt mit `office_agents`). Diese Erweiterung ist **Beta** für Claude-Enterprise-Organisationen und nutzt denselben Compliance Access Key mit dem Scope `read:compliance_user_data`.
- **Einsatz:** Bestehende Abfrage der lokalen Session-Endpunkte; die neuen Oberflächen über das Feld `product_surface` filtern.
- **Mehrwert:** Wer die Compliance-Abfrage bereits für Claude Code gebaut hat, bekommt die neuen Oberflächen ohne Umbau dazu — sollte aber wissen, dass **dieselbe Abfrage jetzt mehr zurückgibt als vorher**. Wer nach Claude-Code-Sessions filtern will, tut gut daran, `product_surface` explizit einzuschränken, statt sich darauf zu verlassen, dass nur CLI-Sessions in der Antwort stehen.
- **Version:** Keine CLI-Version — Platform-Feature (Beta, Claude Enterprise; Platform-Release-Note vom 26.08.2026)

#### Platform: Admin API jetzt in der `ant`-CLI und in sieben SDKs
- **Was:** Die **Admin API** ist nicht mehr nur per `curl` erreichbar, sondern in der **`ant`-CLI** und in den SDKs für **Python, TypeScript, C#, Go, Java, PHP und Ruby** verfügbar — jeweils unter `client.beta.organization`. Abgedeckt sind: Organisationsinfos, Mitglieder, Einladungen, Workspaces und deren Mitglieder, API-Keys, Rate Limits, Service-Accounts, Issuer und Regeln der **Workload Identity Federation** sowie **kundenverwaltete Verschlüsselungs-Keys**. **Noch nicht dabei:** die Nutzungs- und Kostenberichte sowie die Claude-Enterprise-Endpunkte für Nutzerverwaltung und Analytics — die bleiben `curl`-only. Die CLI und die SDKs lesen den Admin-API-Key aus `ANTHROPIC_API_KEY` oder einen `org:admin`-OAuth-Token aus `ANTHROPIC_AUTH_TOKEN`.
- **Einsatz:** `client.beta.organization.…` im SDK der eigenen Sprache; Key über `ANTHROPIC_API_KEY`, OAuth-Token über `ANTHROPIC_AUTH_TOKEN`.
- **Mehrwert:** Wer bislang Onboarding, Key-Rotation oder Rate-Limit-Anpassungen für sein Team automatisiert hat, tat das mit handgeschriebenen HTTP-Aufrufen samt eigener Fehlerbehandlung, Paginierung und Retry-Logik. Das entfällt — die Verwaltung der Organisation lässt sich mit denselben getypten Clients schreiben wie der übrige Anthropic-Code. Besonders praktisch für Claude-Code-Flotten: Service-Accounts, WIF-Regeln und Rate Limits sind genau die Objekte, die man beim Ausrollen auf ein größeres Team laufend anfasst. Dass Nutzungs- und Kostenberichte weiterhin `curl`-only sind, ist der Wermutstropfen — ausgerechnet die Abfragen für Kostenüberwachung bleiben handgestrickt.
- **Version:** Keine CLI-Version — Platform-/SDK-Feature (Beta-Namespace `client.beta.organization`; Platform-Release-Note vom 26.08.2026)
...
#### Blog: „Claude's memory works everywhere, and you decide what's in it" (25.08.)
- **Was:** Das **Gedächtnis von Claude** ist ab sofort **eines** für Chat und **Claude Cowork** — was Claude aus den Chats über einen weiß, steht auch bereit, wenn Cowork eine Aufgabe in der Cloud abarbeitet, und umgekehrt wandert zurück. Drei weitere Änderungen: (1) **Gedächtnis füllt sich während des Gesprächs** statt erst durch eine Zusammenfassung nach dessen Ende — wer nebenbei erwähnt, dass sich ein Termin verschoben hat, muss kein „merk dir das" hinterherschieben; pausieren und zurücksetzen geht jederzeit. (2) **Alles Gemerkte liegt offen** als Liste kurzer Dateien unter **Settings → Memory → Topics**, einzeln lesbar, editierbar und löschbar — eine Korrektur an einer Stelle wirkt in allen künftigen Gesprächen. (3) **Sensible Themen** (Gesundheit, Herkunft, Religion, Politik, Geschlechtsidentität u. ä.) speichert Claude **standardmäßig nicht**; per Schalter „include sensitive topics in memory" lässt sich das einschalten, gilt dann aber nur **ab Aktivierung** (nichts wird rückwirkend nachgetragen) und wird bei jedem Speichern angezeigt. Hart ausgenommen bleiben auch dann Ausweis-/Sozialversicherungsnummern, Vorstrafen, Aufenthaltsstatus und alles, was gegen die Acceptable Use Policy verstößt — Claude meldet, wenn es deshalb nichts speichert. **Verfügbarkeit:** an by default auf **Free, Pro und Max** (Web, Desktop, Mobile — für iOS/Android App-Update nötig), sensible Themen dort aus by default; auf **Team und Enterprise** entscheiden Admins über die Freigabe, für einzelne Nutzer ist das Gedächtnis dort **aus**, bis sie es selbst einschalten.
- **Einsatz:** **Settings → Memory** (ein-/ausschalten, Topics lesen/bearbeiten/löschen, „include sensitive topics" umlegen); Team/Enterprise vorab durch die Admin-Freigabe
- **Mehrwert:** **Kein direkter Claude-Code-Bezug** — die Ankündigung betrifft Chat und Cowork, nicht die CLI; das projektbezogene Gedächtnis von Claude Code (`CLAUDE.md`, `~/.claude/`) bleibt davon unberührt. Für den Alltag interessant ist es trotzdem an zwei Stellen: Wer Aufgaben zwischen Chat und Cowork hin- und herreicht, spart das Wiedererklären von Projektstand, Beteiligten und Konventionen. Und für alle, die Claude beruflich in einer Organisation nutzen, ist die Topics-Ansicht das eigentlich Wichtige — erstmals lässt sich nachsehen und korrigieren, was das Modell über einen angesammelt hat, statt es nur zu vermuten. Wer das nicht will, findet den Aus-Schalter an derselben Stelle; auf Team/Enterprise ist ohnehin nichts an, solange man es nicht selbst aktiviert.
- **Version:** — (Blog-Announcement, 25.08.2026; keine CLI-Version)
...
#### v2.1.241 — zweiter Release in Folge ohne aufgeschlüsselten Changelog
- **Was:** Die Version wurde am **22.08. um 23:58 UTC auf npm veröffentlicht** und am **23.08. um 00:52 UTC als GitHub-Release** nachgezogen. Damit ist der 22.08. der **erste Tag dieser Serie mit zwei Releases** — v2.1.240 kam morgens um 13:03 UTC, v2.1.241 kurz vor Mitternacht. Der Versatz zwischen npm-Publish und GitHub-Release schrumpft weiter: **54 Minuten** nach 1 h 42 min bei v2.1.240 und 2 h 36 min bei v2.1.239. Dokumentiert ist die Version wie ihre Vorgängerin mit **einem einzigen Punkt**: „Bug fixes and reliability improvements" — identisch in `CHANGELOG.md` und im GitHub-Release-Body. Ebenfalls geprüft: Der ebenso einzeilige Abschnitt zu **v2.1.240 wurde nicht nachträglich gefüllt**. Es sind also **zwei aufeinanderfolgende Versionen ohne jede Aufschlüsselung**; welche Fehler behoben wurden, geben die öffentlichen Quellen nicht her, und hier steht deshalb bewusst kein geratener Inhalt. `latest` und `next` zeigen beide auf 2.1.241, `stable` bleibt bei 2.1.231.
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Ein Routine-Update ohne angekündigte Funktion. Praktisch bedeutsam ist eher das Muster: Wer bisher am Changelog ablesen konnte, ob ein Update den eigenen Bug mitnimmt, muss das bei zwei Versionen in Folge ausprobieren. Für Setups, die diese Unsicherheit nicht wollen, bleibt der `stable`-Tag (2.1.231) der ruhigere Weg — er steht seit dem 22.08. still.
- **Version:** v2.1.241

### Woche 34 (23. August 2026) — What's-New-Digests Week 33 & Week 34 nachveröffentlicht

#### Digest-Rückstand beendet: Week 33 und Week 34 sind da
- **Was:** Beide überfälligen Wochen-Digests wurden veröffentlicht — **Week 33** (10.–14.08., v2.1.225–v2.1.233) und **Week 34** (17.–21.08., v2.1.234–v2.1.239). Der Week-33-Digest war zuletzt vierzehn Tage überfällig. Inhaltlich bringt jeder drei Schwerpunkt-Features plus elf bzw. dreizehn „Other wins"; der weit überwiegende Teil davon war über die Changelogs längst in dieser Datei dokumentiert (unter anderem `/goal`-Check-in-Intervalle, `CLAUDE_CODE_TOOL_MEMORY_LIMIT`, `additionalMarketplaces`, der Output-Style *Concise*, `spellcheck`, `ANTHROPIC_DEFAULT_MODEL`, `notify_when_idle`, Fork-Modus als Default, GitLab-Merge-Requests via `glab` und Markdown-Rendering eigener Prompts). Neu hinzugekommen sind die drei unten stehenden Einträge. **Week 31 bleibt unveröffentlicht** — die Digest-Liste springt weiterhin von Week 32 auf Week 30.
- **Einsatz:** [Week 33](https://code.claude.com/docs/en/whats-new/2026-w33) · [Week 34](https://code.claude.com/docs/en/whats-new/2026-w34)
- **Mehrwert:** Die Digests liefern zu bereits bekannten Changelog-Zeilen das, was dort fehlt: lauffähige Beispiele, kurze Demos und die Einordnung, warum eine Änderung den Arbeitsalltag betrifft. Wer die vergangenen zwei Wochen nur über den Changelog verfolgt hat, findet dort vor allem die drei Schwerpunkt-Features je Woche als geschlossene Erklärung.
- **Version:** — (What's New, Week 33 & Week 34)

#### `/design` — Artboards für die Oberfläche direkt aus der CLI (Research Preview)
- **Was:** Der Skill `/design` bringt den Artboard-Workflow von **Claude Design** in die CLI und in Claude Code Desktop; technisch setzt er auf Artifacts auf. Man ruft ihn mit einem Briefing auf, Claude veröffentlicht daraufhin eine Canvas mit **editierbaren Artboards** für die gewünschte Oberfläche und gibt einen Link darauf aus. Man öffnet die Canvas, wählt eine Variante, passt sie an — und lässt Claude anschließend genau diese Variante implementieren. Verfügbar auf Pro, Max, Team und Enterprise; setzt **v2.1.233 oder neuer** voraus. Der Skill ist als Research Preview gekennzeichnet.
- **Einsatz:** `/design redesign the composer based on what people actually use it for`
- **Mehrwert:** Schließt die Lücke zwischen „beschreib mir mal, wie es aussehen soll" und fertigem Code. Statt eine UI-Idee in Prosa zu verhandeln und dann direkt implementieren zu lassen, sieht man mehrere Entwürfe nebeneinander, entscheidet am Bild statt am Text und übergibt die Entscheidung ohne Werkzeugwechsel an die Implementierung. Besonders nützlich, wenn die eigentliche Unsicherheit im Layout liegt und nicht in der Logik.
- **Version:** ab v2.1.233 (Research Preview, Week-34-Digest)

#### Automatisch weiterarbeiten, sobald das Nutzungslimit zurückgesetzt ist
- **Was:** Läuft man in Claude Code Desktop in sein **Session-Limit**, bietet die Limit-Karte jetzt das Häkchen **„Auto-continue when limits reset"** an. Ist es gesetzt und bleibt die Session offen, wiederholt die Desktop-App den abgebrochenen Turn automatisch, sobald das Limit zurückgesetzt ist; die Karte zeigt dazu „Auto-resuming at" samt Uhrzeit. Für die **Wochen-Limit-Karte** gibt es die Option bewusst nicht. In der CLI gibt es das Gegenstück: Claude Code setzt die Session nach dem Zurücksetzen eines claude.ai-Nutzungslimits von selbst fort — abschalten lässt sich das über die Zeile **„Continue automatically at usage limit"** in `/config`.
- **Einsatz:** Desktop: Häkchen auf der Session-Limit-Karte · CLI: `/config` → „Continue automatically at usage limit"
- **Mehrwert:** Nimmt dem Limit seine teuerste Nebenwirkung — dass ein halb fertiger Turn liegen bleibt und man später von Hand rekonstruieren muss, wo Claude stehen geblieben war. Wer gegen Abend ins Limit läuft, lässt die Session einfach offen und findet die Arbeit fortgesetzt vor. Dass die Wochen-Limit-Karte die Option nicht anbietet, ist konsequent: Bei Wartezeiten in Tagen wäre ein offen gehaltener Turn wenig sinnvoll.
- **Version:** — (Desktop & CLI, Week-33-/Week-34-Digest)

#### Remote Control: Gerätekarte im Code-Tab — und raus aus der Research Preview
- **Was:** Jede Maschine, auf der `claude remote-control` läuft, erscheint jetzt als **Gerätekarte** ganz oben im **Code-Tab der Claude-App**. Ein Tipp darauf genügt, um ein Verzeichnis zu wählen und dort eine Session zu starten — man muss also nicht mehr an der Maschine selbst eine Session anlegen, um sie vom Telefon aus erreichen zu können. Zugleich verlässt **Remote Control die Research Preview**.
- **Einsatz:** Auf der Zielmaschine `claude remote-control` starten, dann in der Claude-App den Code-Tab öffnen und die Gerätekarte antippen
- **Mehrwert:** Der bisher fehlende erste Schritt. Remote Control konnte laufende Sessions fernsteuern, aber das Anlegen einer Session blieb an den Rechner gebunden — wer unterwegs eine Idee hatte, musste warten, bis er wieder davor saß. Mit der Gerätekarte beginnt die Arbeit auf dem Telefon und läuft auf der Maschine mit dem Repository. Das Ende der Research Preview ist dabei das eigentliche Signal für den produktiven Einsatz.
- **Version:** — (Mobile & Remote Control, Week-34-Digest)


### Woche 34 (22. August 2026) — v2.1.240

#### v2.1.240 — Release ohne aufgeschlüsselten Changelog
- **Was:** Die sechste Version in Folge (17.–22.08.) ist da, dokumentiert wird sie allerdings mit einem einzigen Satz: „Bug fixes and reliability improvements". Sowohl der Abschnitt `## 2.1.240` in der offiziellen `CHANGELOG.md` als auch der Body des GitHub-Releases enthalten **exakt diesen einen Punkt** und sonst nichts — zum ersten Mal in dieser Release-Serie gibt es also keine Aufschlüsselung der einzelnen Änderungen. Zum Vergleich: v2.1.239 brachte 59 Punkte, v2.1.238 39, v2.1.236 33. Welche Fehler behoben wurden, lässt sich aus den öffentlichen Quellen damit nicht sagen; hier steht bewusst kein geratener Inhalt. Ausgerollt ist die Version dagegen bereits vollständig: **npm-Publish am 22.08. um 13:03 UTC**, **GitHub-Release um 14:45 UTC**, `latest` und `next` zeigen beide auf 2.1.240, `stable` bleibt bei 2.1.231.
- **Einsatz:** `claude update` bzw. `npm i -g @anthropic-ai/claude-code@latest`
- **Mehrwert:** Für den Alltag ein Routine-Update — mangels Changelog gibt es keine neue Funktion, auf die man sich einstellen müsste, aber auch keinen benannten Fehler, dessen Behebung man gezielt abwarten könnte. Wer auf einen konkreten Bug aus v2.1.239 wartet, muss schlicht ausprobieren, ob dieses Update ihn mitnimmt. Für konservative Setups bleibt der `stable`-Tag (2.1.231) der ruhigere Weg.
- **Version:** v2.1.240

### Woche 34 (21. August 2026) — Blog-Ankündigungen

#### Blog: „Bringing the cybersecurity capabilities of Claude Mythos 5 to more defenders" (21.08.)
- **Was:** Anthropic öffnet **Claude Mythos 5** — bisher nur über das nach Einladung vergebene **Project Glasswing** zugänglich — für die Cyber-Verteidigung auf vier Wegen: in **Claude Security** für Kunden mit Claude-Enterprise-Plan, eingebettet in Produkte von Security-Partnern, über einen mit **35 Mio. USD** ausgestatteten **Defender Advantage Fund (0xDAF)** für Open-Source-Projekte sowie über ein erweitertes **Cyber Verification Program** für verifizierte Verteidiger. Der direkte Modellzugriff bleibt dabei bewusst geschlossen: Die Oberflächen lassen Mythos im Hintergrund für eine fest umrissene Aufgabe laufen und geben nur das jeweilige Artefakt heraus — etwa einen Schwachstellenbefund samt Patch-Vorschlag oder einen Security-Alert. **Direkter Claude-Code-Bezug:** Aus einem Befund in Claude Security heraus lässt sich **Claude Code on the web** öffnen, um den Fix tatsächlich umzusetzen; die menschliche Prüfung vor dem Einbau bleibt Pflicht.
- **Einsatz:** Claude Security auf einem Claude-Enterprise-Plan; aus dem Befund heraus „in Claude Code on the web öffnen" für den Patch
- **Mehrwert:** Für Teams mit Enterprise-Plan wird das stärkste Sicherheitsmodell erstmals ohne Einladung nutzbar — und zwar genau an der Stelle, an der ein Befund sonst liegen bleibt: Der Weg vom gefundenen Bug zum geschriebenen Patch führt ohne Werkzeugwechsel direkt in Claude Code. Der Zuschnitt „Artefakt statt Modellzugriff" bedeutet zugleich, dass man Mythos 5 nicht frei prompten kann — wer allgemeine Sicherheitsanalysen fahren will, bleibt beim regulären Modellangebot.
- **Version:** — (Blog-Announcement, 21.08.2026)

#### Blog-Nachtrag: „Self-service data analytics in Slack: how Anthropic deploys Claude Tag for ad-hoc questions" (13.08., Woche 33)
- **Was:** Erfahrungsbericht, wie Anthropic **Claude Tag** (Public Beta) als `@Claude` in Slack betreibt, damit Mitarbeitende Datenfragen stellen können, ohne Zugriff auf das Warehouse zu haben. Fünf Architektur-Entscheidungen stehen im Mittelpunkt: **Skills als laufend aktualisierte Inhalte** statt einmalig ausgelieferter Dateien (sie werden aus dem Daten-Repository gemountet und bei jedem Gespräch frisch gezogen); **Skills über SQL hinaus** (Forecasting, Kohorten- und Funnel-Definitionen, Chart-Konventionen, Schreibstandards für Analysen); **Anbindung interner Wissensindizes** (Incidents, Release Notes, Slack-Diskussionen), damit die Antwort auch das *Warum* einer Kennzahlbewegung liefert; **Berechtigungen** über einen auf freigegebene Daten beschränkten Service-Account, Spalten-Filter für PII, Kanal-Mitgliedschaft als Zugriffsrecht und durchgängige Query-Labels für das Audit-Log; sowie **Instrumentierung** mit strukturiertem Logging jeder Frage inklusive Skill-Version, Reaktions-Feedback und inline angezeigten Datenqualitäts-Warnungen. Der Beitrag war beim Crawl vom 13.08. übersehen worden — an jenem Tag erschienen zwei Announcements, dokumentiert wurde nur „Claude Tag now reads even more of the room".
- **Einsatz:** Nicht Claude-Code-spezifisch, kein Befehl — die Skill-Muster lassen sich aber auf `.claude/skills/` übertragen
- **Mehrwert:** Die interessanteste Übertragung auf den Entwickleralltag ist Punkt eins: Skills als *ausgelieferten Inhalt* zu behandeln, der sich bei jedem Lauf aktualisiert, statt als eingefrorene Datei. Wer Skills aus einem Repository mountet, hält Konventionen und Definitionen an einer Stelle aktuell, ohne jede Änderung in jeden Client zu tragen. Die Kombination aus Query-Labels und strukturiertem Logging ist zudem eine brauchbare Blaupause dafür, Agenten-Zugriffe auf produktive Daten nachvollziehbar zu machen.
- **Version:** — (Blog-Announcement, 13.08.2026)

### Woche 34 (18.–20. August 2026) — Blog- & Platform-Ankündigungen

#### Platform: Python-SDK 1.0 — `httpx` wird zu `httpx2` (20.08.)
- **Was:** Version 1.0 des Python-SDKs ist erschienen. Die HTTP-Schicht wechselt von `httpx` auf [`httpx2`](https://httpx2.pydantic.dev), einen gepflegten, API-kompatiblen Fork: Eigene `http_client`-, `Timeout`- und Transport-Objekte baut man jetzt aus `httpx2` (die `DefaultHttpxClient`-Helfer bleiben unverändert), und wer Tracing- oder Mocking-Bibliotheken nutzt, die `httpx` patchen, ruft beim Start `httpx2.alias_httpx()` auf. 1.0 verlangt Python 3.10 oder neuer und entfernt lange veraltete Oberfläche: die alte Text-Completions-API, die Parameter `temperature`, `top_p` und `top_k` an den Messages-Methoden sowie das clientseitige `compaction_control` des Tool-Runners. Im async-Client brauchen `.with_raw_response`-Ergebnisse jetzt ein `await response.parse()`, und `AnthropicBedrock` wirft einen Fehler, wenn keine AWS-Region konfiguriert ist, statt still auf `us-east-1` zurückzufallen.
- **Einsatz:** `pip install -U anthropic` — Details im [v1-Migrationsleitfaden](https://github.com/anthropics/anthropic-sdk-python/blob/main/MIGRATION.md); in Claude Code führt `/claude-api upgrade` (v2.1.239) die Migration durch
- **Mehrwert:** Der erste echte Major-Sprung des Python-SDKs. Die stillen Fallbacks fallen weg — vor allem die Bedrock-Region, die bisher unbemerkt auf `us-east-1` zeigen konnte, obwohl man ganz woanders arbeiten wollte. Der `httpx2`-Wechsel betrifft jeden, der einen eigenen HTTP-Client konfiguriert.
- **Quelle:** Platform Release Notes


#### Blog: „Build production agents with computer use, the Skills API, and the Files API" (20.08.)
- **Was:** Anthropic-Ankündigung zum Bauen produktionsreifer Agenten mit den am 19.08. GA gegangenen Bausteinen — Computer Use, Skills API und Files API. Der Beitrag bündelt die Platform-Neuerungen desselben Zeitraums zu einem durchgehenden Weg vom Prototyp zum Produktivbetrieb.
- **Einsatz:** Nicht Claude-Code-spezifisch, kein Befehl
- **Mehrwert:** Orientierung, welche der frisch GA gewordenen API-Bausteine zusammengehören, wenn man eigene Agenten über die Claude API betreibt.
- **Version:** — (Blog-Announcement, 20.08.2026)

#### Blog: „Anthropic's approach to teaching and learning AI" (20.08.)
- **Was:** Anthropic beschreibt seinen Ansatz für Lehre und Lernen mit KI — der erste Blog-Announcement seit dem 13.08. („Claude Tag now reads even more of the room").
- **Einsatz:** Nicht Claude-Code-spezifisch, kein Befehl
- **Mehrwert:** Kontext für alle, die Claude in Ausbildungs- oder Onboarding-Szenarien einsetzen.
- **Version:** — (Blog-Announcement, 20.08.2026)

#### Platform: Computer-Use-Tool allgemein verfügbar (19.08.)
- **Was:** Das Computer-Use-Tool ist auf der Claude API GA — als Toolset `computer_toolset_20260801`, ohne Beta-Header, mit Batch-Aktionen (mehrere Aktionen in einem Zug), standardmäßig aktivem `zoom` und Konfiguration je Toolset-Mitglied über `configs`. Ältere Beta-Versionen bleiben nutzbar; ein Upgrade ändert Request-Form und Tool-Behandlung (Migrationsleitfaden von `computer_20251124`).
- **Einsatz:** Toolset `computer_toolset_20260801` in der Messages-API-Anfrage
- **Mehrwert:** GUI-Automatisierung lässt sich ohne Beta-Vorbehalt produktiv einsetzen; Batch-Aktionen sparen Modell-Runden und damit Zeit und Kosten.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Neues Browser-Use-Tool (19.08.)
- **Was:** Mit `browser_toolset_20260801` steuert Claude einen Browser, den die eigene Anwendung hostet — statt eines ganzen Desktops nur das Browser-Viewport. Das Tool liest die Seite selbst (Accessibility-Tree, Elemente, Formulare, Tabs) und ergänzt Element-Referenzen, Formulareingaben, Tab-Verwaltung, Download-Meldungen und optionalen Datei-Upload zusätzlich zur reinen Screenshot-und-Klick-Steuerung. Beide Toolsets laufen auf Fable 5, Mythos 5, Opus 5, Sonnet 5 und Opus 4.8.
- **Einsatz:** Toolset `browser_toolset_20260801` (Client-Toolset, Browser wird selbst gehostet)
- **Mehrwert:** Web-Automatisierung wird robuster als reines Pixel-Klicken — Claude arbeitet auf der Seitenstruktur statt auf Bildschirmkoordinaten und muss keine vollständige Desktop-Umgebung bereitgestellt bekommen.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Files API allgemein verfügbar (19.08.)
- **Was:** Die Files API ist auf der Claude API GA. Der Beta-Header `files-api-2025-04-14` entfällt; ohne Header gilt das GA-Antwortformat mit Datei-Ablauf (`expires_in_seconds` beim Upload, `expires_at` am Objekt) sowie `page`/`next_page`-Pagination und `ids[]`-Filter beim Auflisten. Anfragen mit Beta-Header funktionieren im alten Format weiter.
- **Einsatz:** `POST /v1/files` ohne `anthropic-beta`-Header
- **Mehrwert:** Datei-Uploads sind produktionsreif, und Dateien können mit Ablaufdatum hochgeladen werden, statt manuell aufgeräumt zu werden.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Agent Skills und Skills API allgemein verfügbar (19.08.)
- **Was:** Agent Skills und die Skills API (`/v1/skills`) sind GA — der Beta-Header `skills-2025-10-02` ist nicht mehr nötig, auch nicht für Messages-API-Anfragen, die Skills über den `container`-Parameter laden.
- **Einsatz:** `/v1/skills` ohne Beta-Header
- **Mehrwert:** Skills lassen sich ohne Beta-Vorbehalt in produktive API-Integrationen einbauen.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Admin-API-Nutzerverwaltung für Claude Enterprise GA (19.08.)
- **Was:** Die Admin-API-Endpunkte zur Nutzerverwaltung für Claude-Enterprise-Organisationen (Mitglieder, Einladungen, Gruppen, eigene Rollen) sind allgemein verfügbar; der Header `anthropic-beta: ce-user-management-2026-07-13` entfällt für Gruppen- und Rollen-Anfragen.
- **Einsatz:** Admin API (`/v1/organizations/...`) ohne Beta-Header
- **Mehrwert:** Nutzer- und Rollenverwaltung lässt sich stabil automatisieren, etwa gegen ein IdP oder HR-System.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Domain-Beschränkung für `web_search` und `web_fetch` in Managed Agents (19.08.)
- **Was:** Über `allowed_domains` bzw. `blocked_domains` im `configs`-Array des `agent_toolset_20260401` lässt sich festlegen, welche Seiten ein Managed-Agents-Agent erreichen darf; `web_fetch` akzeptiert zusätzlich `max_content_tokens`, `web_search` ein `user_location`.
- **Einsatz:** `configs`-Eintrag pro Tool im Agent-Toolset setzen
- **Mehrwert:** Web-Zugriff eines Agenten lässt sich auf freigegebene Quellen eingrenzen — wichtig für Compliance und gegen Prompt-Injection über fremde Seiten.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Memory Stores in selbst gehosteten Sandboxes (19.08.)
- **Was:** Managed-Agents-Sessions in einer self-hosted Sandbox können Memory Stores einhängen; die Python-, TypeScript- und Go-SDK-Worker laden jeden angehängten Store an seinen `mount_path` und synchronisieren Änderungen zurück.
- **Einsatz:** Memory Store an die Session anhängen (`mount_path`)
- **Mehrwert:** Persistentes Agenten-Gedächtnis auch dann, wenn die Ausführung auf eigener Infrastruktur läuft.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Neuer Session-Viewer in der Console (19.08.)
- **Was:** Der Session-Viewer wurde überarbeitet: Timeline-Minimap, nach Modell-Request gruppiertes Transkript und ein Inspector-Panel für Session-Details, Kosten, Rohereignisse, Statistiken pro Tool, eingehängte Ressourcen und Aktivität pro Thread.
- **Einsatz:** Claude Console → Session-Ansicht
- **Mehrwert:** Fehlersuche und Kostenanalyse langer Agenten-Läufe ohne Export in eigene Tools.
- **Version:** — (Claude Platform, 19.08.2026)

#### Platform: Aus Workbench wird Playground (18.08.)
- **Was:** Die Workbench der Claude Console heißt jetzt **Playground**, unterstützt sämtliche Messages-API-Parameter und bringt Vorlagen für API-Funktionen wie Code Execution und Web Search mit; zu jedem Lauf werden der vollständige SDK-Request und die API-Antwort angezeigt.
- **Einsatz:** [platform.claude.com/playground](https://platform.claude.com/playground)
- **Mehrwert:** Ausprobieren und Übernehmen in einem Schritt — der gezeigte SDK-Request lässt sich direkt in den eigenen Code kopieren.
- **Version:** — (Claude Platform, 18.08.2026)

---

### Woche 34 (21. August 2026) — v2.1.239

#### Kostenschätzung berücksichtigt den US-only-Inference-Aufschlag
- **Was:** In Workspaces mit Daten-Residenz auf US-only-Inference rechnen `/cost`, die Statusline und `--max-budget-usd` jetzt den Aufschlag von 1,1× mit ein.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Das angezeigte Budget entspricht wieder dem, was am Monatsende tatsächlich abgerechnet wird — vorher lag die Schätzung in solchen Workspaces systematisch 10 % zu niedrig.
- **Version:** v2.1.239

#### Fullscreen-Angebot jetzt auch auf Bedrock, Vertex und Foundry
- **Was:** Das einmalige Angebot, auf den Fullscreen-Renderer zu wechseln, erscheint jetzt auch auf Bedrock, Vertex, Foundry und anderen bisher ausgenommenen Setups; Neuinstallationen starten dort direkt im Fullscreen. Zugleich behoben: Der Hinweis tauchte bei jedem Start erneut auf, wenn er gar nicht beantwortet werden konnte — nach drei Anläufen gibt er jetzt Ruhe.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Wer über einen Cloud-Anbieter arbeitet, bekommt dieselbe Oberfläche wie alle anderen — und niemand muss denselben Dialog zum vierten Mal wegklicken.
- **Version:** v2.1.239

#### `/claude-api upgrade` — Python-Projekte von `anthropic` 0.x auf 1.x
- **Was:** Neuer Unterbefehl des `claude-api`-Skills, der ein Python-Projekt von der SDK-Reihe 0.x auf 1.x migriert. Die Python-Referenz des Skills ist auf 1.x aktualisiert — Timeouts werden dort etwa über `anthropic.Timeout` gesetzt, nicht mehr über `httpx.Timeout`.
- **Einsatz:** `/claude-api upgrade`
- **Mehrwert:** Der Sprung auf das neue Python-SDK (siehe Platform-Eintrag vom 20.08.) lässt sich anstoßen, statt jede Breaking Change von Hand aus dem Migrationsleitfaden abzuarbeiten.
- **Version:** v2.1.239

#### Cloud-Sessions: von claude.ai synchronisierte Plugins als `name@synced`
- **Was:** Plugins, die aus claude.ai in eine Cloud-Session synchronisiert werden, erscheinen jetzt als `name@synced`, lassen sich mit `claude plugin enable/disable <name>@synced` schalten und überschreiben nie ein gleichnamiges Plugin, das man selbst installiert hat.
- **Einsatz:** `claude plugin disable <name>@synced`
- **Mehrwert:** Es ist auf einen Blick erkennbar, welches Plugin von wo kommt — und die eigene lokale Version gewinnt nicht mehr zufällig oder verliert zufällig gegen die synchronisierte.
- **Version:** v2.1.239

#### Alpine/musl-Builds: Bild-Paste, Zwischenablage und Audioaufnahme funktionieren
- **Was:** Die nativen Zusatzmodule für Bild-Einfügen, Zwischenablage und Audioaufnahme wurden auf Alpine/musl-Systemen von der Laufzeit abgelehnt, weil glibc-Binaries ausgeliefert wurden; jetzt kommen musl-Builds mit.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** In schlanken Alpine-Containern — der Standard für viele CI- und Dev-Images — funktionieren Screenshot-Paste und Diktat endlich so wie auf dem Laptop.
- **Version:** v2.1.239

#### Nutzungslimit-Meldung nennt auch Session- und Wochen-Reset
- **Was:** Wenn das monatliche Ausgabenlimit bereits ausgeschöpft ist, verrät die Meldung jetzt zusätzlich, wann das Session- bzw. Wochenlimit zurückgesetzt wird.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Man weiß sofort, ob man in einer Stunde oder erst nächsten Monat weiterarbeiten kann, ohne die Console aufzurufen.
- **Version:** v2.1.239

#### Bedrock-Streaming hinter Proxys: doppelte Abrechnung behoben
- **Was:** Proxys, die den `Content-Type`-Header der Antwort entfernen, brachten Claude Code dazu, jeden Turn still ein zweites Mal ohne Streaming zu fahren — die abgerechneten API-Aufrufe verdoppelten sich unbemerkt. Behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der teuerste Fehler dieses Releases: Wer Bedrock hinter einem Unternehmens-Proxy nutzt, hat schlicht doppelt bezahlt, ohne dass es irgendwo sichtbar war.
- **Version:** v2.1.239

#### Bedrock mit SSO hinter HTTPS-Proxy startet wieder
- **Was:** Claude Code blieb beim Start hängen, wenn Bedrock mit einem SSO-Profil und `awsAuthRefresh` hinter einem HTTPS-Proxy lief; die Vorabprüfung der Zugangsdaten beachtet jetzt `HTTPS_PROXY`.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kein rätselhafter Hänger mehr beim Start in genau der Konfiguration, die in vielen Unternehmen Standard ist.
- **Version:** v2.1.239

#### Gelöschte Verzeichnisse werfen die Session nicht mehr aus der Bahn
- **Was:** Gleich drei Stellen repariert: Der Start aus einem nicht mehr existierenden Verzeichnis warf einen rohen Crash-Dump statt einer verständlichen Meldung; Hooks scheiterten mit „posix_spawn ENOENT", nachdem das Arbeitsverzeichnis der Session gelöscht worden war (sie laufen jetzt aus dem Projekt- oder Home-Verzeichnis); und `/resume` im Alle-Projekte-Modus forderte auf, in ein gelöschtes Verzeichnis zu wechseln — etwa ein entferntes Worktree. Solche Sessions werden jetzt im aktuellen Verzeichnis fortgesetzt.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Wer mit Worktrees oder temporären Verzeichnissen arbeitet, verliert nach dem Aufräumen weder die Session noch die Hooks.
- **Version:** v2.1.239

#### JetBrains-Terminals: Edit und Write ohne 5-Sekunden-Pause
- **Was:** Bei verbundenem Claude-Code-Plugin hingen Edit- und Write-Aufrufe im JetBrains-Terminal rund fünf Sekunden; behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Bei einem Refactoring über zwanzig Dateien waren das anderthalb Minuten reines Warten pro Durchlauf.
- **Version:** v2.1.239

#### Esc bei wartendem Prompt beendet den Turn nicht mehr vorzeitig
- **Was:** Ein Wettlauf sorgte dafür, dass Esc mit einem eingereihten Prompt den nächsten Turn vorzeitig abschließen konnte: Die Session sah untätig aus, während Claude noch arbeitete — und ein späteres erneutes Absenden wiederholte bereits ausgeführte Aktionen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kein doppelt ausgeführter Commit oder Deploy mehr, nur weil man einmal Esc gedrückt hat.
- **Version:** v2.1.239

#### WebFetch hält abgelaufene Seiten nicht mehr die ganze Session
- **Was:** WebFetch behielt geholte Seiteninhalte im Speicher, bis die Session endete, statt sie nach den vorgesehenen 15 Minuten freizugeben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Lange Recherche-Sessions mit vielen Seitenabrufen wachsen nicht mehr unbegrenzt im Speicher.
- **Version:** v2.1.239

#### Cloud-Sessions bleiben im Plan-Modus
- **Was:** Nach einem Neustart des Workers im Leerlauf kamen Cloud-Sessions (Claude Code im Web sowie Desktop- und Mobile-App) nicht mehr im Plan-Modus zurück; behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Eine bewusst im Plan-Modus geparkte Session fängt nach der Pause nicht plötzlich an, Dateien zu schreiben.
- **Version:** v2.1.239

#### MCP: Formulare im Fullscreen und Reconnect nach 5xx
- **Was:** Elicitation-Formulare, die höher als das Terminal waren, wurden im Fullscreen abgeschnitten — sie passen sich jetzt dem Fenster an, verdeckte Felder sind scrollbar, Accept und Decline immer sichtbar. Außerdem blieben entfernte MCP-Server nach einem vorübergehenden 5xx bei einem Reconnect mitten in der Session dauerhaft im Fehlerzustand (in Cloud-Sessions und über `setMcpServers()` im SDK).
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein kurzer Serverschluckauf legt die MCP-Anbindung nicht mehr für den Rest der Session lahm, und Formulare bleiben bedienbar.
- **Version:** v2.1.239

#### `/resume`: Titel, ähnliche Pfade und Sortierung repariert
- **Was:** Drei Fehler auf einmal: Eigene Session-Titel verschwanden aus `/resume`, sobald nach dem Umbenennen mehr als rund 64 KB Konversation dazukamen; `claude -c` und der Resume-Picker griffen Sessions aus einem anderen Verzeichnis auf, dessen Pfad sich nur in Zeichen wie `_`, `-` oder `.` unterschied; und `/resume` sowie die Agent-Ansicht markierten eine Session als kürzlich geändert (und sortierten sie nach oben), wenn nur ihre Datei angefasst oder sie schlicht wieder geöffnet worden war.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Session-Liste stimmt wieder — richtige Namen, richtige Reihenfolge, und man landet nicht versehentlich im falschen Projekt.
- **Version:** v2.1.239

#### `.worktreeinclude`: `**/`-Muster greifen auch in gitignorierten Verzeichnissen
- **Was:** Muster, die mit `**/` beginnen, trafen still auf nichts, wenn das Ziel in einem gitignorierten Verzeichnis lag.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Gerade `.env`-Dateien und lokale Konfigurationen liegen typischerweise gitignoriert — sie landen jetzt wie beabsichtigt im Worktree.
- **Version:** v2.1.239

#### UTF-8-BOM legt Agents, Skills und Commands nicht mehr still
- **Was:** Agents, Skills und Commands, deren `.md`-Datei mit einem UTF-8-BOM beginnt, wurden kommentarlos ignoriert.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein von einem Windows-Editor gespeichertes Skill funktioniert jetzt — vorher fehlte es einfach, ohne jede Fehlermeldung.
- **Version:** v2.1.239

#### `/insights` ohne literale `<message>`-Tags
- **Was:** `/insights` gab auf manchen Modellen die `<message>`-Tags wörtlich mit aus.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Auswertung ist wieder lesbar statt mit Markup durchsetzt.
- **Version:** v2.1.239

#### Marketplace: `metadata.pluginRoot` wirkt wieder
- **Was:** `metadata.pluginRoot` im Marketplace blieb wirkungslos; bloße Plugin-Quellnamen werden jetzt darunter aufgelöst, wie es die Dokumentation beschreibt.
- **Einsatz:** `metadata.pluginRoot` im Marketplace-Manifest
- **Mehrwert:** Marketplaces mit einem gemeinsamen Unterverzeichnis müssen nicht mehr in jedem Eintrag den vollen Pfad wiederholen.
- **Version:** v2.1.239

#### Browser-Terminals: Mausbewegung schreibt keinen Müll mehr in den Prompt
- **Was:** In browserbasierten Terminals landete bei Mausbewegung Text wie `"35;150;7M"` im Prompt, wenn ein Maus-Report auf mehrere Schreibvorgänge verteilt ankam.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Claude Code im Browser-Terminal ist ohne Zeichensalat im Eingabefeld benutzbar.
- **Version:** v2.1.239

#### Eigene Theme-Farben für Badge und aufgeklappte Tool-Ergebnisse
- **Was:** Eigene Theme-Overrides für die Farben des Effort-/Ultracode-Badges wurden ignoriert. Zudem stellte das Theme `dark-ansi` aufgeklappte Tool-Ergebnisse im Fullscreen in Textfarbe gleich Hintergrundfarbe dar — also unsichtbar.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Eigene Themes sehen wieder so aus wie definiert, und bei `dark-ansi` muss man Tool-Ausgaben nicht mehr blind markieren, um sie zu lesen.
- **Version:** v2.1.239

#### OpenTelemetry: verzögerte Tool-Aufrufe bleiben im selben Trace
- **Was:** Tool-Ausführungen, die ein `PreToolUse`-Hook verzögert hat, starteten anschließend einen neuen Trace statt im Trace des ursprünglichen Turns fortzusetzen — die Traces zersplitterten.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Wer Claude Code über OpenTelemetry beobachtet, sieht einen Turn wieder als einen zusammenhängenden Vorgang.
- **Version:** v2.1.239

#### Terminal-Eingabe: Vim-Escape, Textauswahl, Paste-Platzhalter, maskierte Felder
- **Was:** Ein Bündel von Eingabe-Korrekturen. Im Vim-Modus der Agent-Ansicht schaltet Escape jetzt in den NORMAL-Modus und behält den Text, statt den Prompt zu leeren. Die Tastenbelegung `selection:copy` verwarf still eine Auswahl, die mit Shift+Pfeiltasten erweitert worden war. Ctrl+W, Ctrl+U, Ctrl+K, Option+Backspace, Option+D sowie vim `df`/`dt` hinterließen einen kaputten `[Pasted text #N]`-Platzhalter, wenn der Cursor darin stand. Maskierte Eingaben wie das Feld für den Login-Code ließen ihren Text mit Ctrl+Y anderswo wieder einfügen oder wanderten beim Leeren per doppeltem Esc in die Prompt-History. Und Ctrl+Backspace löschte in Suchfeldern nur ein Zeichen statt eines Wortes.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der letzte Punkt ist ein Sicherheitsfix: Ein eingegebener Login-Code konnte in der Prompt-History landen oder sich woanders wieder einfügen lassen.
- **Version:** v2.1.239

#### `/voice`-Starthinweis verschwindet nach dem Aktivieren
- **Was:** Der Starthinweis zu `/voice` erschien weiter, obwohl die Diktierfunktion über die Einstellung `voice.enabled` bereits aktiviert war.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein Tipp weniger bei jedem Start, den man längst befolgt hat.
- **Version:** v2.1.239

#### Shell-Modus: Tab-Vervollständigung behält das `./`
- **Was:** Im Shell-Modus (`!`) schnitt die Tab-Vervollständigung das `./` von einem `./script`-Pfad ab — übrig blieb ein Kommando, das die Shell nicht ausführen konnte.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** `!./deploy.sh` per Tab vervollständigt läuft wieder direkt, ohne dass man das `./` von Hand nachträgt.
- **Version:** v2.1.239

#### Fullscreen: Fensterfokus, Slash-Panels und `/workflows`-Dialog
- **Was:** Drei Fullscreen-Korrekturen: Ein Klick ins Terminalfenster, der nur den Fokus zurückholen sollte, beantwortete eine Berechtigungsabfrage oder drückte einen Knopf. Panels von Slash-Commands wie `/config` oder `/model` verdeckten die neuesten Nachrichten — die Konversation bleibt jetzt über dem Panel angeheftet. Und der Detaildialog von `/workflows` lief über den Bildschirmrand hinaus und verlor seine Kopfzeile, wenn man ihn öffnete, während Claude noch antwortete.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Vor allem der erste Punkt zählt: Ein Klick zum Fensterwechsel durfte nie eine Berechtigung erteilen.
- **Version:** v2.1.239

#### Linux-Sandbox: git in Repos mit `extensions.worktreeConfig`
- **Was:** Die Linux-Sandbox machte eine nicht existierende `.git/config.worktree` unlesbar, was in Repos mit gesetztem `extensions.worktreeConfig` jedes gesandboxte git-Kommando scheitern ließ.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Worktree-Setups funktionieren wieder im Sandbox-Modus, statt bei jedem git-Aufruf zu blockieren.
- **Version:** v2.1.239

#### `claudeMdExcludes` erfasst symlinkte `.claude/rules`
- **Was:** `claudeMdExcludes` schloss eine per Symlink eingebundene `.claude/rules`-Datei nicht aus, wenn das Muster das Rules-Verzeichnis oder den Symlink selbst benannte statt dessen Ziel.
- **Einsatz:** `claudeMdExcludes` in den Settings
- **Mehrwert:** Geteilte Regel-Dateien, die per Symlink in mehreren Projekten hängen, lassen sich dort ausschließen, wo sie nicht gelten sollen.
- **Version:** v2.1.239

#### Remote Control: Titel-Synchronisierung gebändigt, klarere Meldung
- **Was:** Teilten sich zwei Claude-Code-Prozesse den Zustand eines Hintergrund-Jobs, synchronisierten sie den Session-Titel in Dauerschleife zu Remote Control — eine Regression aus 2.1.232. Titel-Updates werden jetzt entdoppelt und in der Rate begrenzt. Außerdem melden Claude Code und `claude doctor` jetzt verständlich, wenn Remote Control für das Konto gar nicht freigeschaltet ist.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kein unnötiger Dauerverkehr mehr — und wer Remote Control vergeblich sucht, erfährt endlich den Grund.
- **Version:** v2.1.239

#### Von der Organisations-Policy abgelehnte Anfragen werden nicht vorschnell erneut gesendet
- **Was:** Eine Anfrage, die eine Richtlinienprüfung der Organisation abgelehnt hatte, wurde erneut abgeschickt, bevor die Ablehnung überhaupt angezeigt wurde.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Ablehnung ist sichtbar, statt hinter einem stillen Wiederholungsversuch zu verschwinden — wichtig in Umgebungen mit Inference-Hooks.
- **Version:** v2.1.239

#### Nach der Kompaktierung: Skill-Argumente laufen nicht als neue Anfrage
- **Was:** Der Hinweis, der nach einer Kompaktierung eingeblendet wird, ist so überarbeitet, dass die ursprünglichen Argumente eines Skills nicht als neue Anfrage erneut ausgeführt werden.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Lange Sessions, in denen ein Skill mit Argumenten gestartet wurde, wiederholen nach der Kompaktierung nicht plötzlich den ganzen Auftrag.
- **Version:** v2.1.239

#### Lange Dateipfade bleiben auf einer Zeile
- **Was:** Lange Pfade in Tool-Zeilen werden jetzt in der Mitte gekürzt, statt umzubrechen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Anfang und Dateiname bleiben sichtbar — genau die beiden Teile, an denen man einen Pfad erkennt.
- **Version:** v2.1.239

#### Remote-Sessions: Keep-Alives während langer Start-Hooks
- **Was:** Remote-Sessions senden jetzt weiter Keep-Alives, während ein langer `SessionStart`- oder `Setup`-Hook läuft, damit der Container nicht mitten im Hook als untätig eingesammelt wird.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Aufwendige Setup-Hooks — Abhängigkeiten installieren, Datenbank hochfahren — bringen die Session nicht mehr um, bevor sie begonnen hat.
- **Version:** v2.1.239

#### `/goal`: seltenere Check-ins, Ziel überlebt den Resume-Picker
- **Was:** Wiederholte Check-ins zu lange laufender Hintergrundarbeit werden jetzt seltener — erst nach 30 Minuten, dann nach einer Stunde, danach alle zwei Stunden — statt starr alle 30 Minuten. Außerdem stellt eine über `claude --resume` fortgesetzte Session ihr aktives Ziel wieder her.
- **Einsatz:** `/goal`
- **Mehrwert:** Ein über Stunden laufendes Ziel meldet sich nicht mehr im Minutentakt und geht beim Fortsetzen nicht verloren.
- **Version:** v2.1.239

#### `ListAgents` und `SendMessage`: eigener Name, Teammates, `/`-Titel
- **Was:** `ListAgents` nennt einer Session jetzt ihren eigenen Namen — also den, unter dem Peers sie ansprechen —, und `SendMessage` an den eigenen Namen sagt das auch, statt „kein Agent namens …" zu melden. `ListAgents` und `/list-agents` listen zudem die lebenden Teammates auf (bisher erschienen nur Subagenten und andere Sessions, ein erreichbarer Teammate sah also abwesend aus). Und Sessions, deren Titel mit `/` beginnt, waren für `SendMessage` gar nicht adressierbar und erschienen als „(untitled)".
- **Einsatz:** `ListAgents`, `/list-agents`, `SendMessage`
- **Mehrwert:** Cross-Session-Messaging ist erst dann brauchbar, wenn man zuverlässig weiß, wie man selbst und die Gegenstelle heißen — genau das war bisher lückenhaft.
- **Version:** v2.1.239

#### `keybindingFlavor: "readline"` deckt jetzt auch Wort-Tasten ab
- **Was:** Die in v2.1.238 eingeführte Einstellung greift jetzt auch bei den Wort-Tasten wie in der Bash: Alt+F und Ctrl/Option+→ halten am Wortende, Alt+D löscht bis dorthin (Ctrl+Y fügt es wieder ein), und Satzzeichen trennen Wörter.
- **Einsatz:** `keybindingFlavor: "readline"` in den Settings
- **Mehrwert:** Die Bash-Belegung ist jetzt vollständig statt nur bei Ctrl+W — Muskelgedächtnis aus der Shell trägt durch.
- **Version:** v2.1.239

#### Retry-Watchdog bricht bei erschöpftem Guthaben sofort ab
- **Was:** Der dauerhafte Wiederholungsmodus (`CLAUDE_CODE_RETRY_WATCHDOG`) scheitert jetzt sofort bei Ausgabenlimit der Organisation und aufgebrauchtem Guthaben, statt unbegrenzt auf ein Zurücksetzen zu warten.
- **Einsatz:** `CLAUDE_CODE_RETRY_WATCHDOG`
- **Mehrwert:** Ein über Nacht laufender Auftrag hängt nicht bis zum Morgen in einer Warteschleife, die nie aufgehen kann.
- **Version:** v2.1.239

#### Claude in Chrome: `/clear` schließt die Tab-Gruppe
- **Was:** `/clear` schließt jetzt die Chrome-Tab-Gruppe der Session; leere Gruppen werden zusätzlich bei `/resume` und beim Beenden von Claude Code geschlossen.
- **Einsatz:** `/clear`
- **Mehrwert:** Nach einer Recherche-Session bleiben keine verwaisten Tab-Gruppen im Browser zurück.
- **Version:** v2.1.239

#### Remote-Sessions: Bilder vom Handy kommen mit Dateipfad
- **Was:** Vom Mobilgerät hochgeladene Bilder tragen jetzt ihren gespeicherten Dateipfad mit, sodass Claude sie in erzeugte Dateien kopieren kann.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein unterwegs abfotografierter Screenshot lässt sich direkt in ein Repository übernehmen, statt nur beschrieben zu werden.
- **Version:** v2.1.239

#### Claude Code on the web: Bash-Anfragen an anthropic.com über den Session-Proxy
- **Was:** Anfragen von Bash und anderen Tools an Nicht-API-Hosts von anthropic.com — etwa `www` oder `docs` — laufen jetzt über den Netzwerk-Proxy der Session, sodass die erlaubten Domains der eigenen Umgebung greifen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Netzwerkregeln der Umgebung gelten lückenlos, statt für einzelne Anthropic-Hosts umgangen zu werden.
- **Version:** v2.1.239

#### Windows: Cross-Session-Messaging verfügbar
- **Was:** Claude-Code-Sessions können sich unter Windows jetzt gegenseitig Nachrichten schicken (`SendMessage`) und sich über `ListAgents` finden — wie bereits unter macOS und Linux.
- **Einsatz:** `SendMessage`, `ListAgents`
- **Mehrwert:** Die letzte große Plattformlücke beim Cross-Session-Messaging ist geschlossen; gemischte Windows-/Linux-Setups arbeiten jetzt zusammen.
- **Version:** v2.1.239

#### VS Code: „View usage" sitzt wieder in der Zeile
- **Was:** Der Link „View usage" im Banner zum Nutzungslimit steht jetzt in einer Zeile mit dem Warntext, statt mitten im Banner zu schweben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kosmetik, aber das Banner ist wieder auf einen Blick lesbar.
- **Version:** v2.1.239


### Woche 34 (20. August 2026) — v2.1.238

#### `keybindingFlavor: "readline"` — Ctrl+W wie in der Bash
- **Was:** Neue Einstellung `keybindingFlavor`. Auf `"readline"` gesetzt, löscht Ctrl+W im Prompt bis zum vorigen Leerzeichen — genau wie in der Bash. Der Standard `"classic"` bleibt unverändert.
- **Einsatz:** `keybindingFlavor: "readline"` in den Settings
- **Mehrwert:** Wer den ganzen Tag in der Shell arbeitet, muss im Claude-Prompt nicht umdenken — Ctrl+W löscht dort das, was man erwartet.
- **Version:** v2.1.238

#### Plugin-Marketplaces: `headersHelper` für kurzlebige Tokens
- **Was:** Ein URL-Marketplace oder ein einzelner Katalog-Eintrag kann einen `headersHelper` hinterlegen — ein Kommando, das HTTP-Header (z. B. ein kurzlebiges Token) für Katalog- und Archiv-Abrufe derselben Origin erzeugt. Der Helper eines Katalog-Eintrags läuft nur beim Installieren oder Aktualisieren des Plugins, nachdem sein Kommando angezeigt wurde; `claude plugin install/update` fragen dann `[y/N]` (oder man übergibt `-y`).
- **Einsatz:** `headersHelper` im Marketplace- bzw. Katalog-Eintrag
- **Mehrwert:** Firmen-interne Plugin-Marketplaces hinter einer Authentifizierung lassen sich anbinden, ohne ein statisches Token in die Konfiguration zu schreiben.
- **Version:** v2.1.238

#### Self-hosted Runner: geordnetes Herunterfahren mit `--defer-shutdown-max-min`
- **Was:** `claude self-hosted-runner --defer-shutdown-max-min <Minuten>` sorgt dafür, dass der Runner bei SIGTERM angehängte Sessions weiterbedient, den Rest nach der angegebenen Frist parkt und dann beendet.
- **Einsatz:** `claude self-hosted-runner --defer-shutdown-max-min 15`
- **Mehrwert:** Deployments und Node-Rotationen reißen laufende Sessions nicht mehr mitten in der Arbeit ab.
- **Version:** v2.1.238

#### Self-hosted Runner: `Proxy-Authorization` pro Verbindung
- **Was:** Neu sind `claude self-hosted-runner --proxy-authorization-command` und `--proxy-authorization-file` für Egress-Proxys, die bei jeder Verbindung einen frisch ausgestellten `Proxy-Authorization`-Header verlangen.
- **Einsatz:** `--proxy-authorization-command '<Kommando>'` bzw. `--proxy-authorization-file <Pfad>`
- **Mehrwert:** Runner laufen auch hinter Unternehmens-Proxys mit rotierenden Zugangsdaten, ohne dass jemand von Hand Tokens nachlegt.
- **Version:** v2.1.238

#### Speicherwachstum in langen Sessions gestoppt
- **Was:** Ergebnisse von Subagent-Tools wurden in langen interaktiven Sessions dauerhaft gehalten; sie werden jetzt freigegeben, sobald sie aus dem angezeigten Fenster herauslaufen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Stundenlange Sessions mit vielen Subagenten fressen nicht mehr unbegrenzt RAM — kein Neustart mehr „weil der Prozess so groß geworden ist".
- **Version:** v2.1.238

#### Output-Styles bleiben über die Session hinweg gültig
- **Was:** Eigene, Projekt- und Plugin-Output-Styles fielen mitten in der Session in die Standard-Stimme zurück; das ist behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein festgelegter Antwortstil hält durch — man muss ihn nicht nach ein paar Turns erneut setzen.
- **Version:** v2.1.238

#### Prompt-Vorschläge bleiben nahe am Nutzungslimit an
- **Was:** `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=true` wurde ignoriert, wenn das Konto nahe an sein Nutzungslimit kam, ohne es überschritten zu haben; die Vorschläge bleiben jetzt an.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Oberfläche verhält sich nicht mehr unvorhersehbar, je nachdem wie voll das Limit gerade ist.
- **Version:** v2.1.238

#### Worktree-Isolation: verständlichere Bash-Ablehnungen
- **Was:** Eine wegen Worktree-Isolation abgelehnte Bash-Ausführung forderte zum Entfernen einer Umleitung auf, die das Kommando gar nicht enthielt.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Fehlermeldung nennt jetzt den tatsächlichen Grund, statt auf eine falsche Fährte zu schicken.
- **Version:** v2.1.238

#### Self-hosted Runner überleben einen langsamen Poll
- **Was:** Ein einzelner langsamer oder verlorener Poll-Request führte gelegentlich dazu, dass der Server einen gesunden Runner entfernte und dessen Session einem anderen Runner übergab.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Keine grundlosen Session-Wanderungen mehr bei kurzen Netzaussetzern in der eigenen Runner-Flotte.
- **Version:** v2.1.238

#### MCP-Dialoge und Berechtigungs-Prompts bei extremen Längen repariert
- **Was:** MCP-Elicitation-Dialoge zeigten bei URLs über 4.096 Zeichen gar nichts an, und Berechtigungs-Prompts verloren die Option „nicht mehr fragen", wenn der Projektpfad nicht in die Terminalbreite passte.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Lange OAuth-URLs und tief verschachtelte Projektpfade blockieren die Bedienung nicht mehr.
- **Version:** v2.1.238

#### Aufgeräumte `/tmp`-Reste nach abgebrochenen Bash-Kommandos
- **Was:** Wurde ein Bash-Kommando abgeschossen, lief es in einen Timeout oder wurde es unterbrochen, blieben `/tmp/claude-*-cwd`-Dateien liegen; sie werden jetzt entfernt.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kein langsam volllaufendes `/tmp` mehr auf Maschinen mit vielen abgebrochenen Läufen.
- **Version:** v2.1.238

#### Terminal-Eingabe: Backspace, Umbrüche, bracketed-paste
- **Was:** Drei Eingabe- und Darstellungsfehler behoben: gehaltene Backspace-Taste wurde auf Terminals ignoriert, die Ctrl+H senden, wenn Tastendrücke in großen Schüben ankommen (langsame SSH-/mosh-Verbindungen); Zeilen mit breiten Mehr-Codepoint-Zeichen (etwa Emoji) oder Tabs wurden in Diffs der Berechtigungs-Prompts abgeschnitten; und das Beenden einer mit Ctrl+Z angehaltenen Session ließ das Terminal mitunter im bracketed-paste-Modus mit unsichtbarem Cursor zurück.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Vor allem über SSH wird die Eingabe wieder verlässlich — und ein hängengebliebenes Terminal muss nicht mit `reset` gerettet werden.
- **Version:** v2.1.238

#### stdio-MCP-Server starten nicht mehr unnötig
- **Was:** stdio-MCP-Server erhielten einen `server/discover`-Request noch vor `initialize`, was faul startende Server zwang, ihr Backend bei jedem Session-Start hochzufahren.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Schnellere Session-Starts und keine überflüssig laufenden Backend-Prozesse, wenn der Server in dieser Session gar nicht gebraucht wird.
- **Version:** v2.1.238

#### `claude mcp list`/`get` fragen deaktivierte Server nicht mehr an
- **Was:** Beide Kommandos verbanden sich zur Statusprüfung auch mit deaktivierten Servern; jetzt werden diese schlicht als `⊘ Disabled` angezeigt. Zusätzlich wird die Ablehnung einer Verbindung durch einen Proxy als solche gemeldet, statt als allgemeiner Netzwerkfehler.
- **Einsatz:** `claude mcp list`, `claude mcp get <name>`
- **Mehrwert:** Die Übersicht kommt sofort statt nach Timeouts, und Proxy-Probleme sind als solche erkennbar.
- **Version:** v2.1.238

#### MCP `headersHelper`: Vertrauensprüfung und saubere Umgebung
- **Was:** Ein `headersHelper` aus einer Projekt-`.mcp.json` sowie inline definierte MCP-Server in Projekt- oder `--add-dir`-Agent-Dateien setzen jetzt voraus, dass der Vertrauensdialog für diesen Ordner bestätigt wurde — auch unter `claude -p`. Außerdem läuft ein Helper aus Projekt, Plugin oder Agent-Datei ohne geerbte Credential-Umgebungsvariablen; Helper mit User-, Managed- oder claude.ai-Scope laufen aus dem Claude-Konfigurationsverzeichnis.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein geklontes fremdes Repository kann nicht mehr allein durch Öffnen ein Kommando mit den eigenen Zugangsdaten ausführen.
- **Version:** v2.1.238

#### Remote Control: großes Stabilitätspaket
- **Was:** Zahlreiche Fehler behoben: der Stop-Knopf einzelner Tasks im Remote-Control-Panel blieb bei CLI-gehosteten Sessions wirkungslos; Sessions beendeten sich, wenn ein Client eine Nachricht ohne gültige Rolle schickte; von `claude remote-control` gestartete Sessions erbten Session-Umgebungsvariablen der aufrufenden Shell; eine abgestürzte Session blieb bis zum Neustart unerreichbar und kann jetzt bei der nächsten Nachricht wiederverwendet werden; aus Web oder Desktop mitten im Turn gesendete Nachrichten verschwanden danach aus dem Transkript; auf Telefon oder Web getroffene Modellwahlen kamen im Terminal nicht an; ein kurzer Netzaussetzer beim Erneuern der Anmeldung führte zu „login expired" statt zu einem Retry; und ein Sign-out wurde als fehlgeschlagene Verbindung gemeldet statt als sauberes Session-Ende. Zusätzlich werden kurze HTTP-403-Ablehnungen durch Netzwerk-Edge, VPN oder Proxy bis zu drei Minuten toleriert, wobei bei anhaltender Blockade die ablehnende Stelle benannt wird.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Steuerung vom Handy oder aus dem Browser wird verlässlich — Nachrichten und Modellwahlen gehen nicht mehr verloren, und kurze Netzprobleme werfen einen nicht aus der Sitzung.
- **Version:** v2.1.238

#### `ListAgents`/`SendMessage` finden Remote-Control-Peers überall
- **Was:** In Sessions, die von `claude remote-control` (Server-Modus) oder von Desktop-/IDE-Hosts betrieben werden, meldeten beide Werkzeuge „Remote Control is not connected"; sie listen und erreichen die Peers jetzt. Außerdem zeigen sie nicht länger den vorgewärmten Leerlauf-Worker der Agent-Ansicht an — der erscheint erst, wenn ihn eine Aufgabe beansprucht.
- **Einsatz:** `ListAgents`, `SendMessage`
- **Mehrwert:** Session-übergreifende Koordination funktioniert unabhängig davon, wie die Session gestartet wurde, und die Liste zeigt nur echte Gegenstellen.
- **Version:** v2.1.238

#### Cross-Session-Messaging meldet Zustellprobleme ehrlich
- **Was:** Eine Nachricht an eine Session auf derselben Maschine, die eingehende Nachrichten ablehnt (z. B. `crossSessionInbound: "refuse"`), wird dem Absender jetzt als „refused" gemeldet statt als stiller Erfolg. Ebenso erfährt die sendende Session, wenn der Posteingang der Gegenstelle Nachrichten wegen Rate-Limit oder voller Warteschlange verwirft.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Man wartet nicht mehr auf eine Antwort zu einer Nachricht, die nie angekommen ist.
- **Version:** v2.1.238

#### Schnellerer Start und ruhigerer Update-Check
- **Was:** Ein nacktes `claude` startet auf macOS spürbar früher, und die automatische Update-Prüfung läuft jetzt rund 10 Sekunden nach dem Start, statt mit dem Startvorgang um CPU zu konkurrieren.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Der Prompt ist schneller da, gerade auf ausgelasteten Rechnern.
- **Version:** v2.1.238

#### Bash-Berechtigungsprüfung versteht zsh-Bedingungen
- **Was:** Die Berechtigungsprüfung des Bash-Tools wertet zsh-spezifische Syntax in Shell-Bedingungen jetzt korrekt aus.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Weniger falsche Rückfragen bei zsh-Nutzern, deren Kommandos zuvor als undurchschaubar galten.
- **Version:** v2.1.238

#### Fullscreen: Ctrl+L und Cmd+K zeichnen nur noch neu
- **Was:** Der Doppeldruck-Shortcut für `/clear` wurde entfernt — Ctrl+L und Cmd+K im Fullscreen-Modus zeichnen immer nur neu. Damit lösen auch einzeilige nvim-Terminals keine automatischen `/clear`-Schleifen mehr aus.
- **Einsatz:** Ctrl+L bzw. Cmd+K
- **Mehrwert:** Kein versehentlich gelöschter Gesprächsverlauf mehr durch einen zweiten Tastendruck.
- **Version:** v2.1.238

#### `claude-api`-Skill auf den Stand vom 19.08. gebracht
- **Was:** Das mitgelieferte `claude-api`-Skill kennt jetzt das Managed-Agents-Release vom 19. August: Domain-Einstellungen für Web-Search und Web-Fetch sowie Memory Stores in selbst gehosteten Sandboxes.
- **Einsatz:** Automatisch aktiv (Skill `claude-api`)
- **Mehrwert:** Fragen zur Claude API werden gegen den aktuellen Funktionsumfang beantwortet statt gegen den Stand von vorletzter Woche.
- **Version:** v2.1.238

#### `/model`- und `/effort`-Warnung nur noch bei echtem Cache-Verlust
- **Was:** Die Warnung vor einem Cache-Miss beim Wechsel von Modell oder Effort erschien auch dann, wenn der Prompt-Cache ohnehin schon abgelaufen war.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Die Warnung bedeutet wieder etwas — man zögert einen sinnvollen Modellwechsel nicht mehr grundlos hinaus.
- **Version:** v2.1.238

---

### Woche 34 (20. August 2026) — v2.1.237

#### Output-Style „Concise"
- **Was:** Neuer eingebauter Ausgabestil, bei dem Claude mit dem Ergebnis beginnt und Vorrede sowie Zwischenerzählung weglässt — die Arbeit selbst bleibt unverändert gründlich.
- **Einsatz:** `/config` → „Output style" → *Concise*
- **Mehrwert:** Weniger Text zum Überfliegen bei Routineaufgaben; das Ergebnis steht sofort oben statt hinter einer Erklärung.
- **Version:** v2.1.237

#### Prompt-Caching hinter LLM-Gateways repariert
- **Was:** Sessions, die über ein LLM-Gateway oder eine eigene Base-URL laufen, verloren das Prompt-Caching; das ist behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Deutlich niedrigere Kosten und schnellere Antworten für alle, die Claude Code über einen Proxy oder ein Firmen-Gateway betreiben.
- **Version:** v2.1.237
...
- **Mehrwert:** Team-weite Vorgabe eines Standardmodells, ohne dass Entwickler in die Wahl per `/model` eingesperrt werden.
- **Version:** v2.1.236

#### `notify_when_idle` für `SendMessage`
- **Was:** Eine andere Claude-Code-Session auf derselben Maschine kann gebeten werden, genau eine Nachricht zu schicken, sobald sie das nächste Mal idle wird — opt-in, einmalig, ohne Polling (macOS und Linux).
- **Einsatz:** `SendMessage` mit `notify_when_idle: true`
- **Mehrwert:** Koordination zwischen parallelen Sessions ohne Warteschleifen — man erfährt, wann die Nebensession fertig ist, statt sie abzufragen.
- **Version:** v2.1.236

#### Sandbox: Wildcard-Deny-Regeln haben Vorrang (macOS)
- **Was:** Lese-Verbote mit Wildcard (z. B. `**/.env`) schlagen jetzt auch innerhalb erlaubter Lesebereiche durch, erfassen den Inhalt getroffener Verzeichnisse und lassen sich nicht mehr durch Umbenennen der gesperrten Datei umgehen.
- **Einsatz:** Automatisch aktiv (Sandbox-Konfiguration)
- **Mehrwert:** `.env`-Dateien und Secrets bleiben zuverlässig unlesbar, auch wenn das umgebende Verzeichnis freigegeben ist.
- **Version:** v2.1.236

#### Auto-Modus: Monitor-Regeln und Git-Status abgesichert
- **Was:** Im Auto-Modus werden `Monitor`-Allow-Regeln beiseitegelegt, sodass Monitor-Kommandos wie Bash-Kommandos geprüft werden; zusätzlich lässt sich die Git-Statusprüfung nicht mehr durch `status.showUntrackedFiles=no` täuschen. Auf Bedrock, Vertex AI und Foundry sowie bei abgeschalteter Telemetrie nutzt der Klassifikator jetzt dieselben Defaults wie auf der Claude API.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Auto-Modus verhält sich über alle Betriebsarten hinweg gleich streng — keine stillen Lücken je nach Plattform oder Repo-Konfiguration.
- **Version:** v2.1.236

#### Fullscreen-Renderer fällt sauber zurück
- **Was:** Ein einzelner Fehlstart des Fullscreen-Renderers führte bisher dazu, dass jeder weitere Start scheiterte; jetzt wird auf den klassischen Renderer zurückgefallen, statt zu beenden.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein einmaliger Terminal-Aussetzer legt die Oberfläche nicht dauerhaft lahm.
- **Version:** v2.1.236

#### `/model`-Picker passt sich der Fensterhöhe an
- **Was:** Die Modellauswahl zeigt nur noch so viele Einträge, wie ins Terminal passen, der Rest ist scrollbar; hervorgehoben wird nur noch der Name des neuesten Modells.
- **Einsatz:** `/model`
- **Mehrwert:** In kleinen Terminals bleibt die Auswahl bedienbar, und die Hervorhebung markiert wirklich das Neue.
- **Version:** v2.1.236

#### `/goal` meldet sich bei langen Hintergrundaufgaben
- **Was:** Ein Ziel, das hinter langlaufender Hintergrundarbeit geparkt ist, meldet sich nach 30 Minuten von selbst (danach nach 1 h, 2 h), statt auf die Rückkehr des Nutzers zu warten.
- **Einsatz:** `/goal`; abschaltbar über `CLAUDE_CODE_GOAL_CHECKIN_MINUTES=0`
- **Mehrwert:** Ein Ziel bleibt nicht stundenlang unbemerkt hängen, wenn ein Build oder Test länger braucht als gedacht.
- **Version:** v2.1.236

#### Slash-Command-Tippfehler laufen nicht mehr ins Falsche
- **Was:** Enter auf einem vertippten oder in dieser Session nicht verfügbaren Slash-Command meldet das jetzt, statt den nächstbesten Fuzzy-Treffer auszuführen; Präfixe und Aliase funktionieren weiterhin.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Kein versehentlich gestarteter falscher Befehl mehr wegen eines Buchstabendrehers.
- **Version:** v2.1.236

#### SIGTERM im Print-/SDK-Modus beendet sauber
- **Was:** Ein SIGTERM protokolliert keinen abgebrochenen Turn und keine synthetischen Tool-Ablehnungen mehr; laufende Kommandos werden weiterhin beendet, der Exit-Code bleibt 143.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Abgebrochene CI-Läufe hinterlassen keinen verfälschten Sessionverlauf.
- **Version:** v2.1.236

#### VS Code: Screenreader-Unterstützung im Transkript
- **Was:** Live-Ansagen für Antworten, Berechtigungsabfragen, Fehler und Statuswechsel sowie Überschriften-Navigation pro Turn.
- **Einsatz:** Automatisch aktiv (VS Code-Erweiterung)
- **Mehrwert:** Claude Code wird in VS Code für Screenreader-Nutzer bedienbar.
- **Version:** v2.1.236
...
- **Mehrwert:** Tippfehler im Prompt fallen vor dem Absenden auf, statt in einer missverstandenen Anweisung zu enden.
- **Version:** v2.1.235

#### Prompt-Cache überlebt Language-Server-Wechsel
- **Was:** Trennt oder verbindet sich ein Language-Server mitten in der Session, wird nicht mehr der gesamte Prompt-Cache verworfen.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Keine plötzlichen Kosten- und Latenzsprünge mehr, wenn die IDE-Sprachunterstützung kurz aussetzt.
- **Version:** v2.1.235

#### Shift+Tab im Kommentarfeld der Berechtigungsabfrage
- **Was:** Shift+Tab im Kommentarfeld einer Permission-Abfrage schloss bisher nicht das Feld, sondern genehmigte die Änderung *und* erteilte session-weite Schreibrechte; jetzt schließt es nur das Feld.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein Tastendruck erteilt keine weitreichenden Rechte mehr aus Versehen — sicherheitsrelevanter Fix.
- **Version:** v2.1.235

#### Berechtigungsdialoge zeigen, was sie wirklich abdecken
- **Was:** Anzeigetext und „nicht mehr fragen"-Optionen entsprechen jetzt exakt dem Umfang der Freigabe; ist der Inhalt nicht vollständig darstellbar, wird „nicht mehr fragen" gar nicht erst angeboten.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Man erteilt keine Dauerfreigabe mehr für etwas, das man nur teilweise gesehen hat.
- **Version:** v2.1.235

#### Cloud-Sessions im Hintergrund kosten weniger Ressourcen
- **Was:** Laufende Cloud-Sessions wie `/ultrareview` oder `/autofix-pr` werden nicht mehr bei jedem Update komplett neu eingelesen und gerendert.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Spürbar geringere Speicher- und CPU-Last, während eine Cloud-Review im Hintergrund läuft.
- **Version:** v2.1.235

#### Eingebettetes `grep` gehärtet (native macOS-/Linux-Builds)
- **Was:** Pathologische Suchmuster brechen jetzt schnell ab, statt den Speicher auszureizen; `-m N` in Kombination mit `-A`/`-C` gibt den korrekten Kontext aus.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Eine unglückliche Regex hängt die Session nicht mehr auf, und Kontextzeilen stimmen.
- **Version:** v2.1.235

#### Vim-Modus behält NORMAL-Modus und Cursorposition
- **Was:** Beim Umschalten des ausführlichen Transkripts (Ctrl+O) oder beim Schließen eines Panels bleiben Modus und Cursorposition erhalten.
- **Einsatz:** Automatisch aktiv (Vim-Modus)
- **Mehrwert:** Kein Neuorientieren nach jedem Panel-Wechsel.
- **Version:** v2.1.235

#### Agent-Tool meldet verfügbare Agenten statt zu raten
- **Was:** In Sessions ohne General-Purpose-Agent liefert ein ausgelassener `subagent_type` jetzt einen klaren Fehler mit Liste der verfügbaren Agenten.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Fehlgeschlagene Subagent-Starts sind sofort erklärbar statt rätselhaft.
- **Version:** v2.1.235
...
- **Einsatz:** Standardmäßig an; abschaltbar über `/config` → „Continue automatically at usage limit"
- **Mehrwert:** Lange Aufgaben laufen über die Limit-Grenze hinweg weiter, ohne dass man zum passenden Zeitpunkt am Rechner sitzen muss.
- **Version:** v2.1.234

#### GitLab-Merge-Request-Badge in Footer und Statusline
- **Was:** Repos mit GitLab-Remote und authentifizierter `glab`-CLI zeigen `MR !N` samt Draft-, Pending- und Green-Status.
- **Einsatz:** GitLab-Remote + `glab auth login`
- **Mehrwert:** MR-Status ohne Kontextwechsel in den Browser — was es für GitHub-PRs längst gab, gibt es jetzt für GitLab.
- **Version:** v2.1.234

#### Sicherheit: `\??\`-Härtung auf alle Pre-Approval-Dateizugriffe ausgeweitet
- **Was:** Remote-Dateilesevorgänge, Session-Restore, `CLAUDE.md`-Includes, Workflow-Skripte und Datei-Uploads weisen Windows-NT-Namespace-Pfade (`\??\`) jetzt ebenfalls zurück.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Schließt den in v2.1.233 begonnenen NTLM-Credential-Leak-Vektor an den verbliebenen Stellen, an denen Dateien vor der Freigabe angefasst werden.
- **Version:** v2.1.234

#### `claude-api`-Skill kostet ~25k statt ~200k Tokens
- **Was:** Das eingebaute `claude-api`-Skill lädt seine Referenzdokumente jetzt bedarfsweise nach.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Achtfach geringere Kontextkosten — das Skill kann geladen bleiben, ohne das Kontextfenster zu dominieren.
- **Version:** v2.1.234

#### `CLAUDE_CODE_PROJECT_DIR_NAME`
- **Was:** Optionale Umgebungsvariable für Hosts, die jeder Session ein eigenes Config-Verzeichnis geben: kurzer Name für das Transkript-Verzeichnis pro Projekt.
- **Einsatz:** `export CLAUDE_CODE_PROJECT_DIR_NAME=<kurzname>`
- **Mehrwert:** Vermeidet überlange, unhandliche Verzeichnisnamen in containerisierten oder gehosteten Setups.
- **Version:** v2.1.234

#### Berechtigungsdialoge und `/add-dir` mitten im Turn
- **Was:** `/permissions` lässt sich öffnen, während Claude arbeitet — Regeländerungen gelten für den Rest des laufenden Turns. Gleiches gilt für `/add-dir`; `/autocompact`, `/theme`, `/help`, `/config` und `/advisor` öffnen im Fullscreen-TUI ebenfalls mitten im Turn.
- **Einsatz:** `/permissions` bzw. `/add-dir <pfad>` während der Bearbeitung
- **Mehrwert:** Man kann eine Regel nachschärfen oder ein Verzeichnis nachreichen, ohne den laufenden Turn abzubrechen.
- **Version:** v2.1.234

#### Session-Berechtigungsantworten für Hintergrund-Subagenten
- **Was:** Session-weite Antworten (auch Ablehnungen) auf Berechtigungsabfragen von Hintergrund-Subagenten gingen verloren; das ist behoben.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein „nicht mehr fragen" gilt auch dann, wenn die Frage aus einem Subagenten kam.
- **Version:** v2.1.234

#### MCP-Diagnose gibt keine aufgelösten Secrets mehr aus
- **Was:** Scope-Konflikt-Warnungen zeigen die konfigurierte `${VAR}`-Form, Verbindungsfehler nur noch den Server-Origin.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** MCP-Logs und Fehlermeldungen können gefahrlos geteilt werden.
- **Version:** v2.1.234

#### Remote Control: Effort, Permission-Mode und Datei-Uploads
- **Was:** Effort-Auswahl vom Handy oder von claude.ai/code wirkt jetzt auch auf Terminal- und Desktop/VS-Code-Sessions; der Permission-Mode wird an verbundene Clients gespiegelt; Dateien aus Remote-Control-Sessions werden hochgeladen und öffnen sich auf Handy und Web statt als leere Karte. Meldet sich der Rechner an einem anderen Konto an, stoppt die Session binnen Sekunden mit Begründung statt Stunden später mit HTTP 404.
- **Einsatz:** Automatisch aktiv (Remote Control)
- **Mehrwert:** Steuerung vom Handy verhält sich wie am Terminal — inklusive Effort und Dateiaustausch.
- **Version:** v2.1.234

#### Eigene Prompts werden als Markdown gerendert
- **Was:** Im Transkript erscheinen die eigenen Eingaben mit Syntax-Highlighting, Inline-Code und Listen — genauso wie die Antworten.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Zurückgeblätterte Prompts mit Codeblöcken sind endlich gut lesbar.
- **Version:** v2.1.234

#### Aussagekräftigere Session-Titel und `/goal`-Aufräumen
- **Was:** Automatisch erzeugte Titel lesen sich als kurze Namen („Login button bug") statt als Sätze; ein `/goal` löscht sich mit Hinweis selbst, wenn ein Turn an einem nicht behebbaren Fehler stirbt (z. B. widerrufene Auth, aufgebrauchtes Guthaben, Kontext-Overflow).
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Sessions bleiben in der Liste unterscheidbar, und ein totes Ziel bleibt nicht scharf geschaltet.
- **Version:** v2.1.234

#### Fullscreen-Renderer-Wechsel verliert keine Start-Flags mehr
- **Was:** Das Annehmen der „Try the new fullscreen renderer?"-Abfrage startete die Session ohne Permission-Mode (z. B. `--dangerously-skip-permissions`), Allow/Deny-Regeln, Modell- oder Effort-Flags neu. `/tui` verwirft ebenfalls keine `--allowed-tools`/`--disallowed-tools`-Regeln mehr, sondern lehnt den Wechsel mit Begründung ab, wenn ein Neustart die Einschränkungen nicht übernehmen kann.
- **Einsatz:** Automatisch aktiv
- **Mehrwert:** Ein Renderer-Wechsel kann eine Session nicht mehr still entschärfen — sicherheitsrelevant bei restriktiv gestarteten Sessions.
- **Version:** v2.1.234

#### Teammate-Modell folgt dem Leader
- **Was:** Die Einstellung „Default teammate model" ist aus `/config` verschwunden; Agent-Team-Teammates nutzen jetzt das Modell des Leaders, sofern der Spawn keines nennt.
- **Einsatz:** Automatisch aktiv (Modell beim Spawn explizit setzbar)
- **Mehrwert:** Eine Einstellung weniger, die unbemerkt vom erwarteten Modell abweichen kann.
- **Version:** v2.1.234

---

### Woche 33 (14. August 2026) — v2.1.233

---

### [Todo-/Task-Tools auf neueren Modellen abgeschaltet — `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` holt sie zurück]
- **Was:** Die **Todo-/Task-Tracking-Tools** (`TaskCreate`, `TaskGet`, `TaskUpdate`, `TaskList` und `TodoWrite`) stehen auf **Opus 4.8, Sonnet 5, Fable 5, Mythos 5 und neueren Modellen nicht mehr zur Verfügung**. Wer sie behalten will, setzt **`CLAUDE_CODE_ENABLE_TODO_TOOLS=1`**.
- **Einsatz:** `export CLAUDE_CODE_ENABLE_TODO_TOOLS=1` (bzw. in `settings.json` unter `env`), um die Tools auf neueren Modellen wieder freizuschalten.
- **Mehrwert:** Der auffälligste Eingriff dieses Releases — und einer, der ohne Vorwarnung Workflows verändert. Die Todo-Liste war ursprünglich eine Krücke: Ältere Modelle verloren bei mehrstufigen Aufgaben den Faden, das sichtbare Abhaken hielt sie auf Kurs. Bei den neueren Modellen kostet dieselbe Mechanik mehr, als sie bringt — Tokens für Listen, die das Modell ohnehin im Kopf führt, plus die Neigung, das Führen der Liste mit dem Erledigen der Arbeit zu verwechseln. Praktisch heißt das: Wer Hooks, Statuszeilen oder Auswertungen auf `TodoWrite`-Ereignissen aufgebaut hat, sieht diese ab v2.1.233 auf modernen Modellen nicht mehr feuern — die Umgebungsvariable ist dann der Weg zurück. Bemerkenswert nebenbei: In der Aufzählung taucht mit **Mythos 5** ein bislang nicht dokumentierter Modellname auf.
- **Version:** v2.1.233

### [Rücknahme: Die Bash-Permission-Änderungen aus v2.1.232 für Cygwin-Symlinks und `< datei` sind zurückgerollt]
- **Was:** Zwei **Permission-Änderungen aus v2.1.232 wurden zurückgenommen**: die Behandlung von **Cygwin-Symlinks unter Windows** und die **Permission-Prüfung von Bash-Eingabeumleitungen** (`< datei`). Eine **engere Fassung soll in einem späteren Release** zurückkommen.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Beide Punkte stehen in dieser Datei noch als Sicherheits-Fixes unter v2.1.232 — sie sind ab v2.1.233 **nicht mehr wirksam**. Der Grund ist absehbar: Eine Regel, die jede Eingabeumleitung prüft, trifft nicht nur `cat < /etc/geheim`, sondern auch das harmlose `while read … done < liste.txt` in jedem zweiten Skript; der Permission-Dialog geht dann bei Routinearbeit auf. Genau diese Reibung zeigte sich unmittelbar (siehe den Auto-Mode-Fix in diesem Release). Der Umgang damit ist die richtige Reihenfolge — erst zurückrollen, dann präziser neu bauen —, aber wer sich auf die Härtung verlassen hat, sollte wissen, dass die Lücke vorerst wieder offen ist.
- **Version:** v2.1.233

### [Sicherheits-Fix: NT-Gerätepräfix `\??\` umging die UNC-Pfadvalidierung (NTLM-Credential-Leak)]
- **Was:** **Windows-Pfade in der NT-Schreibweise mit `\??\`-Gerätepräfix** umgingen die **UNC-Pfadvalidierung** — damit war ein **NTLM-Credential-Leak** möglich. Die Lücke ist geschlossen.
- **Einsatz:** Automatisch aktiv (Windows).
- **Mehrwert:** Der Angriff dahinter ist unter Windows ein Klassiker: Bringt man ein Programm dazu, auf einen UNC-Pfad wie `\\angreifer\share\x` zuzugreifen, versucht Windows die Authentifizierung selbsttätig und schickt den NTLM-Hash des angemeldeten Benutzers an den fremden Host — ohne Zutun des Nutzers. Genau deshalb prüft Claude Code UNC-Pfade. Die NT-Schreibweise `\??\UNC\…` beschreibt dasselbe Ziel, sah für den Validator aber nicht wie ein UNC-Pfad aus. Es genügte also ein anders geschriebener Pfad in einer Datei, die Claude liest, um Anmeldedaten abfließen zu lassen. Zusammen mit dem `\\?\`-Fix aus v2.1.229 ist das der zweite Fund derselben Familie binnen einer Woche.
- **Version:** v2.1.233

### [Speicherlimit für Bash-Kommandos unter Linux (`CLAUDE_CODE_TOOL_MEMORY_LIMIT`)]
- **Was:** Bash-Tool-Kommandos können unter **Linux optional in eine Memory-Cgroup** gesteckt werden — konfiguriert über **`CLAUDE_CODE_TOOL_MEMORY_LIMIT`** —, damit ein **entlaufener Build die Session nicht mehr lahmlegt**.
- **Einsatz:** `export CLAUDE_CODE_TOOL_MEMORY_LIMIT=<limit>` (Opt-in, Linux).
- **Mehrwert:** Wer Claude schon einmal einen großen Build oder Test-Lauf hat starten lassen, kennt das Ergebnis: Der Prozess frisst sich durch den RAM, die Maschine geht ins Swappen, und die Session selbst wird zäh oder stirbt am OOM-Killer — gerne zusammen mit allem anderen, was auf dem Host lief. Das Ärgerliche daran war die Kollateralschaden-Logik: Nicht der Übeltäter wurde erwischt, sondern wer gerade am meisten Speicher hielt. Eine Cgroup dreht das um — das Limit trifft das Kommando, die Session bleibt bedienbar, und der Fehler ist als OOM des Builds sofort erkennbar statt als mysteriöser Session-Tod.
- **Version:** v2.1.233

### [GitLab-Merge-Requests in `--worktree` und der `claude agents`-Ansicht]
- **Was:** Der **`--worktree`-Flag** und die **`claude agents`-Ansicht** akzeptieren jetzt **GitLab-Merge-Request-URLs**; MRs erscheinen in der Ansicht als **`!N`**.
- **Einsatz:** `claude --worktree <gitlab-mr-url>`; in `claude agents` sind MRs an der `!N`-Notation erkennbar.
- **Mehrwert:** Die konsequente Fortsetzung des GitLab-Blocks aus v2.1.232 (Secret-Redaction, `glab`-Schutz, Plugin-Marketplaces): Nachdem die Sicherheits- und Verteilungsseite stand, folgt jetzt der Alltag. Eine MR-URL in die Zwischenablage kopieren und daraus direkt einen Worktree aufmachen, war für GitHub-Nutzer seit Wochen der Standardeinstieg in ein Review — GitLab-Nutzer mussten Branch-Namen von Hand heraussuchen. Das `!N` statt `#N` ist dabei kein kosmetisches Detail, sondern GitLabs eigene Notation: In einer gemischten Agent-Liste sieht man auf einen Blick, welcher Eintrag zu welchem Host gehört.
- **Version:** v2.1.233

### [`forward_user_identity`: Gateway reicht die Nutzeridentität als Header weiter]
- **Was:** Neue **Opt-in-Einstellung `forward_user_identity`** im Apps-Gateway für **Anthropic-Upstreams**: Sie sendet die **Identität des angemeldeten Nutzers als Header** mit, damit ein **Proxy hinter dem Gateway die Kosten pro Nutzer zuordnen** kann.
- **Einsatz:** Gateway-Konfiguration: `forward_user_identity` auf dem Anthropic-Upstream aktivieren.
- **Mehrwert:** Ein zentral betriebenes Gateway hat einen unangenehmen Nebeneffekt: Von außen betrachtet stammt der gesamte Verbrauch der Organisation von einem einzigen Client. Wer intern nach Team oder Kostenstelle umlegen will, stand vor einer Blackbox — es gab schlicht keine Information darüber, welcher Mensch hinter einem Request stand. Dass die Einstellung ein Opt-in ist, ist richtig so: Nutzeridentitäten an ein nachgelagertes System weiterzureichen, ist eine datenschutzrelevante Entscheidung und keine, die man einer Voreinstellung überlässt.
- **Version:** v2.1.233

### [`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`: WebFetch-Cache-Dauer konfigurierbar]
- **Was:** Die neue Umgebungsvariable **`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`** stellt die **TTL des WebFetch-URL-Caches innerhalb einer Session** ein. Der **Standard bleibt unverändert bei 15 Minuten**.
- **Einsatz:** `export CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS=<millisekunden>`
- **Mehrwert:** Der 15-Minuten-Cache ist normalerweise ein Segen — er verhindert, dass dieselbe Doku-Seite in einer Session fünfmal geholt wird. Er wird zum Problem, sobald sich die abgerufene Seite tatsächlich ändert: Wer einen Build-Status, eine Statusseite oder ein frisch deploytes Preview beobachtet, bekam eine Viertelstunde lang beharrlich die alte Antwort und musste raten, ob nichts passiert ist oder nur der Cache zuschlägt. Für Monitoring-artige Schleifen ist ein kurzer Wert die Lösung; umgekehrt spart ein längerer Wert bei reiner Recherche in großen Doku-Beständen spürbar Zeit und Tokens.
- **Version:** v2.1.233

### [Fix: Cloud-Sessions galten als verloren, wenn die Umgebung während eines Permission-Prompts herunterfuhr]
- **Was:** **Cloud-Sessions** wurden gelegentlich **als verloren markiert**, wenn die **Umgebung heruntergefahren wurde, während Claude auf eine Permission-Antwort wartete**.
- **Einsatz:** Automatisch aktiv (Cloud-Sessions).
- **Mehrwert:** Die Konstellation ist häufiger, als sie klingt: Ein Permission-Prompt ist genau der Zustand, in dem eine Session lange nichts tut — und Untätigkeit ist das Kriterium, nach dem Cloud-Umgebungen abgeräumt werden. Wer eine Aufgabe anstieß, zum Mittagessen ging und zurückkam, fand nicht die wartende Frage vor, sondern eine als verloren gemeldete Session. Das kostete nicht nur die Antwort, sondern den gesamten aufgebauten Kontext.
- **Version:** v2.1.233

### [Fix: MCP-v2-Verbindungen öffneten den `subscriptions/listen`-Stream endlos neu]
- **Was:** **MCP-v2-Verbindungen** öffneten den **`subscriptions/listen`-Stream endlos neu** gegenüber Servern, die **lang gehaltene Streams nach einem festen Timeout beenden** — typisch für **serverlose Hosts**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein Aufeinandertreffen zweier je für sich vernünftiger Annahmen: MCP v2 hält einen Stream offen und baut ihn nach Abbruch wieder auf; serverlose Plattformen kappen lang laufende Verbindungen nach fester Frist. Zusammen ergibt das eine Endlosschleife aus Verbinden und Gekapptwerden — mit Last auf beiden Seiten, unnötigen Kaltstarts beim Anbieter und einer Session, die im Hintergrund dauerhaft beschäftigt ist. Wer seinen MCP-Server auf Lambda, Cloud Run oder Vercel betreibt, war praktisch garantiert betroffen.
- **Version:** v2.1.233

### [Fix: Notification-Hooks feuerten unter Claude Desktop und VS Code nicht bei Permission-Prompts]
- **Was:** **Notification-Hooks** feuerten **bei Permission-Prompts nicht**, wenn Claude Code **unter Claude Desktop oder VS Code** lief.
- **Einsatz:** Automatisch aktiv; betrifft `Notification`-Hooks in `settings.json`.
- **Mehrwert:** Der Notification-Hook hat genau einen Zweck: einen darauf aufmerksam zu machen, dass Claude wartet — per Systembenachrichtigung, Ton oder Push aufs Telefon. Dass er ausgerechnet in den beiden Oberflächen schwieg, in denen das Fenster typischerweise im Hintergrund liegt, hob die Funktion faktisch auf. Der Schaden ist nicht der verpasste Ton, sondern die stillstehende Session: Man merkt zwanzig Minuten später, dass seit dem zweiten Schritt eine Freigabe aussteht.
- **Version:** v2.1.233

### [Fix: Leerlaufende Sessions belegten unter Linux mit Sandbox einen CPU-Kern zu 100 %]
- **Was:** **Leerlaufende Sessions** hielten unter **Linux mit aktiviertem Sandboxing** manchmal **einen CPU-Kern dauerhaft bei 100 %**.
- **Einsatz:** Automatisch aktiv (Linux mit Sandbox).
- **Mehrwert:** Eine Busy-Wait-Schleife im Leerlauf ist der teuerste denkbare Fehler pro Zeile Code: Die Session tut nichts, kostet aber einen Kern — auf dem Notebook heißt das Lüfter und Akku, auf dem geteilten Build-Host oder in der Cloud-Umgebung heißt es weniger Leistung für alle anderen. Besonders unangenehm, weil es niemandem als Fehler auffällt: Man sucht die Ursache bei Buildprozessen oder dem Browser, nicht bei der Session, die gerade auf die eigene Eingabe wartet.
- **Version:** v2.1.233

### [Fix: Gebündelte Skill-Aliase wie `/checkup` und `/review` meldeten „Unknown command"]
- **Was:** **Gebündelte Skill-Aliase** wie **`/checkup`** und **`/review`** meldeten **„Unknown command"** — im **`-p`-Modus** oder bei geladenen **Plugins/MCP**, sobald ein **User- oder Projekt-Skill den gebündelten Skill überdeckte**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein Fehlerbild, das sich nicht reproduzieren ließ, wenn man es demonstrieren wollte: In der eigenen interaktiven Session lief `/checkup`, im Skript oder beim Kollegen mit anderem Plugin-Satz nicht. Der Grund war die Alias-Auflösung — ein gleichnamiger eigener Skill verdrängte nicht nur den gebündelten Skill, sondern nahm dessen Alias gleich mit ins Nichts. Für automatisierte Läufe (`-p`) war das ein harter Abbruch mitten in der Pipeline.
- **Version:** v2.1.233

### [Fix: Skill-/Command-Argumente wurden erneut als Template-Marker expandiert]
- **Was:** Die **Argument-Substitution in Skills und Commands** verhindert jetzt, dass **eingesetzte Argumentwerte selbst noch einmal als Template-Marker ausgewertet** werden.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein doppeltes Expandieren ist in jeder Template-Sprache eine Fehlerquelle mit Ansage — und in einer, die Modell-Prompts zusammensetzt, zugleich ein Einfallstor. Harmlos beginnt es damit, dass ein Argument, das zufällig `$1` oder eine `{{…}}`-Klammer enthält, beim zweiten Durchlauf durch etwas ganz anderes ersetzt wird und der Prompt Unsinn ergibt. Unangenehm wird es, wenn der Argumentwert nicht vom Nutzer stammt, sondern aus einer Datei, einem Issue-Titel oder einer CI-Variablen: Dann bestimmt fremder Text mit, welche Template-Bausteine in den Prompt geraten.
- **Version:** v2.1.233

### [Self-hosted Runner: Session startet spürbar schneller]
- **Was:** Der **Session-Start von `claude self-hosted-runner`** wurde beschleunigt: Der **Session-Branch wird angelegt, ohne den Working Tree neu zu schreiben**, und **zwei Server-Roundtrips blockieren den Start des Agenten nicht mehr**.
- **Einsatz:** Automatisch aktiv (`claude self-hosted-runner`).
- **Mehrwert:** Startzeit ist bei selbst gehosteten Runnern kein Komfortthema, sondern multipliziert sich: Jeder CI-Trigger, jeder PR, jeder Routine-Lauf zahlt sie erneut. Das Neuschreiben des Working Tree war dabei der teuerste Posten — bei großen Repositories bedeutet ein Branch-Wechsel mit Dateisystem-Arbeit zehntausende Schreibvorgänge, obwohl sich am Inhalt nichts ändert. Dass zwei Roundtrips jetzt nebenher laufen statt den Start zu blockieren, kommt gerade bei Runnern in entfernten Regionen dazu.
- **Version:** v2.1.233

### [Apps-Gateway: 400er- und 413er-Fehler tragen die Meldung des Upstreams]
- **Was:** Das **Apps-Gateway reicht Fehler besser durch**: **400- und 413-Fehler von Vertex, Foundry und Claude Platform auf AWS** tragen jetzt die **eigene Meldung des Upstreams**. Damit ist auch ein **Fehler beim Auto-Compact über das Apps-Gateway** behoben.
- **Einsatz:** Automatisch aktiv (Gateway-Deployments).
- **Mehrwert:** Ein Gateway, das Fehlermeldungen verschluckt und durch eine generische ersetzt, macht aus einer Fünf-Minuten-Diagnose eine Stunde Rätselraten — gerade bei 400 und 413, wo die Ursache fast immer in Details steckt, die nur der Upstream kennt (welches Feld, welche Grenze, welches Limit). Der Auto-Compact-Fehler ist das Lehrstück dazu: Compaction läuft genau dann an, wenn der Kontext groß ist, also genau dort, wo ein 413 wahrscheinlich wird — und ohne die Upstream-Meldung war nicht erkennbar, dass die Kompaktierung überhaupt an einer Größengrenze scheiterte.
- **Version:** v2.1.233

### [`claude plugin validate` prüft auch ein nacktes `.claude/skills`-Verzeichnis]
- **Was:** **`claude plugin validate`** prüft jetzt auch ein **nacktes `.claude/skills`-Verzeichnis** und meldet **`SKILL.md`-Dateien, deren Frontmatter nicht parst**.
- **Einsatz:** `claude plugin validate`
- **Mehrwert:** Ein Skill mit kaputtem Frontmatter scheitert nicht laut, sondern lautlos: Er taucht einfach nicht in der Liste auf. Wer ihn aufrufen will, bekommt „Unknown command" und sucht die Ursache überall außer in zwei falsch eingerückten YAML-Zeilen. Dass die Prüfung bisher ein Plugin-Manifest voraussetzte, traf ausgerechnet den häufigsten Fall — die Sammlung eigener Skills unter `.claude/skills`, die nie als Plugin verpackt wurde.
- **Version:** v2.1.233

### [Screen-Reader-Modus: `/effort`-Auswahl als nummerierte Liste, kein abgeschnittener Text mehr]
- **Was:** Im **Screen-Reader-Modus** erscheint die **`/effort`-Auswahl als nummerierte Liste mit Zahleneingabe** statt als visueller Slider; **Hinweis- und Dialogtexte werden nicht mehr abgeschnitten**.
- **Einsatz:** Automatisch aktiv im Screen-Reader-Modus; `/effort` aufrufen.
- **Mehrwert:** Der `/effort`-Slider ist ein rein visuelles Bedienelement — für eine Sprachausgabe ist er ein Zeichenband ohne verwertbare Struktur, und die Auswahl per Pfeiltasten setzt voraus, dass man sieht, wo man gerade steht. Eine nummerierte Liste mit getippter Zahl ist die Form, die ohne Bildschirm funktioniert. Der abgeschnittene Text war dabei der tückischere Teil: Visuell ist eine Ellipse als „hier fehlt etwas" erkennbar, in der Vorlesung endet der Satz einfach.
- **Version:** v2.1.233

### [Print-Modus meldet unbekannte Modell-IDs auf stderr]
- **Was:** Im **Print-Modus** wird eine Zeile **`[claude-code:unrecognized_model]` auf stderr** geschrieben, wenn ein Request mit einer **Modell-ID rausgeht, die Claude Code nicht kennt**. Mit **`modelOverrides`** lässt sich die ID zuordnen und die Meldung abstellen.
- **Einsatz:** Automatisch aktiv im `-p`-Modus; Zuordnung über `modelOverrides` in den Settings.
- **Mehrwert:** Unbekannte Modell-IDs sind kein Randfall, sondern Alltag bei Bedrock, Vertex und Foundry, wo IDs eigene Präfixe und Regionsteile tragen. Kennt Claude Code die ID nicht, fehlen ihm Kontextfenster, Preise und Fähigkeiten — die Session läuft trotzdem, aber Budget-Anzeigen, Kompaktierungs-Schwellen und Modellwahl-Logik arbeiten mit Platzhalterwerten. Bisher passierte das stumm, und die Symptome zeigten sich anderswo: unerwartetes Auto-Compact, falsche Kostenanzeigen. Eine klar markierte stderr-Zeile ist genau das richtige Maß — im Skript greifbar, ohne die Ausgabe auf stdout zu verunreinigen.
- **Version:** v2.1.233

### [GitHub-App-Tipp verschwindet in GitLab- und Bitbucket-Repos]
- **Was:** Der **Einrichtungstipp zur GitHub-App** erscheint **nicht mehr in Repositories, deren `origin`-Remote auf `gitlab.com` oder `bitbucket.org` zeigt**; der **Enterprise-Marketplace-Tipp** deckt jetzt auch **interne Git-Hosts außerhalb von GitHub** ab.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein Hinweis, der in der eigenen Umgebung prinzipiell nicht zutrifft, ist schlimmer als kein Hinweis: Man liest ihn ein paar Mal, stellt fest, dass er nie gemeint ist, und gewöhnt sich an, Tipps grundsätzlich zu überlesen — auch die nützlichen. Zusammen mit den GitLab-Neuerungen dieses und des vorigen Releases fügt sich das zu einem Bild: Claude Code hört auf, GitHub als selbstverständliche Voraussetzung zu behandeln.
- **Version:** v2.1.233

### [Windows-Fix: Auto-Mode stoppte bei gewöhnlichem `cd <dir> && <befehl> > datei`]
- **Was:** Unter **Windows** hielt der **Auto-Mode** bei gewöhnlichen Bash-Kommandos der Form **`cd <verzeichnis> && <befehl> > datei`** wiederholt für eine **manuelle Freigabe** an — eine **Regression aus v2.1.232**.
- **Einsatz:** Automatisch aktiv (Windows).
- **Mehrwert:** Auto-Mode ist seit dem 14.08. der Standard auf Pro, Max und Team — die Regression traf also punktgenau zum Rollout und ausgerechnet das häufigste Kommandomuster überhaupt: ins Verzeichnis wechseln, etwas ausführen, Ausgabe in eine Datei schreiben. Ein Auto-Mode, der bei Alltagskommandos ständig nachfragt, verliert genau die Eigenschaft, für die man ihn einschaltet, und erzieht zum reflexhaften Bestätigen — was die Sicherheitswirkung der Nachfrage untergräbt. Der Fix gehört sachlich zur Rücknahme der Eingabeumleitungs-Prüfung weiter oben: Beide Male war die Ursache eine zu grob gefasste Regel rund um Umleitungen.
- **Version:** v2.1.233

---
### Woche 33 (13./14. August 2026) — v2.1.232

---

### [Subagent-Forking ist jetzt Standard — `fork` erbt Konversation und Prompt-Cache]
- **Was:** **Subagent-Forking ist standardmäßig aktiv**: Ein Subagent mit **`subagent_type: "fork"`** erbt die **komplette Konversation samt Prompt-Cache** statt bei null zu starten. Zusätzlich laufen **nicht-Teammitglied-Agenten in interaktiven Sessions jetzt standardmäßig im Hintergrund**.
- **Einsatz:** `Agent`-Tool mit `subagent_type: "fork"`; Hintergrundlauf ist der neue Default, `run_in_background: false` erzwingt den Vordergrund.
- **Mehrwert:** Der teuerste Teil eines Subagenten war bisher das Wiederbeschaffen von Kontext: Man musste ihm in der Prompt-Beschreibung nacherzählen, was in der Hauptsession längst besprochen war — teuer in Tokens, verlustbehaftet im Inhalt. Ein Fork startet mitten im Gespräch und zahlt dank geerbtem Cache nur einen Bruchteil. Zusammen mit dem Hintergrund-Default verschiebt sich die Arbeitsweise: Man delegiert nebenher weiter, statt auf jeden Subagenten zu warten.
- **Version:** v2.1.232

### [`@`-Mention: andere Claude-Session direkt im Prompt ansprechen]
- **Was:** Ein **`@` im Prompt** erwähnt jetzt eine **andere Claude-Session per Namen**; Claude nutzt daraufhin **`SendMessage`**, um diese Session direkt zu erreichen.
- **Einsatz:** Im Prompt `@<session-name>` tippen — die Autovervollständigung schlägt laufende Sessions vor.
- **Mehrwert:** Cross-Session-Messaging existiert seit Woche 32, war aber ein Werkzeug, das man Claude erst umständlich erklären musste („schau in `ListAgents`, finde die Session X, schick ihr …"). Die `@`-Syntax macht daraus eine Geste, die jeder aus Slack kennt: Man adressiert die Nebensession im Fließtext, und Claude übersetzt das in den Tool-Call.
- **Version:** v2.1.232

### [`SendMessage` akzeptiert den bloßen Namen bei eindeutiger Session]
- **Was:** **`SendMessage`** stellt jetzt an einen **bloßen Namen** zu, wenn dieser **genau auf eine lebende Session** passt — die vorherige Rückfrage nach einer Ref entfällt.
- **Einsatz:** `SendMessage({to: "<name>", message: "…"})`.
- **Mehrwert:** Der Ref-Zwang war eine Vorsichtsmaßnahme gegen Namensgleichheit, kostete aber in 95 % der Fälle eine überflüssige Runde: Claude fragte nach, der Nutzer bestätigte, erst dann ging die Nachricht raus. Jetzt greift die Rückfrage nur noch dort, wo sie einen Zweck hat — bei echter Mehrdeutigkeit.
- **Version:** v2.1.232

### [Eindeutige Session-Namen pro Maschine]
- **Was:** **Interaktive Sessions auf einer Maschine tragen jetzt eindeutige Namen**: Startet oder benennt man eine Session auf einen Namen, den eine andere lebende Session bereits führt, bekommt sie eine **`name-wort-wort`-Variante** — und Claude Code sagt einem das.
- **Einsatz:** Automatisch aktiv beim Start und beim Umbenennen von Sessions.
- **Mehrwert:** Die logische Ergänzung zu den beiden Punkten davor: `@`-Mention und namensbasiertes `SendMessage` funktionieren nur, solange Namen eindeutig sind. Wer dreimal `claude` im selben Repo startet, hatte bisher drei gleichnamige Adressaten — Nachrichten landeten zufällig. Die automatische Variante löst das, ohne den Nutzer zum Namensvergeben zu zwingen.
- **Version:** v2.1.232

### [`/config`: „Dialog expiry" und „Messages from your other sessions"]
- **Was:** Zwei neue **`/config`-Zeilen**: **„Dialog expiry"** und **„Messages from your other sessions"** — letztere steuert eingehende Cross-Session-Nachrichten mit den Stufen **annehmen / zurückhalten / ablehnen**.
- **Einsatz:** `/config` → Zeilen „Dialog expiry" bzw. „Messages from your other sessions".
- **Mehrwert:** Cross-Session-Messaging war bisher ein Alles-oder-nichts-Kanal: Wer es aktiviert hatte, wurde von jeder Nebensession unterbrochen. „Hold" ist der praktisch wichtigste Zustand — Nachrichten gehen nicht verloren, reißen die laufende Arbeit aber auch nicht auf. „Dialog expiry" räumt gleichzeitig hängengebliebene Dialoge nach einer Frist weg.
- **Version:** v2.1.232

### [Secret-Redaction für GitLab-Token-Familien; `glab`-CLI wie `gh` geschützt]
- **Was:** Die **Secret-Redaction** erkennt jetzt die **GitLab-Token-Familien** `glrt-`, `gloas-`, `glptt-`, `glagent-`, `glimt-`, `glsoat-`, `glcbt-`, `glft-` und `glffct-` und **redigiert routbare `glpat-`/`gldt-`-Tokens vollständig**. Der **Config-Store der `glab`-CLI** erhält denselben **Sandbox- und Credential-Pfad-Schutz wie `gh`**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Bisher war der Schutz GitHub-zentriert — wer auf GitLab arbeitet, hatte faktisch keinen. Ein `glpat-`-Token, das in einer Fehlerausgabe oder einem `env`-Dump auftauchte, landete unredigiert im Transkript und damit in Logs, Bug-Reports und Cloud-Sessions. Der zweite Teil verhindert, dass Claude die gespeicherten GitLab-Credentials überhaupt lesen kann.
- **Version:** v2.1.232

### [Plugin-Marketplaces von GitLab]
- **Was:** **Plugin-Marketplaces unterstützen jetzt GitLab**: Nackte **`gitlab.com`-Repo-URLs** — **inklusive verschachtelter Subgruppen** — werden wie `github.com`-URLs geklont. Schlägt die Klon-Authentifizierung fehl, **nennt der Hinweis jetzt den tatsächlichen Git-Host**.
- **Einsatz:** `/plugin marketplace add https://gitlab.com/<gruppe>/<untergruppe>/<repo>`
- **Mehrwert:** Das Plugin-Ökosystem setzte GitHub als Distributionsweg voraus — für Unternehmen mit GitLab-Monorepo bedeutete das, interne Plugins gar nicht oder nur über Umwege zu verteilen. Die Subgruppen-Unterstützung ist dabei kein Detail: GitLab-Organisationen sind fast immer mehrstufig geschachtelt, und genau daran scheiterte die naive URL-Erkennung.
- **Version:** v2.1.232

### [Settings-Aliase `additionalMarketplaces` und `allowedMarketplaces`]
- **Was:** Die Settings akzeptieren **`additionalMarketplaces`** und **`allowedMarketplaces`** als **freundlichere Aliase** für `extraKnownMarketplaces` und `strictKnownMarketplaces`.
- **Einsatz:** In `settings.json` wahlweise die neuen oder die alten Schlüssel verwenden.
- **Mehrwert:** Reine Ergonomie, aber an einer Stelle, an der es zählt: Die alten Namen waren schwer zu erraten und wurden regelmäßig falsch geschrieben — mit dem Ergebnis, dass die Einstellung stillschweigend ignoriert wurde und ein Marketplace unerwartet blockiert oder erlaubt blieb.
- **Version:** v2.1.232

### [Enterprise-Policy: `blockedMarketplaces` greift auch bei Klassifikation als Git-Clone]
- **Was:** Ein **url-typisierter `blockedMarketplaces`-Eintrag** für eine nackte Repo-URL **blockiert diese URL jetzt weiterhin**, auch wenn die CLI sie als **Git-Clone** klassifiziert.
- **Einsatz:** Automatisch aktiv; relevant für Managed/Enterprise-Settings mit `blockedMarketplaces`.
- **Mehrwert:** Eine Umgehung mit Ansage: Die Sperre wirkte je nachdem, wie die CLI dieselbe URL intern einordnete — als Marketplace-URL griff sie, als Git-Clone-Ziel nicht. Wer eine Quelle organisationsweit verboten hatte, konnte sie über die alternative Schreibweise trotzdem installieren. Für Enterprise-Deployments ist das der Unterschied zwischen einer Policy und einer Empfehlung.
- **Version:** v2.1.232

### [Gateway: `desktop:`-Overlay akzeptiert alle Desktop-Settings, gegen das Desktop-Schema validiert]
- **Was:** Das **`desktop:`-Overlay** im Gateway akzeptiert jetzt **jede veröffentlichte Desktop-Einstellung** (vorher: 11 handgepflegte Schlüssel) und wird **beim Boot gegen Desktops eigenes Schema validiert** — **unbekannte oder ungültige Schlüssel lassen den Boot scheitern**.
- **Einsatz:** Gateway-Konfiguration, Abschnitt `desktop:`.
- **Mehrwert:** Die handgepflegte Liste hinkte jedem Desktop-Release hinterher: Neue Einstellungen ließen sich zentral nicht ausrollen, und ein Tippfehler wurde stillschweigend verschluckt. Jetzt gilt die vollständige Menge, und Fehler zeigen sich beim Boot statt Wochen später als „warum greift diese Policy auf den Desktops nicht?".
- **Version:** v2.1.232

### [Gateway: leere Gruppen-Matcher und kaputte `email_domain`-Werte scheitern beim Boot]
- **Was:** **Leere `managed.policies[].match.groups`- bzw. `admin.admin_groups`-Einträge** und **fehlerhafte `email_domain`-Werte** (leer oder mit `@`, Leerzeichen oder Kommas) lassen den Gateway-Boot jetzt **scheitern**, statt **still auf niemanden zu matchen oder Admin-Rechte zu vergeben**.
- **Einsatz:** Automatisch aktiv beim Gateway-Start.
- **Mehrwert:** Zwei Fehlerbilder mit entgegengesetztem, aber gleich schlechtem Ausgang: Eine leere Gruppenliste ließ eine Policy geräuschlos ins Leere laufen (die Regel existierte, wirkte aber für niemanden), während ein malformter `email_domain`-Wert im Admin-Kontext im Zweifel **zu viel** gewährte. Beides fiel im Betrieb nicht auf, weil nichts abstürzte. Fail-fast beim Boot ist hier die einzig richtige Antwort.
- **Version:** v2.1.232

### [Fable 5 wieder als Advisor in `/advisor`]
- **Was:** **Fable 5** wird Organisationen mit Fable-Zugang **wieder als Advisor in `/advisor`** angeboten; die Zustimmung zu Usage-Credits läuft über **`/model fable`**.
- **Einsatz:** `/advisor` → Fable 5 wählen; vorab `/model fable` für die Credits-Zustimmung.
- **Mehrwert:** Das Advisor-Feature lebt von Modellvielfalt — ein zweites Modell schaut der Hauptsession über die Schulter. Fable war zwischenzeitlich aus der Auswahl verschwunden; wer den Zugang bezahlt hatte, konnte ihn an dieser Stelle nicht nutzen. Der Consent-Weg über `/model fable` ist dabei die saubere Trennung: erst Kostenzustimmung, dann Verfügbarkeit.
- **Version:** v2.1.232

### [Sicherheits-Fix: PowerShell-Permission-Bypass über `$PSDefaultParameterValues`]
- **Was:** In **PowerShell** konnten **variablenschreibende Parameter** still **`$PSDefaultParameterValues` überschreiben** und damit den **Dateizugriff späterer Kommandos umlenken** — an der Permission-Prüfung vorbei.
- **Einsatz:** Automatisch aktiv (Windows/PowerShell).
- **Mehrwert:** Ein klassischer Bypass zweiter Ordnung: Das genehmigte Kommando war harmlos, aber es veränderte die Default-Parameter der Shell, sodass ein *späteres*, ebenfalls genehmigtes Kommando plötzlich woanders hinschrieb. Die Permission-Prüfung sah jedes Kommando einzeln und für sich korrekt — der Schaden entstand aus der Kombination.
- **Version:** v2.1.232

### [Sicherheits-Fix: Windows-Bypass über Cygwin-Symlinks in Git Bash]
- **Was:** Unter **Windows** folgte **Git Bash Symlinks im Cygwin-Stil**, die die **Pfadvalidierung als gewöhnliche Dateien** ansah. **Schreibzugriffe über solche Links verlangen jetzt eine Genehmigung.**
- **Einsatz:** Automatisch aktiv (Windows/Git Bash).
- **Mehrwert:** Cygwin-Symlinks sind auf NTFS technisch normale Dateien mit einer Markierung im Inhalt — der Validator sah eine unverdächtige Datei im erlaubten Verzeichnis, Git Bash dagegen einen Link, der irgendwohin zeigte. Damit ließ sich jede Pfadbeschränkung unterlaufen, ohne dass etwas verdächtig aussah.
- **Version:** v2.1.232

### [Sicherheits-Fix: Verschachtelte Git-Repos erben das Vertrauen des Elternverzeichnisses nicht mehr]
- **Was:** **Verschachtelte Git-Repositories** erbten bisher das **Trust des übergeordneten Verzeichnisses**; jedes Repository verlangt jetzt seine **eigene Trust-Bestätigung**.
- **Einsatz:** Automatisch aktiv; beim ersten Betreten eines eingebetteten Repos erscheint der Trust-Dialog.
- **Mehrwert:** Genau der Fall, für den die Trust-Abfrage gedacht ist: Ein fremdes Repo landet als Unterverzeichnis im eigenen Workspace — als Klon zum Nachschauen, als eingebettetes Vendor-Verzeichnis, als Ergebnis eines Setup-Skripts. Dessen `.claude/`-Konfiguration, Hooks und Settings wurden bisher ungefragt aktiv, weil das Elternverzeichnis einmal bestätigt worden war.
- **Version:** v2.1.232

### [Sicherheits-Fix: Bash-Eingabeumleitungen (`< datei`) werden jetzt permission-geprüft]
- **Was:** **Bash-Eingabeumleitungen** (`< datei`) werden jetzt **wie ihre Argument-Schreibweisen permission-geprüft** — auf **allen Plattformen**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Eine Lücke, die sich in einem Zeichen versteckte: `cat /etc/geheim` fiel unter die Regeln, `cat < /etc/geheim` nicht — dieselbe Leseoperation, nur anders geschrieben. Wer Dateizugriffe per Permission-Regel eingegrenzt hatte, hatte damit faktisch eine Hintertür in jeder Bash-Zeile.
- **Version:** v2.1.232

### [Härtung: Socket-Verzeichnis für Cross-Session-Messaging auf gemeinsam genutztem `/tmp`]
- **Was:** Das **automatisch angelegte Socket-Verzeichnis** für Cross-Session-Messaging auf gemeinsam genutztem **`/tmp`** ist gehärtet: Ein **vorab platzierter Symlink** oder das **Verzeichnis eines anderen Benutzers** wird jetzt **abgelehnt** statt verwendet.
- **Einsatz:** Automatisch aktiv (macOS/Linux).
- **Mehrwert:** Der Lehrbuchangriff auf `/tmp`: Wer auf einem Mehrbenutzer-Host — Build-Server, Jumphost, geteilte Entwicklungsmaschine — den erwarteten Pfad vorbelegt, bekommt den Nachrichtenverkehr fremder Sessions frei Haus geliefert. Bei Cross-Session-Messaging fließen dort Inhalte aus laufenden Konversationen.
- **Version:** v2.1.232

### [Härtung: Linux-Dateisystem-Sandbox und `sandbox.ripgrep`-Overrides]
- **Was:** Zwei Sandbox-Härtungen: Die **Linux-Dateisystem-Sandbox** wurde gegen eine **Umgehung geschützter Pfade** abgesichert, und **`sandbox.ripgrep`** wird jetzt **nur noch aus User-, Managed- und `--settings`-Einstellungen** berücksichtigt — **Projekt-Settings können das ripgrep-Binary der Sandbox nicht mehr überschreiben**.
- **Einsatz:** Automatisch aktiv (Linux-Sandbox bzw. Settings-Auswertung).
- **Mehrwert:** Der zweite Teil ist der interessantere: Projekt-Settings kommen aus dem geklonten Repository, also potenziell von fremder Hand. Ein dort gesetzter Pfad auf ein „ripgrep" konnte beliebigen Code als Teil der Sandbox-Mechanik ausführen — die Sandbox sabotierte sich also mit Bordmitteln. Sicherheitsrelevante Binärpfade gehören dorthin, wo der Nutzer oder die Organisation entscheidet.
- **Version:** v2.1.232

### [Fix: MCP-Verbindungen hingen die vollen 30 Sekunden am Connect-Timeout]
- **Was:** **MCP-Verbindungen** hingen den **kompletten 30-Sekunden-Connect-Timeout** ab, wenn ein Server auf die **Protokollversions-Probe** **nicht antwortete** oder eine **fehlerhafte Antwort** schickte.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein einziger kaputter MCP-Server verzögerte damit jeden Sessionstart um eine halbe Minute — und weil das Warten unspezifisch aussah, suchte man den Fehler beim Netzwerk oder bei Claude Code selbst statt beim Server. Besonders ärgerlich bei kurzen `-p`-Läufen, in denen die Startzeit die Laufzeit dominiert.
- **Version:** v2.1.232

### [Fix: Remote-Control-Bridge in einer Cloud-Session erbte deren Transkript und Credentials]
- **Was:** **Remote-Control-Sessions**, die von einer **Bridge innerhalb einer Cloud-Session** gehostet wurden, **erbten das Transkript oder die Credentials dieser Session**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der schwerwiegendste Fund dieser Release-Reihe im Remote-Control-Bereich: Eine frisch verbundene Fernsteuerungs-Session konnte den Verlauf einer fremden Cloud-Session sehen — und schlimmer, deren Anmeldedaten mitbenutzen. Das ist eine Vermischung zweier Sicherheitskontexte, die getrennt bleiben müssen.
- **Version:** v2.1.232

### [Fixes: Remote Control — Wiederanbinden, Erreichbarkeit, Historie, gelöschte Sessions]
- **Was:** Vier Korrekturen am Remote-Control-Lebenszyklus: Aus **Claude Desktop oder einer IDE** gestartete Sessions erschienen **bei jedem Resume der lokalen Session als neue claude.ai-Session** — sie hängen sich jetzt wieder an die bestehende an; **im Leerlauf** wirkten Sessions für **neu verbundene Clients unerreichbar**; **Bridge-Sessions stellten die Konversationshistorie nach einem Neustart des Session-Workers nicht wieder her**; und das **Fortsetzen einer Konversation, deren Session in claude.ai oder der App gelöscht wurde**, startet jetzt einen **Ersatz**, statt mit einer irreführenden Login-Meldung zu scheitern (Regression aus v2.1.227).
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Zusammengenommen war Remote Control genau in den Momenten unzuverlässig, für die es gebaut wurde: nach einem Resume, nach einer Denkpause, nach einem Worker-Neustart, nach dem Aufräumen alter Sessions in der App. Die claude.ai-Session-Vermehrung erzeugte zudem eine unbrauchbar zugemüllte Sessionliste. Die Login-Fehlermeldung war doppelt ärgerlich, weil sie auf ein Authentifizierungsproblem zeigte, das gar nicht existierte.
- **Version:** v2.1.232

### [Remote Control: 30 Minuten Reconnect-Fenster und ehrliche Statusmeldungen]
- **Was:** **Remote Control versucht nach einem Netz-Aussetzer jetzt rund 30 Minuten lang, sich wieder zu verbinden**, und bricht **nicht mehr nach ein paar über eine Stunde verteilten Aussetzern** ab. Das **Terminal sagt jetzt außerdem, ob eine Session von einem anderen Gerät übernommen, aus einer anderen App beendet oder gelöscht wurde** — und schlägt keinen Reconnect mehr vor, der das rückgängig machen würde. Ergänzend: Das **Fortsetzen einer Konversation nimmt einem anderen Claude Code auf derselben Maschine die Fernsteuerung nicht mehr still weg** (dort `/remote-control` ausführen, um sie zu verschieben).
- **Einsatz:** Automatisch aktiv; `/remote-control` zum bewussten Übernehmen.
- **Mehrwert:** Der mobile Anwendungsfall lebt von wackligem Netz — Zug, Aufzug, WLAN-Wechsel. Ein zu strenger Abbruch bedeutete: Handy raus, Session weg. Die zweite Hälfte behebt ein Erklärungsproblem: „Verbindung verloren, erneut verbinden?" war bei einer bewusst übernommenen oder gelöschten Session die falsche Antwort und lud dazu ein, sich gegenseitig die Kontrolle wegzunehmen.
- **Version:** v2.1.232

### [Fixes: Enterprise-Anbindung — Cloud-Gateway-Login, mTLS-Rotation, Regionen, Idle-Timeouts]
- **Was:** Vier Fixes an der Anbindung: Der **Cloud-Gateway-`/login`** beendete sich **still** oder hinterließ nach „Press Enter to continue" ein **reaktionsloses Terminal**, wenn Managed Settings nicht geladen werden konnten — der **Grund wird jetzt angezeigt**; die **Rotation eines mTLS-Client-Zertifikats** verlangte einen **Neustart**, jetzt lädt Claude Code **Zertifikat und Schlüssel bei Verbindungsfehlern automatisch neu**; **fehlerhafte AWS- oder Vertex-Regionswerte** wurden in Request-URLs eingebaut und fallen jetzt auf die **Default-Region** zurück; und **Stream-Idle-Timeout-Fehler** ließen den Request scheitern, statt sich auf **Bedrock, Vertex und Gateway-Deployments** zu **erholen**.
- **Einsatz:** Automatisch aktiv; relevant für Bedrock-, Vertex- und Gateway-Setups.
- **Mehrwert:** Alle vier teilen dasselbe Muster: Ein Infrastruktur-Detail, das im Betrieb völlig normal ist — rotierende Zertifikate, ein Tippfehler in einer Region-Variable, eine kurze Streaming-Pause, ein temporär nicht erreichbarer Settings-Endpunkt — führte zu einem harten Abbruch statt zu einer Erholung oder wenigstens einer verständlichen Meldung. Der mTLS-Punkt ist dabei der teuerste: Zertifikatsrotation läuft automatisiert, der erzwungene Neustart traf also planmäßig und regelmäßig.
- **Version:** v2.1.232

### [Fix: Voice-Mode blieb auf „listening…" hängen, wenn der Dienst die Verbindung ablehnte]
- **Was:** Der **Voice-Mode** in **nativen Builds** blieb auf **„listening…"** stehen, wenn der **Sprachdienst die Verbindung ablehnte**; die **Ablehnung wird jetzt sofort angezeigt**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein stiller Fehlzustand, den man nicht als solchen erkennt: Man sprach weiter ins Mikrofon und wartete auf eine Reaktion, die nie kam. Ohne Rückmeldung ließ sich nicht unterscheiden, ob der Dienst ablehnt, das Mikrofon stumm ist oder Claude einfach lange braucht.
- **Version:** v2.1.232

### [Managed-Settings-Dialog: Endpunkt-URLs sichtbar, Sandbox-Binary-Overrides zustimmungspflichtig]
- **Was:** Der **Genehmigungsdialog für Managed Settings** zeigt jetzt **Endpunkt-URLs**, formuliert **rein telemetriebezogene Änderungen klarer**, **überspringt Routine-OpenTelemetry-Optionen** und **verlangt ausdrücklich Zustimmung für server-verwaltete Sandbox-Binary-Overrides** (`sandbox.bwrapPath`, `sandbox.socatPath`, `sandbox.ripgrep`).
- **Einsatz:** Automatisch aktiv beim Übernehmen server-verwalteter Einstellungen.
- **Mehrwert:** Ein Zustimmungsdialog, der bei jeder Kleinigkeit erscheint, wird weggeklickt — genau dann, wenn einmal etwas Ernstes drinsteht. Die Neuordnung dreht das um: Routine-Telemetrie verschwindet aus dem Dialog, während die drei Einstellungen, die den Sandbox-Mechanismus selbst austauschen können, jetzt garantiert eine bewusste Entscheidung erfordern. Genau die Klasse von Override, die der `sandbox.ripgrep`-Fix aus derselben Version auch auf Settings-Ebene entschärft.
- **Version:** v2.1.232

### [Performance: Fullscreen-Streaming bleibt in langen Sessions flüssig]
- **Was:** Das **Fullscreen-Streaming** wurde verbessert: **Lange Sessions bleiben reaktionsfähig**, weil die **gesamte Konversation nicht mehr bei jedem Update neu normalisiert** wird. Zusätzlich werden **eingefügte und aus der Zwischenablage stammende Bilder gelesen, ohne den Event-Loop zu blockieren**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Beides sind Ruckler, die mit der Sitzungsdauer bzw. der Dateigröße wachsen — also genau dann auftreten, wenn man tief in einer Aufgabe steckt. Die Neu-Normalisierung der kompletten Historie bei jedem Streaming-Tick machte lange Sessions zäh, obwohl sich nur die letzten Zeilen änderten; ein großer Screenshot aus der Zwischenablage fror die Oberfläche kurz komplett ein.
- **Version:** v2.1.232

### [`/feedback` und `/bug` öffnen sofort, auch während Claude antwortet]
- **Was:** **`/feedback`** und **`/bug`** öffnen jetzt **sofort**, wenn sie während einer laufenden Antwort aufgerufen werden — statt bis zum Ende des Zuges zu warten.
- **Einsatz:** `/feedback`, `/bug`
- **Mehrwert:** Man meldet einen Fehler in dem Moment, in dem er passiert — mitten in der Antwort, in der Claude gerade danebenliegt. Bisher musste man den Zug abwarten und hatte den Kontext (und oft die Lust) verloren, bis der Dialog aufging.
- **Version:** v2.1.232

### [`/plugin install` frischt den Marketplace vorab auf; `/code-review` durchgängig im Hintergrund]
- **Was:** **`/plugin install plugin@marketplace`** **aktualisiert jetzt zuerst den Marketplace**, sodass **frisch veröffentlichte Plugins ohne manuelles Marketplace-Update** installierbar sind. Außerdem läuft **`/code-review` auf den Stufen high, xhigh und max jetzt ebenfalls in einem Hintergrund-Agenten** wie die übrigen Stufen.
- **Einsatz:** `/plugin install <plugin>@<marketplace>`; `/code-review high|xhigh|max`
- **Mehrwert:** Der Plugin-Teil beseitigt eine klassische Support-Frage („ich habe es gerade veröffentlicht, aber die Installation findet es nicht"). Der Review-Teil beendet eine Inkonsistenz mit Ansage: Ausgerechnet die aufwendigsten — also langsamsten — Review-Stufen blockierten die Session, während die schnellen im Hintergrund liefen.
- **Version:** v2.1.232

### [Fix: `/update` und `/tui` verweigerten den Neustart bei laufender Arbeit, die den Neustart überlebt]
- **Was:** **`/update`** und **`/tui`** verweigerten den Neustart, während **Arbeit lief, die den Neustart ohnehin überdauert**.
- **Einsatz:** `/update`, `/tui`
- **Mehrwert:** Die Sperre war als Schutz gedacht, traf aber den falschen Fall: Hintergrundaufgaben, die den Relaunch überstehen, waren nie in Gefahr. In der Praxis hieß das, dass man ein Update wegen eines Jobs aufschob, der davon gar nicht betroffen war — und es dann vergaß.
- **Version:** v2.1.232

### [Fixes: Rendering-Kleinkram — Overlay-Breite, abgeschnittene Emojis, Agent-Panel]
- **Was:** Mehrere Darstellungs-Fixes: **Inhaltsbreite-basierte Overlays mit gekürztem Text** rendern nicht mehr **eine Spalte zu breit**, und **am Anfang gekürzter Text** fällt nicht mehr auf eine bloße Ellipse zusammen; ein **verunstaltetes Zeichen** an der Abschneidekante **mitten in einem Emoji** (lange Shell-Kommando- oder Agent-Beschreibungs-Vorschauen) ist behoben. Das **Agent-Panel** blendet **abgeschlossene Subagenten sofort aus** und zeigt stattdessen einen **`/tasks`-Fußzeilenhinweis**; der **„↓ N more"-Überlauf-Indikator** wurde **nach links** geschoben, damit er sichtbar bleibt. Die Meldung beim **Fortsetzen eines abgeschlossenen Hintergrund-Agenten** wurde gekürzt.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Einzeln Kosmetik, zusammen der Unterschied zwischen einem Terminal-UI, dem man traut, und einem, das ständig leicht verrutscht wirkt. Das Agent-Panel-Verhalten hat dabei echten Nutzwert: Bei vielen parallelen Subagenten verstopften die längst fertigen die Liste, sodass man die noch laufenden nicht mehr sah.
- **Version:** v2.1.232

### [Fixes: Plugin-Marketplace-Startrace, Usage-Limit-Hinweise, Advisor-Consent-Meldung]
- **Was:** Drei weitere Korrekturen: Ein **Start-Race** konnte durch **gleichzeitige Schreibzugriffe auf `known_marketplaces.json`** einen **Plugin-Marketplace still abmelden**; die **Usage-Limit-Hinweise** schlugen in **SDK- und Remote-Sessions Slash-Kommandos vor, die dort gar nicht verfügbar sind**; und die **Consent-Meldung für interaktive `--advisor fable`-Starts** forderte dazu auf, `/model fable` in einer interaktiven Session auszuführen, **die sich soeben beendet hatte**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der Marketplace-Race ist der ernsteste der drei, weil er sich als Nutzerfehler tarnte: Ein Marketplace verschwand ohne Zutun, Plugins waren plötzlich weg, und niemand konnte erklären, warum. Die anderen beiden sind Sackgassen-Meldungen — Anweisungen, denen man in der jeweiligen Umgebung schlicht nicht folgen kann.
- **Version:** v2.1.232

### [Cowork-Sessions inlinen keine externen `@`-Imports aus User-Scope-Memory mehr]
- **Was:** **Cowork-Sessions** binden **externe `@`-Importe aus User-Scope-Memory-Dateien nicht mehr inline ein**.
- **Einsatz:** Automatisch aktiv (Cowork).
- **Mehrwert:** User-Scope-Memory (`~/.claude/CLAUDE.md`) verweist typischerweise per `@`-Import auf private Notizen, Vault-Dateien oder Projektdokumente außerhalb des aktuellen Arbeitsverzeichnisses. In einer Cowork-Session — die geteilt und synchronisiert läuft — wanderten diese Inhalte damit unbemerkt in einen Kontext, für den sie nie gedacht waren.
- **Version:** v2.1.232

### [Entfernt: Startup-Tipp zu eigenen Subagenten und der `/powerup`-Hinweis dazu]
- **Was:** Der **Startup-Tipp**, eigene Subagenten anzulegen, sowie der passende **Hinweis in der `/powerup`-Tour** wurden **entfernt**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Eine konsequente Folge des Fork-Defaults aus derselben Version: Wenn ein Fork die laufende Konversation erbt, ist das Anlegen eines handgeschriebenen Subagenten für viele Fälle nicht mehr der empfohlene Weg. Ein Tipp, der zu einem überholten Muster rät, kostet mehr als er nützt.
- **Version:** v2.1.232

### [Claude Tag liest den Raum besser — rund 30 % treffsicherere Wortmeldungen in Slack]
- **Was:** **Claude Tag**, die Slack-Integration, entscheidet jetzt anhand **breiteren Kanalkontexts**, wann es sich **von selbst einschaltet**: Statt jede Nachricht **isoliert** mit Ja/Nein zu bewerten, betrachtet Claude **mehrere Nachrichten zusammen** — plus **kanalübergreifenden Kontext**, **Gesprächsverlauf**, **eigenes Gedächtnis** und **konfigurierte Anweisungen**. Vier mögliche Reaktionen: **inline antworten**, **Thread-Arbeit starten**, **an einen bestehenden Workstream weiterleiten** oder **schweigen**. Laut Anthropic rund **30 % treffsicherer** in der Entscheidung, wann ein Beitrag angebracht ist. Claude kann in Kanälen, in denen es wiederholt nichts beizutragen findet, **„einschlafen"** und wacht dort nur noch bei direkter Erwähnung auf. Steuerbar in normaler Sprache („Never respond here unless tagged") oder pro Kanal; jedes Mitglied kann **„Respond automatically"** ein- und ausschalten. **Der zusätzliche Kontext zählt auf keinem Plan gegen die Nutzungslimits.**
- **Einsatz:** Claude Tag in Slack; Verhalten per Klartext-Anweisung oder Kanal-Einstellung steuern, Schalter „Respond automatically".
- **Mehrwert:** Das Kernproblem eines proaktiven Bots in einem Team-Kanal ist nicht die Antwortqualität, sondern das Timing — ein sachlich korrekter Einwurf im falschen Moment ist Lärm, und Lärm führt dazu, dass die Integration abgeschaltet wird. Die Isolationsbetrachtung war dafür strukturell blind: Eine Frage kann rhetorisch sein, längst beantwortet oder an eine bestimmte Person gerichtet — das sieht man erst im Zusammenhang. Der Schlafmodus ist die ehrlichste Neuerung: Claude zieht sich aus Kanälen, in denen es nichts beizutragen hat, von selbst zurück. Dass der zusätzliche Kontext nicht auf die Limits zählt, nimmt der Sache die Kostenangst.
- **Version:** Keine CLI-Version — Claude-Tag/Slack-Feature (Blog-Announcement vom 13.08.2026; Claude Teams und Enterprise)

---

### Woche 33 (13. August 2026) — v2.1.231

---

### [Fix: MCP-OAuth-Login scheiterte bei Servern mit vorregistriertem OAuth-Client (z. B. Slack)]
- **Was:** Der **MCP-OAuth-Login** schlug mit einem **Redirect-URI-Mismatch** fehl, wenn der Server einen **vorregistrierten OAuth-Client** verwendet — **Slack** ist das prominenteste Beispiel.
- **Einsatz:** Automatisch aktiv beim Login an OAuth-geschützten MCP-Servern (`claude mcp login`, `/mcp`).
- **Mehrwert:** Das ist die Kehrseite des Fixes aus v2.1.229 vom Vortag: Dort wurde die Redirect-URI von `localhost` auf `127.0.0.1` umgestellt, damit strikte Authorization-Server den Loopback akzeptieren. Server mit **vorregistriertem Client** vergleichen die URI aber **zeichengenau gegen den hinterlegten Wert** — und der lautet bei ihnen weiterhin `localhost`. Dadurch reparierte v2.1.229 die eine Server-Klasse und brach die andere; wer Slack per MCP angebunden hat, kam seit gestern nicht mehr durch den Login. v2.1.231 behandelt jetzt beide Fälle. Betroffene sollten direkt auf v2.1.231 gehen und v2.1.229/230 überspringen.
- **Version:** v2.1.231

---

### Woche 33 (12. August 2026) — v2.1.229

---

### [`claude remote-control --continue` — zuletzt genutzte Remote-Control-Session fortsetzen]
- **Was:** Der Schalter **`--continue`** für `claude remote-control` ist jetzt **dokumentiert**: Er nimmt die **zuletzt genutzte Remote-Control-Session** wieder auf, statt eine neue zu eröffnen oder erst aus einer Liste auszuwählen.
- **Einsatz:** `claude remote-control --continue`
- **Mehrwert:** Remote Control ist ein Feature, das man typischerweise mehrfach am Tag an derselben Session andockt — vom Handy, vom zweiten Rechner, nach einem Netzabbruch. Der Weg dorthin führte bisher jedes Mal über die Auswahl; jetzt genügt ein Flag. Das Verhalten spiegelt `claude --continue` und macht die Wiederaufnahme skript- und alias-tauglich.
- **Version:** v2.1.229

### [Self-hosted Runner: Server-gelieferte Hooks wie in Managed Environments]
- **Was:** **Self-hosted-Runner-Sessions** unterstützen jetzt **vom Server ausgelieferte Claude-Code-Hooks** — dasselbe Verhalten, das Managed Environments bereits kennen.
- **Einsatz:** Automatisch aktiv auf Self-hosted Runnern (Team/Enterprise); Hooks werden zentral vom Server bereitgestellt.
- **Mehrwert:** Schließt die letzte große Verhaltenslücke zwischen selbst betriebener und von Anthropic gemanagter Infrastruktur: Organisationsweite Policies, die per Hook durchgesetzt werden (Audit-Logging, Secret-Scanning, verbotene Kommandos), galten auf eigenen Runnern bisher schlicht nicht. Wer Self-hosted Compute wegen Compliance wählt, bekommt jetzt genau dort auch die Compliance-Mechanik.
- **Version:** v2.1.229

### [SSE-Keepalive: keine Idle-Timeout-Abbrüche mehr bei langen Denkpausen auf Vertex und Bedrock]
- **Was:** Streaming-Antworten über das Gateway senden während **langer Denkpausen** jetzt **SSE-Keepalive-Pings** und verhindern so **Idle-Timeout-Trennungen** bei **Vertex-AI-** und **Bedrock-Upstreams**.
- **Einsatz:** Automatisch aktiv bei Nutzung über Vertex AI bzw. Amazon Bedrock.
- **Mehrwert:** Das klassische Enterprise-Ärgernis bei hohen Effort-Leveln: Genau die schweren Aufgaben, bei denen Claude minutenlang denkt, bevor das erste Token kommt, rissen an Load-Balancern und Proxies ab — die teuerste Arbeit ging als Erstes verloren. Ein Ping-Herzschlag hält die Verbindung offen, ohne dass man Timeouts in der Infrastruktur hochdrehen muss.
- **Version:** v2.1.229

### [Plugin-Marketplaces aus einem lokalen Kommando (`command`-Quellen)]
- **Was:** Ein Plugin-Marketplace kann jetzt als **`command`-Quelle** definiert werden: Ein **lokales Kommando** (z. B. eine IDE) gibt das **Plugin-Verzeichnis** aus, das **bei jeder Session neu aufgelöst** und **ohne Neustart** angewendet wird. Mit **`mode: "link"`** wird das Verzeichnis **an Ort und Stelle** verwendet statt kopiert.
- **Einsatz:** Marketplace-Eintrag in den Settings mit `command`-Quelle definieren; optional `"mode": "link"` setzen.
- **Mehrwert:** Löst zwei Probleme auf einmal. Erstens Tool-Integration: Eine IDE oder ein internes CLI kann Claude Code sagen, wo ihre mitgelieferten Plugins gerade liegen — auch wenn sich der Pfad mit jedem Update ändert, statt ihn in Settings hart zu verdrahten. Zweitens Plugin-Entwicklung: `mode: "link"` arbeitet direkt im Arbeits-Checkout, sodass Änderungen ohne Neuinstallation greifen.
- **Version:** v2.1.229

### [`ListAgents` markiert getrennte Remote-Control-Sessions als `offline` und Cloud-Sessions als `cloud`]
- **Was:** Die Agenten-Liste kennzeichnet jetzt **getrennte Remote-Control-Sessions** ausdrücklich als **`offline`** und **eigene Cloud-Sessions** als **`cloud`**.
- **Einsatz:** `ListAgents` (Tool) bzw. die Agenten-Ansicht — Kennzeichnung erscheint automatisch.
- **Mehrwert:** Wer per `SendMessage` zwischen Sessions kommuniziert, schickte Nachrichten bisher blind an Adressaten, die längst weg waren — ohne erkennbaren Unterschied zwischen „läuft auf dem anderen Rechner" und „ist tot". Die Kennzeichnung macht die Zustellbarkeit vorab sichtbar und trennt zugleich lokale von Cloud-Sessions, wenn dieselben Namen mehrfach auftauchen.
- **Version:** v2.1.229

### [Fix: Lange Antworten verschwanden beim Streaming teilweise und wurden doppelt ausgegeben]
- **Was:** **Lange Antworten** konnten während des Streamings **teilweise verschwinden** und anschließend **zweimal im Terminal** erscheinen.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der sichtbarste Darstellungsfehler dieser Release-Reihe: Man las eine Erklärung mit, verlor mittendrin einen Absatz und bekam am Ende den ganzen Block ein zweites Mal — was in einem gescrollten Transkript kaum von einer echten Wiederholung durch Claude zu unterscheiden war. Betraf ausgerechnet lange, inhaltsreiche Antworten.
- **Version:** v2.1.229

### [Fix: Absturz zum Fehlerbildschirm bei Tool-Calls mit nicht-textuellem `glob`, `file_path` oder `command`]
- **Was:** Enthielt ein Tool-Call für **`glob`**, **`file_path`** oder **`command`** einen **Wert, der kein String war**, stürzte Claude Code auf den Fehlerbildschirm ab — **auch beim `--resume`** der betroffenen Session.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der zweite Teil wog schwerer als der erste: Ein einziger fehlgeformter Tool-Call machte die Session **dauerhaft unbrauchbar** — jeder Wiederaufnahme-Versuch lief in denselben Absturz, die gesamte Konversation war damit verloren. Solche Werte entstehen typischerweise bei Modell-Ausrutschern oder in SDK-Integrationen, also unverschuldet.
- **Version:** v2.1.229

### [Fix: RangeError-Absturz bei Fortschrittsbalken und Markdown-Tabellen in sehr schmalen Terminals]
- **Was:** Ein **Fortschrittsbalken** oder eine **Markdown-Tabelle** in einem **sehr schmalen Terminalfenster** löste einen **RangeError-Absturz** aus — der auch `claude --continue`/`--resume` **direkt beim Start** treffen konnte.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Betraf reale Alltags-Layouts: ein schmaler Split neben dem Editor, ein tmux-Pane, ein Terminal am Bildschirmrand. Da eine wiederaufgenommene Session ihr Transkript beim Start neu rendert, genügte **eine** früher ausgegebene Tabelle, um den Start in dieser Fensterbreite reproduzierbar scheitern zu lassen.
- **Version:** v2.1.229

### [Fix: Windows-Absturz bei Extended-Length- (`\\?\`) und UNC-Pfaden]
- **Was:** Unter **Windows** stürzte Claude Code ab, wenn ein Tool-Call oder eine Nachricht eine Datei über einen **Extended-Length-Pfad (`\\?\…`)** oder einen **UNC-Pfad** (`\\server\share\…`) referenzierte.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Genau die beiden Pfadformen, die in Unternehmensumgebungen normal sind: Netzlaufwerke per UNC und die `\\?\`-Schreibweise, die Windows für Pfade jenseits von 260 Zeichen erzwingt — beides tauchte in tief verschachtelten Monorepos regelmäßig auf und beendete die Session.
- **Version:** v2.1.229

### [Fix: Auto-Mode scheiterte bei jedem Tool-Call, wenn der Attribution-Header deaktiviert war]
- **Was:** Wer den Attribution-Header über **`CLAUDE_CODE_ATTRIBUTION_HEADER`** deaktiviert hatte, erlebte bei **direkten Anthropic-API-Verbindungen**, dass der **Auto-Mode bei jedem einzelnen Tool-Call fehlschlug**.
- **Einsatz:** Automatisch aktiv; relevant bei gesetzter `CLAUDE_CODE_ATTRIBUTION_HEADER`-Variable.
- **Mehrwert:** Eine harmlos wirkende Datenschutz-/Compliance-Einstellung legte das komplette Permission-System lahm — und das ausgerechnet kurz vor dem **Standard-Rollout des Auto-Mode am 14.08.** Wer aus Gründen der Datensparsamkeit den Header abschaltet, hätte den neuen Default sonst als „Auto-Mode funktioniert bei mir grundsätzlich nicht" erlebt.
- **Version:** v2.1.229

### [Fix: `/model` verweigerte Sonnet/Opus 1M bei claude.ai-Abo hinter eigenem Gateway]
- **Was:** **`/model`** lehnte die **1M-Kontext-Varianten von Sonnet und Opus** für **claude.ai-Abonnenten** ab, die eine eigene **`ANTHROPIC_BASE_URL`** (Gateway) verwenden.
- **Einsatz:** Automatisch aktiv; betrifft Setups mit `ANTHROPIC_BASE_URL` und claude.ai-Abo.
- **Mehrwert:** Firmen-Gateways sind der übliche Weg, Claude-Code-Traffic zu protokollieren oder zu filtern — und genau dieses Setup schnitt Nutzer vom stärksten verfügbaren Merkmal ab, dem 1M-Token-Fenster. Betroffene mussten zwischen Gateway und großem Kontext wählen; jetzt geht beides.
- **Version:** v2.1.229

### [Fix: MCP-OAuth mit strikten Authorization-Servern nutzt `127.0.0.1` statt `localhost`]
- **Was:** Die **Redirect-URI** im MCP-OAuth-Flow verwendet jetzt **`127.0.0.1`** statt **`localhost`** — damit funktionieren auch **strikte Authorization-Server**.
- **Einsatz:** Automatisch aktiv beim Login an OAuth-geschützten MCP-Servern (`claude mcp login`, `/mcp`).
- **Mehrwert:** Manche OAuth-Server folgen der Empfehlung, für native Clients ausschließlich Loopback-**IP-Literale** zu akzeptieren, und wiesen `http://localhost:…` als ungültige Redirect-URI zurück. Der Login scheiterte dann mit einer Fehlermeldung, die auf den Client zeigte, obwohl die Ursache in einer Zeichenkette lag — betroffene MCP-Server sind jetzt ohne serverseitige Ausnahme nutzbar.
- **Version:** v2.1.229

### [Fix: Remote-Control-Clients hingen nach einem Slash-Kommando im Laptop-Terminal im „Working"-Spinner]
- **Was:** Wurde ein **Slash-Kommando direkt im Laptop-Terminal** getippt, zeigten die verbundenen **Remote-Control-Clients** dauerhaft einen **hängenden Arbeits-Spinner**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Für die Gegenstelle — meist das Handy — sah eine längst fertige Session aus wie eine ewig rechnende. Man wartete auf ein Ergebnis, das schon da war, oder brach eine gesunde Session ab. Der Zustand ließ sich nur durch Neuverbinden auflösen.
- **Version:** v2.1.229

### [Fix: `/install-github-app`-Review-Workflow lief durch, ohne den Review zu posten]
- **Was:** Der von **`/install-github-app`** erzeugte **Claude-Code-Review-Workflow** schloss erfolgreich ab, **ohne seinen Review am Pull Request zu veröffentlichen**.
- **Einsatz:** Automatisch aktiv für neu generierte Workflows; bestehende Workflows ggf. per `/install-github-app` neu erzeugen.
- **Mehrwert:** Der heimtückischste Fehlerzustand für CI: grüner Haken, keine Fehlermeldung, kein Review. Teams, die den automatischen Review als Qualitätsstufe eingeplant hatten, mergten wochenlang ungeprüft und hielten sich für abgesichert — ein stiller Ausfall statt eines sichtbaren.
- **Version:** v2.1.229

### [Fix: Sekundenlange UI-Hänger bei Dateien mit tausenden IDE-Diagnostics]
- **Was:** Nach dem Bearbeiten einer Datei mit **tausenden IDE-Diagnostics** blockierte die Oberfläche bei verbundener **IDE-Extension** mehrere Sekunden.
- **Einsatz:** Automatisch aktiv bei verbundener IDE-Extension (VS Code / JetBrains).
- **Mehrwert:** Betraf typischerweise große generierte Dateien oder Projekte mitten in einer Migration, in denen der Type-Checker Tausende Fehler meldet — also genau die Situationen, in denen man Claude beim Aufräumen zusieht. Jede Bearbeitung kostete dann einen mehrsekündigen Freeze; das Werkzeug wurde ausgerechnet dort langsam, wo es am meisten arbeitet.
- **Version:** v2.1.229

### [Fix: Einmalige `claude plugin`-Kommandos ließen eine Liveness-Datei zurück]
- **Was:** **One-shot-`claude plugin`-Kommandos** hinterließen eine **verwaiste Liveness-Datei**, die das Aufräumen veralteter Plugin-Versionen verhindern konnte.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Die Liveness-Datei signalisiert „hier läuft noch jemand" und schützt Plugin-Versionen vor der Bereinigung. Blieb sie liegen, sammelten sich alte Versionen unbegrenzt an — ein leiser Plattenfresser, der niemandem auffiel, weil nichts kaputtging.
- **Version:** v2.1.229

### [Fix: Dynamische Workflows rechneten in CPU-limitierten Containern mit den Kernen des Hosts]
- **Was:** **Dynamische Workflows** in **CPU-limitierten Containern** bestimmten ihre Parallelität anhand der **Kernzahl der Host-Maschine** statt anhand des **CPU-Limits des Containers**.
- **Einsatz:** Automatisch aktiv; relevant für Docker/Kubernetes/CI-Umgebungen mit CPU-Limits.
- **Mehrwert:** Der Nebenläufigkeits-Deckel von Workflows leitet sich aus der Kernzahl ab — auf einem 64-Kern-Host mit einem 2-Kern-Limit startete ein Fan-out damit ein Vielfaches dessen, was der Container tragen konnte. Ergebnis: Throttling, Timeouts und Workflows, die in CI langsamer liefen als lokal, ohne erkennbaren Grund.
- **Version:** v2.1.229

### [Fix: File-Watcher-Handle-Leak nach atomaren Dateiersetzungen; Windows-Fehler beim Scheduled-Tasks-Watcher]
- **Was:** Zwei Watcher-Fixes: Nach **atomaren Dateiersetzungen** (Schreiben in eine temporäre Datei + Umbenennen) **leckten File-Watcher-Handles**; unter **Windows** trat außerdem ein **nicht abgefangener Fehler** auf, wenn der **Scheduled-Tasks-Watcher** auf einem **Netz- oder virtuellen Dateisystem** scheiterte.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Atomares Ersetzen ist das Standardverhalten der meisten Editoren und Formatter — in langen Sessions mit vielen Speichervorgängen liefen die Handles voll, bis das Betriebssystem Grenzen zog und Datei-Operationen sporadisch scheiterten. Der Windows-Teil traf Setups mit Projekten auf Netzlaufwerken.
- **Version:** v2.1.229

### [Fix: Leere Nachricht führte in SDK- und `stream-json`-Sessions zu API-Fehler 400]
- **Was:** **SDK-Sessions** und Läufe mit **`--input-format stream-json`** liefen in einen **API-Fehler 400**, wenn eine Nachricht **nur aus Leerzeichen** bestand.
- **Einsatz:** Automatisch aktiv; relevant für Agent-SDK- und Pipeline-Integrationen.
- **Mehrwert:** In automatisierten Pipelines entstehen leere Eingaben ganz beiläufig — eine Variable ist nicht gesetzt, ein Template rendert nichts, ein Log-Tail liefert eine Leerzeile. Bisher riss das den ganzen Lauf mit einem rohen HTTP-Fehler ab, statt die Nachricht sauber zu behandeln.
- **Version:** v2.1.229

### [Fix: Konversationen jenseits des 32-MB-Request-Limits scheitern jetzt einmal mit klarer Meldung]
- **Was:** Überschritten allein die **Nachrichten** einer Konversation das **32-MB-Request-Limit der API**, versuchte Claude Code die **Kompaktierung wiederholt** — auch wenn sich **keine Bilder oder Dokumente mehr entfernen** ließen. Jetzt schlägt der Versuch **einmal** mit einer **klaren Meldung** fehl.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein aussichtsloser Retry-Kreisel: Die Kompaktierung strippt normalerweise Anhänge, um unter das Limit zu kommen — gibt es keine, ändert jeder weitere Versuch nichts, kostet aber Zeit und Tokens. Jetzt erfährt man sofort, dass der Kontext strukturell zu groß ist, und kann gezielt handeln (neue Session, `/clear`, Historie aufteilen).
- **Version:** v2.1.229

### [Fix: OpenTelemetry-Export aus Claude-Desktop-Sessions wurde vom Desktop-Gateway abgelehnt]
- **Was:** Der **OpenTelemetry-Export** aus **Claude-Desktop-Sessions** wurde vom **Desktop-verwalteten Gateway** zurückgewiesen, wenn dieses Gateway **zugleich der Telemetrie-Endpunkt** ist.
- **Einsatz:** Automatisch aktiv; relevant für Setups mit OTel-Monitoring von Claude Code.
- **Mehrwert:** Traf Organisationen, die Nutzung und Kosten zentral auswerten: Desktop-Sessions fehlten in den Dashboards, ohne dass jemand einen Fehler sah — die Zahlen wirkten nur unauffällig niedrig. Wer Budget-Entscheidungen auf diese Telemetrie stützt, sollte nach dem Update prüfen, ob Desktop-Sessions jetzt auftauchen.
- **Version:** v2.1.229

### [Self-hosted Runner: `managed-mcp.json` beendete Sessions beim Start, Git-Credential-Prompt hängte die Repo-Vorbereitung, Windows verlangt `--base-dir`]
- **Was:** Drei Änderungen für **Self-hosted Runner** und andere Remote-Sessions. **Erstens:** War **`managed-mcp.json`** ausgerollt **und** lieferte der Server zusätzlich MCP-Server aus, **beendete sich die Session beim Start** — diese Server werden jetzt **mit Warnung übersprungen**. **Zweitens:** Die **Repository-Vorbereitung** blieb an einer Eingabeaufforderung des **Git Credential Managers** hängen; `git` **scheitert jetzt sofort**, wenn Credentials fehlen. **Drittens** (Verhaltensänderung): Der **Windows-Start** verlangt jetzt ein **explizites `--base-dir`** — ein Standard-Checkout-Verzeichnis gibt es unter Windows nicht.
- **Einsatz:** Automatisch aktiv; unter Windows `claude self-hosted-runner --base-dir <Pfad>` angeben.
- **Mehrwert:** Die dritte Härtungsrunde für Self-hosted Compute in Folge (nach v2.1.225 und v2.1.228) — und wieder sind es Fehler, die die ganze Flotte trafen statt einzelner Läufe. Beide Fixes haben dasselbe Muster: still hängen oder still sterben statt sagen, was fehlt. Wichtig für Betreiber: Der `--base-dir`-Zwang unter Windows ist eine **breaking change** in bestehenden Start-Skripten.
- **Version:** v2.1.229

### [Workflow-Fan-outs staffeln Geschwister-Agents und sparen Prompt-Cache-Kosten]
- **Was:** Bei **Workflow-Fan-outs** werden **Geschwister-Agents mit gleichem Prompt-Präfix zeitlich gestaffelt** gestartet, damit die nachfolgenden Agents das **bereits gecachte Präfix lesen**, statt es erneut zu bezahlen. Abschaltbar über **`CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS=0`**.
- **Einsatz:** Automatisch aktiv in dynamischen Workflows; `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS=0` deaktiviert die Staffelung.
- **Mehrwert:** Direkt spürbare Kostensenkung bei genau dem Muster, das Workflows ausmacht: N Agents mit identischem Systemprompt und Kontext. Starten sie gleichzeitig, findet keiner einen Cache vor und alle zahlen den vollen Präfix-Preis; ein minimaler Versatz macht aus N teuren Writes einen Write und N−1 billige Reads. Bei großen Fan-outs ist das der Unterschied zwischen einem Bruchteil und dem Vielfachen der Präfix-Kosten.
- **Version:** v2.1.229

### [„Prompt is too long"-Fehler erklärt jetzt, warum die automatische Kompaktierung nicht half]
- **Was:** Die Meldung **„prompt is too long"** erklärt jetzt, **warum die automatische Kompaktierung den Kontext nicht retten konnte**, statt nur pauschal `/compact` vorzuschlagen.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der alte Hinweis war irreführend: Er empfahl genau das, was gerade automatisch versucht und gescheitert war — man tippte `/compact`, bekam denselben Fehler und stand ratlos da. Die neue Meldung nennt die tatsächliche Ursache (Ergänzung zum 32-MB-Fix oben) und damit den passenden Ausweg.
- **Version:** v2.1.229

### [Sandbox: IPv6-Literale in eckigen Klammern, mehrdeutige Schreibweisen fail-closed und im `/doctor` markiert]
- **Was:** Zwei Sandbox-Verbesserungen: **IPv6-Literale** in Netzwerk-Domain-Listen werden jetzt **in eckige Klammern gesetzt** (z. B. `[::1]:443`), und **mehrdeutige Schreibweisen** werden **fail-closed durchgesetzt** — also im Zweifel blockiert — sowie von **`/doctor`** angezeigt.
- **Einsatz:** IPv6-Einträge als `[::1]:443` schreiben; `/doctor` zeigt beanstandete Einträge.
- **Mehrwert:** Sicherheitsrelevante Konfiguration muss eindeutig sein. Eine mehrdeutig geschriebene IPv6-Adresse konnte bisher anders interpretiert werden als gemeint — im schlechtesten Fall wurde ein Ziel erlaubt, das man ausschließen wollte. Fail-closed dreht die Richtung des Irrtums um: Unklares wird blockiert statt durchgelassen, und `/doctor` zeigt, wo man nachbessern muss, statt es stillschweigend anders auszulegen.
- **Version:** v2.1.229

### [`/login` wiederholt die Warnung zum `CLAUDE_CODE_OAUTH_TOKEN`-Override nach erfolgreichem Login]
- **Was:** **`/login`** gibt die Warnung, dass **`CLAUDE_CODE_OAUTH_TOKEN`** die Anmeldung überschreibt, jetzt **auch nach erfolgreichem Login** noch einmal aus.
- **Einsatz:** Automatisch aktiv bei gesetzter `CLAUDE_CODE_OAUTH_TOKEN`-Umgebungsvariable.
- **Mehrwert:** Eine der verwirrendsten Situationen überhaupt: Man loggt sich erfolgreich als Person A ein und arbeitet danach trotzdem unter dem Token von Person B (oder einem Service-Account) weiter — inklusive falscher Abrechnung und falscher Limits. Die Warnung am Ende steht dort, wo man sie liest, statt vor einem mehrschrittigen Browser-Flow, der sie aus dem Blick schiebt.
- **Version:** v2.1.229

### [`/commit-push-pr` genehmigt gefährliche git/gh-Flags nicht mehr automatisch]
- **Was:** In **`/commit-push-pr`** werden `git`- und `gh`-Kommandos mit **gefährlichen Flags** (`--force`, `--amend`, `--no-verify` u. a.) **nicht mehr automatisch genehmigt**.
- **Einsatz:** Automatisch aktiv; betroffene Kommandos lösen jetzt eine Rückfrage aus.
- **Mehrwert:** Ein Convenience-Kommando hatte damit faktisch eine Blanko-Vollmacht für die drei git-Operationen, die man am wenigsten unbeaufsichtigt laufen lassen möchte: `--force` überschreibt fremde Commits auf dem Remote, `--amend` schreibt Historie um, `--no-verify` umgeht die Pre-Commit-Hooks, die genau das verhindern sollen. Die Auto-Genehmigung bleibt für den harmlosen Normalfall, endet aber vor den irreversiblen Aktionen.
- **Version:** v2.1.229

### [VS Code: Session-Gruppen in der Seitenleiste, größenverstellbares `/btw`-Panel, echter Feedback-Dialog]
- **Was:** Drei Verbesserungen der **VS-Code-Extension**: **Session-Gruppen** in der Seitenleiste (Rechtsklick zum Anlegen, Umbenennen, Löschen; Cmd/Ctrl- oder Shift-Klick verschiebt mehrere Sessions auf einmal); das **`/btw`-Seitenfragen-Panel** lässt sich jetzt **an seiner Kante in der Größe ziehen** — sowohl seitlich angedockt als auch gestapelt; und **„Report a problem"** bzw. **`/bug`** öffnen den **eingebauten Feedback-Dialog** statt eines abgeschalteten Umfrage-Links.
- **Einsatz:** VS-Code-Extension; Session-Gruppen per Rechtsklick in der Seitenleiste, `/btw` für Seitenfragen, `/bug` für Feedback.
- **Mehrwert:** Session-Gruppen sind das eigentliche Feature: Wer parallel an mehreren Aufgaben oder Repos arbeitet, hatte bisher eine flache, schnell unübersichtliche Liste — jetzt lässt sie sich nach Projekt oder Thema ordnen, inklusive Mehrfachauswahl. Der `/bug`-Fix ist nebenbei wichtig: Fehlermeldungen liefen bislang ins Leere.
- **Version:** v2.1.229

---

### Woche 33 (11.–12. August 2026) — Blog- & Platform-Ankündigungen

---

### [Claude in Chrome: Die Seitenleiste ist jetzt eine Claude-Cowork-Session]
- **Was:** Die **Seitenleiste von Claude in Chrome** führt jetzt **dieselbe Claude-Cowork-Session** wie Desktop, Web und Mobile — statt eines eigenständigen Browser-Chats. Konsequenzen: **Konversationen landen im Verlauf**, **Skills und Connectors funktionieren im Browser**, und eine **im Tab begonnene Aufgabe lässt sich in der Desktop-, Web- oder Mobile-App weiterführen**, weil die Session am **Account** hängt und nicht am Gerät. Verfügbar ab sofort auf **Max und Team**, Rollout auf **Pro** in den kommenden Wochen.
- **Einsatz:** Claude-in-Chrome-Seitenleiste öffnen — die Session erscheint anschließend im Verlauf und auf den anderen Oberflächen.
- **Mehrwert:** Beendet die Sonderrolle des Browser-Chats: Bisher war die Seitenleiste ein Nebenschauplatz ohne Verlauf, ohne die eigenen Skills und ohne Anschluss an die übrige Arbeit — brauchbar für eine schnelle Frage zur Seite, nicht für mehrstufige Vorgänge. Das Beispiel aus der Ankündigung zeigt den Gewinn: Rechnungen aus mehreren Lieferantenportalen einsammeln (dort kann nur der Browser hin), dann in der Desktop-App lokale Dateien ergänzen — eine Session über beide Welten hinweg. Für Claude-Code-Nutzer ist vor allem relevant, dass **Skills und Connectors** jetzt auch im Browser greifen.
- **Version:** Keine CLI-Version — Claude-Cowork/Chrome-Feature (Blog-Announcement vom 12.08.2026; Max/Team sofort, Pro in den kommenden Wochen)

### [Response-Header `anthropic-workspace-id` zeigt den aufgelösten Workspace]
- **Was:** Die **Claude API** liefert jetzt den Response-Header **`anthropic-workspace-id`**. Er trägt die **`wrkspc_`-präfixierte ID des Workspace**, auf den der API-Key oder das Access-Token der Anfrage aufgelöst wurde — **einschließlich des Default Workspace** der Organisation.
- **Einsatz:** Header `anthropic-workspace-id` aus der API-Antwort auslesen (z. B. für Logging/Debugging); Doku: `platform.claude.com/docs/en/manage-claude/workspaces#identify-the-workspace-behind-an-api-response`.
- **Mehrwert:** Bisher war von außen nicht feststellbar, welchem Workspace ein Request tatsächlich zugerechnet wird — man schloss vom Key auf den Workspace und lag bei rotierten oder verwechselten Keys falsch. Genau das ist die häufigste Ursache für „warum taucht dieser Verbrauch im falschen Kostenblock auf?" und für Limits, die scheinbar grundlos greifen. Wer den Header mitloggt, kann Kosten- und Rate-Limit-Zuordnung künftig belegen statt vermuten.
- **Version:** Keine CLI-Version — Claude-API-Feature (Platform-Release-Note vom 11.08.2026)

---

### Woche 33 (11. August 2026) — v2.1.228

---

### [Fix: Session-Cleanup löschte Inhalte im Memory-Ordner eines Projekts]
- **Was:** Die automatische Session-Bereinigung konnte **Dateien innerhalb des Memory-Ordners eines Projekts** mitlöschen — also genau die Ablage, die Erkenntnisse über Sessions hinweg aufbewahren soll.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der gravierendste Punkt des Releases: Das persistente Projekt-Gedächtnis ist per Definition das, was ein Cleanup **überleben** muss — wurde es stattdessen weggeräumt, verlor Claude still angesammelte Fakten, Präferenzen und Lektionen, und der Verlust fiel erst auf, wenn eine spätere Session „vergesslich" wirkte. Wer Memory aktiv nutzt, sollte dieses Update priorisieren (und prüfen, ob ältere Einträge fehlen).
- **Version:** v2.1.228

### [Härtung: Von claude.ai synchronisierte Skills werden entschärft]
- **Was:** Skills, die von **claude.ai auf den eigenen Rechner synchronisiert** werden, sind jetzt mehrfach eingehegt: Sie können **lokale Befehle und MCP-Prompts nicht mehr überschatten** (gleichnamige Einträge verdrängen), ihre **Beschreibungen werden bereinigt und als synchronisiert gekennzeichnet**, und ihre Bodies führen auf der eigenen Maschine **keine `!`-Befehle** mehr aus und **expandieren keine `@`-Dateien**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Schließt einen echten Vertrauenspfad-Bruch: Ein synchronisierter Skill konnte bisher unter dem Namen eines vertrauten lokalen Kommandos auftreten und beim Aufruf Shell-Befehle ausführen oder lokale Dateien in den Kontext ziehen — mit den Rechten der lokalen Session. Jetzt gilt: Was von claude.ai kommt, ist sichtbar markiert, kann nichts Lokales verdrängen und bleibt auf deklaratives Verhalten beschränkt.
- **Version:** v2.1.228

### [Write-Tool: Neuere Modelle dürfen Dateien überschreiben, die sie in der Session nicht gelesen haben]
- **Was:** Das **Write-Tool** verlangt bei **neueren Modellen** nicht mehr, dass eine bestehende Datei zuerst in der Session gelesen wurde, bevor sie überschrieben werden darf — dieselbe Regel, die beim Edit-Tool bereits gilt. Ältere Modelle behalten die Read-first-Pflicht.
- **Einsatz:** Automatisch aktiv; abhängig vom verwendeten Modell.
- **Mehrwert:** Eine bewusste Verhaltensänderung, die man kennen sollte: Weniger erzwungene Lese-Roundtrips bei generierten Dateien (Reports, Configs, Checkpoints), die ohnehin komplett ersetzt werden — aber auch eine Schutzstufe weniger gegen versehentliches Plattmachen einer Datei, deren Inhalt das Modell nie gesehen hat. Wer das in Automatisierungen kritisch findet, kann Write-Zugriffe weiterhin über Permission-Rules einschränken.
- **Version:** v2.1.228

### [Fix: Interaktive Session hörte komplett auf zu zeichnen, obwohl der Prozess weiterlief]
- **Was:** Nach einem **seltenen internen Layout-Fehler** konnte die interaktive Oberfläche **dauerhaft aufhören, sich neu zu zeichnen** — der Prozess arbeitete im Hintergrund weiter, das Terminal wirkte aber eingefroren.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Das Fehlerbild war ein scheinbar toter Bildschirm bei laufender (und Tokens verbrauchender) Session — die meisten Betroffenen dürften mit Ctrl-C abgebrochen und die Arbeit verloren haben, obwohl Claude noch mitten im Turn war. Jetzt überlebt die Darstellung den Layout-Fehler.
- **Version:** v2.1.228

### [Fix: Windows fand `git`/Git Bash nicht, wenn Claude Code aus einem Elternordner der Git-Installation startete]
- **Was:** Wurde Claude Code unter **Windows** aus einem Verzeichnis gestartet, das ein **Elternordner der Git-Installation** ist, wurden `git` und Git Bash nicht gefunden.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Betraf ausgerechnet naheliegende Setups wie `C:\` oder ein Werkzeug-Stammverzeichnis, unter dem auch Git liegt — dort fehlten dann sämtliche Git-Operationen und der Bash-Fallback. Die Erkennung hängt jetzt nicht mehr davon ab, von wo aus man startet.
- **Version:** v2.1.228

### [Fix: `/tui` setzte die Session auf ein früheres Modell zurück]
- **Was:** Wurde das Modell seit der letzten Antwort per **`/model`** gewechselt, fiel die Session beim Oberflächenwechsel über **`/tui`** auf das **vorherige Modell** zurück.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der zweite `/tui`-Zustands-Fix in Folge (v2.1.227 behob das Zurückholen zurückgespulter Konversationen): Ein bewusst gesetzter Modellwechsel — etwa hoch auf Opus für eine schwere Aufgabe — wurde vom Interface-Wechsel still kassiert, und man arbeitete unbemerkt auf dem falschen (und womöglich schwächeren) Modell weiter.
- **Version:** v2.1.228

### [Cross-Session-Messaging: fehlende Inbox nach Install/Upgrade behoben, Nachrichten erscheinen inline]
- **Was:** Zwei Verbesserungen am Session-zu-Session-Messaging: **Erstens** startete die **erste Session nach einer Installation oder einem Upgrade** manchmal **ohne Inbox** — eingehende Nachrichten konnten sie nicht erreichen. **Zweitens** werden **Absender und Nachrichtentext jetzt inline** angezeigt statt als eingeklappte Zeile; Nachrichten an Remote-Control-Sessions auf anderen Maschinen zeigen den eigenen Remote-Control-Session-Namen als Absender.
- **Einsatz:** Automatisch aktiv; senden weiterhin per `SendMessage`/`ListAgents`.
- **Mehrwert:** Der Inbox-Fix beseitigt den tückischsten Ausfallmodus des Features — ausgerechnet direkt nach dem Update, wenn man es ausprobieren will, kam nichts an, ohne Fehlermeldung auf Empfängerseite. Die Inline-Darstellung macht den Kanal alltagstauglich: Man sieht sofort, wer was schickt, statt jede Nachricht erst aufzuklappen.
- **Version:** v2.1.228

### [Fix: Remote-Control-`/resume` leakte Titel und Historie in die verbundene Session]
- **Was:** Wurde während einer bestehenden **Remote-Control-Verbindung** per `/resume` eine andere Konversation aufgenommen, konnten deren **Titel oder Verlauf in die verbundene Session durchsickern**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Ein Privacy-Fix: Die wiederaufgenommene Konversation kann zu einem völlig anderen Projekt oder Kontext gehören — deren Inhalte haben in der Session auf dem anderen Gerät nichts verloren. Gerade wer Remote Control geräteübergreifend zwischen beruflichen und privaten Kontexten nutzt, bekam hier Vermischung statt Trennung.
- **Version:** v2.1.228

### [Self-hosted Runner: fehlgeschlagener `checkout`-Hook blockiert nicht mehr alles, keine Abbrüche in der Task-Lücke]
- **Was:** Zwei Stabilitäts-Fixes für `claude self-hosted-runner`: **Erstens** ließ ein fehlschlagender **`checkout`-Hook** für ein Repository, in das die Session gar nicht pusht, **jede Session auf jedem frischen Runner** scheitern — das betroffene Repo wird jetzt mit Warnung übersprungen. **Zweitens** beendeten Runner Sessions fälschlich **in der Lücke zwischen dem Ende eines Background-Tasks und dem Start des Folge-Turns**.
- **Einsatz:** Automatisch aktiv auf Self-hosted Runnern (Team/Enterprise).
- **Mehrwert:** Beide Fehler trafen die Zuverlässigkeit des noch jungen Features an der Wurzel: Ein einziges kaputtes Neben-Repo machte die ganze Flotte unbrauchbar, und der Task-Lücken-Abbruch beendete Sessions genau dann, wenn Hintergrundarbeit fertig war und es weitergehen sollte. Nach dem `--base-dir`-Fail-fast aus v2.1.225 die nächste Betriebs-Härtungsrunde — wer Self-hosted Environments pilotiert, sollte die Runner auf 2.1.228 heben.
- **Version:** v2.1.228

### [Fix: Plugin-Cache-Cleanup löschte den Cache symlinkter Entwicklungs-Checkouts]
- **Was:** Die **Hintergrund-Bereinigung des Plugin-Caches** löschte den Cache eines Plugins auch dann, wenn dessen **einzige Version ein symlinkter Entwicklungs-Checkout** ist.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Traf Plugin-Entwickler: Wer sein Plugin lokal per Symlink einbindet, verlor durch die Aufräumroutine wiederholt den Cache-Zustand des eigenen Arbeitsstands. Lokale Entwicklungs-Setups überstehen das Cleanup jetzt.
- **Version:** v2.1.228

### [Fix: Marketplace-Eintrag erbte beim Settings-Merge fremde Custom-Header]
- **Was:** Wurde ein **Marketplace-Eintrag in einer höherrangigen Settings-Ebene neu definiert**, konnte er beim Zusammenführen die **Custom-Header einer anderen Ebene erben**. Marketplace-Einträge werden jetzt **als ganze Einheit** gemerged statt feldweise.
- **Einsatz:** Automatisch aktiv; betrifft Setups mit Marketplace-Definitionen in mehreren Settings-Ebenen (User/Projekt/Managed).
- **Mehrwert:** Custom-Header tragen häufig Auth-Tokens — ein feldweiser Merge konnte so Credentials einer Ebene an die Definition einer anderen heften und Requests mit dem falschen Kontext (oder an den falschen Endpunkt mit echten Tokens) losschicken. Whole-entry-Merge macht das Verhalten vorhersagbar: Wer einen Eintrag überschreibt, überschreibt ihn ganz.
- **Version:** v2.1.228

### [Fix: Deferred-Tools-Reminder ging nach Skill-Aufruf doppelt ans Modell]
- **Was:** Nach einer Skill-Invocation wurde der Hinweis auf per Tool-Search nachladbare Tools dem Modell **gelegentlich zweimal** gesendet.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Doppelte System-Reminder kosten Kontext-Tokens und können das Modell zu redundanten Tool-Nachladungen verleiten — unsichtbar für den Nutzer, aber messbar in Sessions, die viele Skills aufrufen.
- **Version:** v2.1.228

### [Vertex AI: abgelaufene Google-Cloud-Credentials scheitern in Sekunden statt Minuten]
- **Was:** Fehlen die **Google-Cloud-Credentials** für Vertex AI oder sind sie abgelaufen, schlägt der Request jetzt **innerhalb von Sekunden** fehl, statt minutenlang Retries zu fahren.
- **Einsatz:** Automatisch aktiv bei Vertex-AI-Nutzung (`CLAUDE_CODE_USE_VERTEX`).
- **Mehrwert:** Ein abgelaufenes `gcloud auth`-Token ist im Vertex-Alltag der häufigste Startfehler — bisher saß man minutenlang vor einer scheinbar hängenden Session, bevor die eigentliche Ursache sichtbar wurde. Fail-fast heißt: Fehlermeldung lesen, `gcloud auth login`, weiterarbeiten.
- **Version:** v2.1.228

### [Compaction zeigt Retry-Countdown und Stall-Hinweis]
- **Was:** Während der **Kontext-Kompaktierung** erscheinen jetzt auch der **Retry-Countdown** und der **Stall-Hinweis** — bisher gab es dort nur einen Fortschrittsbalken.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Compaction ist einer der wenigen Momente, in denen Claude Code länger blockiert — ohne Statusdetails war nicht unterscheidbar, ob gerade gearbeitet, gewartet oder gehangen wird. Jetzt sieht man, ob ein Retry läuft und wann er feuert, statt im Zweifel eine gesunde Kompaktierung abzubrechen.
- **Version:** v2.1.228

### [Kleinere Änderungen: ruhigere Spinner-Glyphen im Terminal-Titel, veralteter Auto-Mode-Kostenhinweis entfernt]
- **Was:** Die **Busy-Spinner-Glyphen im Terminal-Titel** wurden getauscht, um Zittern in der Tab-Leiste mancher Terminals zu reduzieren. Außerdem wurde aus dem First-Use-Hinweis zum Auto-Mode auf Pro/Max/Team die **veraltete Aussage entfernt, Auto-Mode-Sessions kosteten etwas mehr**.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Der zweite Punkt ist die konsequente Aufräumarbeit zur bereits dokumentierten Änderung, dass Klassifizierer-Aufrufe des Auto-Mode nicht mehr gegen die Nutzungslimits zählen — der Hinweis widersprach sonst ab dem Standard-Rollout am 14.08. der Realität.
- **Version:** v2.1.228

---

### Woche 33 (10.–11. August 2026) — Blog- & Platform-Ankündigungen

---

### [Compliance API liefert jetzt Transkripte lokaler Claude-Code- und Cowork-Sessions]
- **Was:** Die **Compliance API** gibt in **Beta** (Claude-Enterprise-Organisationen) jetzt auch Sessions zurück, die **auf den Rechnern der Nutzer** laufen — also lokale Claude-Code- und Cowork-Sessions, nicht nur Cloud-seitige. Drei neue Endpunkte: `GET /v1/compliance/apps/sessions/local` listet die Sessions der Organisation, `…/local/{session_id}` liefert die Metadaten einer Session, `…/local/{session_id}/messages` deren vollständiges Transkript. Authentifizierung über den vorhandenen Compliance Access Key mit dem Scope `read:compliance_user_data`.
- **Einsatz:** `GET /v1/compliance/apps/sessions/local` mit bestehendem Compliance Access Key (`read:compliance_user_data`) — Doku: `platform.claude.com/docs/en/manage-claude/compliance-content-data#retrieve-local-sessions`.
- **Mehrwert:** Bisher endete die Compliance-Sicht an der Cloud-Grenze — was Mitarbeitende in lokalen CLI- oder Cowork-Sessions taten, war für Audit, Records-Management und eDiscovery unsichtbar. Für regulierte Branchen fällt damit eines der letzten Argumente weg, Claude Code auf Firmenrechnern zu sperren; umgekehrt sollten Nutzer in Enterprise-Orgs wissen, dass ihre lokalen Session-Transkripte jetzt organisationsweit abrufbar sind.
- **Version:** Keine CLI-Version — Platform-Feature (Beta, Claude Enterprise; Blog-Announcement + Platform-Release-Note vom 11.08.2026)

### [Sonnet 5: Einführungspreis \$2/\$10 pro MTok wird zum Dauerpreis — geplante Erhöhung entfällt]
- **Was:** Der Einführungspreis von **Claude Sonnet 5** — **\$2 Input / \$10 Output pro Million Tokens** — ist jetzt der reguläre Standardpreis. Die ursprünglich für den **1. September 2026** angekündigte Erhöhung auf \$3/\$15 pro MTok **findet nicht statt**.
- **Einsatz:** Automatisch aktiv — betrifft die API-Abrechnung von `claude-sonnet-5`; keine Änderung an Code oder Konfiguration nötig.
- **Mehrwert:** Planungssicherheit für alle, die Sonnet 5 produktiv fahren: Wer Budgets oder Preiskalkulationen bereits mit \$3/\$15 ab September gerechnet hatte, kann sie um ein Drittel nach unten korrigieren. Zusammen mit dem 1M-Token-Kontext bleibt Sonnet 5 damit dauerhaft der Preis-Leistungs-Anker der Claude-5-Familie — auch für Subagent-Flotten, deren Kosten fast ganz an den Sonnet-Tarifen hängen.
- **Version:** Keine CLI-Version — Preisänderung (Platform-Release-Note vom 10.08.2026)

---

### Woche 33 (10. August 2026) — v2.1.227

---

### [Fix: Abgelaufenes Login-Token führte zur falschen Fable-Credits-Aufforderung bei Max-Nutzern]
- **Was:** Startete eine Session mit einem **abgelaufenen Login-Token**, wurden die Feature-Flags ohne die Subscription-Stufe des Nutzers ausgewertet. Folge: **Max-Plan-Nutzer** wurden fälschlich aufgefordert, **Usage-Credits für Fable** zu aktivieren — obwohl ihr Plan das gar nicht verlangt.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Die Aufforderung war maximal verwirrend, weil sie genau die Nutzer traf, deren Plan Fable bereits abdeckt — wer sie befolgte, schaltete unnötig ein kostenpflichtiges Credit-Modell frei. Der Fix sorgt dafür, dass die Tier-Erkennung erst nach dem Token-Refresh greift, statt aus einem leeren Zustand zu raten.
- **Version:** v2.1.227

### [Fix: `claude-code-action` mit `allowed_non_write_users` ließ auf GitHub-Runnern jeden Bash-Befehl scheitern]
- **Was:** Lief Claude Code über die **`claude-code-action`** mit gesetztem **`allowed_non_write_users`** auf einem **GitHub-hosted Runner**, schlug **jeder einzelne Bash-Befehl** fehl.
- **Einsatz:** Automatisch aktiv; betrifft GitHub-Actions-Workflows mit `allowed_non_write_users`.
- **Mehrwert:** Das ist der Totalausfall-Fall in CI: Die Option `allowed_non_write_users` ist genau dafür da, externe Beitragende ohne Schreibrechte an Claude heranzulassen — und machte den Job dabei komplett arbeitsunfähig, weil ohne Bash weder Tests noch Builds noch git liefen. Wer Claude in Pull-Request-Workflows für Fremdbeiträge einsetzt, sollte direkt auf v2.1.227 gehen.
- **Version:** v2.1.227

### [Fix: `/tui` holte eine zurückgespulte Konversation wieder hervor]
- **Was:** Wurde eine Konversation per Rewind **vor ihre erste Nachricht** zurückgespult, brachte ein anschließender Wechsel über **`/tui`** genau diese verworfene Historie zurück.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Rewind bis vor den Anfang ist die Geste für „das war der falsche Weg, ich fange neu an" — und ausgerechnet der Oberflächenwechsel machte sie rückgängig. Statt eines leeren Starts bekam man den gerade bewusst entsorgten Kontext (samt dessen Token-Kosten und Fehlannahmen) wieder untergeschoben.
- **Version:** v2.1.227

### [Slash-Command-Menü: ruhigere Hervorhebung, Emoji- und Akzent-Namen bleiben intakt]
- **Was:** Das Menü der Slash-Befehle wurde überarbeitet: **Blau markiert nur noch die ausgewählte Zeile** (statt breitflächig zu färben), die **Treffer-Zeichen der Eingabe werden fett** statt umgefärbt, und Befehlsnamen mit **Emoji oder Akzentzeichen** behalten ihre Glyphen, statt zerlegt zu werden.
- **Einsatz:** Automatisch aktiv — `/` im Prompt tippen.
- **Mehrwert:** Zwei Farbsignale um dieselbe Bedeutung („ausgewählt" und „passt zur Eingabe") konkurrierten bisher miteinander; mit Fettung für den Match und Blau nur für die Auswahl ist auf einen Blick klar, welche Zeile gerade Enter treffen würde. Für Plugin- und Team-Befehle mit Emoji-Präfix — eine verbreitete Konvention, um eigene Kommandos von den eingebauten abzuheben — ist der Glyphen-Fix der eigentliche Gewinn: Die Namen waren dort schlicht verstümmelt.
- **Version:** v2.1.227

### [Performance: weniger Event-Loop-Stalls bei Datei-Vorschlägen und At-Mentions]
- **Was:** Zwei Stellen, an denen die Oberfläche kurz einfror, sind entschärft: die **Vorschläge nach einem „file not found"** und die **Größenprüfung bei At-Mentions** (`@datei`) blockieren die Event-Loop nicht mehr.
- **Einsatz:** Automatisch aktiv.
- **Mehrwert:** Beides sind Tipp-Pfade, keine Hintergrundjobs — ein Stall trifft dort direkt die Eingabe und wirkt wie ein hängendes Terminal. Besonders in großen Repos, wo sowohl die Datei-Suche als auch die Größenermittlung viel Arbeit bedeuten, bleibt der Prompt jetzt responsiv.
- **Version:** v2.1.227

---

### Woche 33 (11. August 2026) — What's-New-Digests Week 30 & Week 32 nachveröffentlicht

---

### [Auto-Mode wird am 14. August 2026 zum Standard-Permission-Modus auf Pro, Max und Team]
- **Was:** Ab dem **14. August 2026** startet jede **neue** Session auf **Pro-, Max- und Team-Plänen** im **Auto-Mode** — dem Modus, in dem ein Klassifizierer die Permission-Prüfung übernimmt, statt für jede Aktion einen Dialog zu zeigen. Wer selbst einen Default-Modus gesetzt hat, behält ihn: Die Umstellung kommt dort nur als **einmalige Rückfrage**, die man annehmen muss. Ein von der **Organisation verwalteter** Default ändert sich gar nicht. Der Modus lässt sich jederzeit weiterhin umschalten.
- **Einsatz:** Vorab selbst festlegen in `~/.claude/settings.json` — `{"permissions": {"defaultMode": "auto"}}` (neue Sessions zeigen dann `auto mode on` in der Statuszeile). Wer den bisherigen Modus behalten will, setzt ihn dort ebenso explizit, statt sich auf den Default zu verlassen.
- **Mehrwert:** Das ist die folgenreichste Verhaltensänderung seit Monaten, und sie kommt ohne Update — sie greift am Stichtag serverseitig. Wer bisher nie einen Default gesetzt hat, arbeitet ab dem 14.08. in einem grundlegend anderen Prompt-Regime; wer das nicht will, sollte den gewünschten Modus **vor** dem Datum explizit in die Settings schreiben. Für Teams ist der Merksatz: Ein managed Default ist die einzige Variante, die garantiert unangetastet bleibt.
- **Version:** Keine CLI-Version — plangebundener Rollout ab 14.08.2026 (angekündigt im What's-New-Digest Week 32)

### [Auto-Mode-Klassifizierer zählt nicht mehr gegen die Nutzungslimits]
- **Was:** Die Modell-Aufrufe, mit denen der **Auto-Mode-Klassifizierer** Aktionen bewertet, werden auf **Pro-, Max- und Team-Plänen** nicht mehr auf die Nutzungslimits angerechnet. Laut Digest ist das **bereits in Kraft**, nicht erst mit dem Stichtag 14.08.
- **Einsatz:** Automatisch aktiv im Auto-Mode.
- **Mehrwert:** Nimmt dem Auto-Mode das letzte handfeste Gegenargument. Bisher war jede Permission-Entscheidung ein zusätzlicher Modell-Call auf das eigene Kontingent — ein spürbarer Aufschlag gerade bei werkzeugintensiven Sessions, und genau der Grund, aus dem viele beim manuellen Prompting geblieben sind. Der Sicherheitsgewinn ist damit gratis, und die Standard-Umstellung am 14.08. ergibt auch abrechnungsseitig Sinn.
- **Version:** Keine CLI-Version — plangebundene Änderung (dokumentiert im What's-New-Digest Week 32)

### [What's-New-Digests Week 30 und Week 32 nachveröffentlicht — Week 31 fehlt weiterhin]
- **Was:** Die offiziellen Wochen-Digests **Week 30** (20.–24.07., v2.1.214–v2.1.219) und **Week 32** (03.–07.08., v2.1.220–v2.1.224) sind jetzt live; beide lieferten seit dem 25.07. bei jedem Crawl noch 404. Week 30 fasst **Opus 5** als neues Default-Opus-Modell, den **iOS-Simulator-Bereich in Claude Code Desktop** und das **Claude-Security-Plugin** zusammen; Week 32 stellt **Cross-Session-Messaging**, **Self-hosted Environments** und die **Auto-Mode-Umstellung** in den Mittelpunkt. **Week 31** (27.–31.07.) bleibt unveröffentlicht — in dieser Woche gab es allerdings auch keinen einzigen CLI-Release (Lücke zwischen v2.1.220 vom 25.07. und v2.1.221 vom 04.08.), die Lücke in der Digest-Reihe passt also zur Release-Historie.
- **Einsatz:** `code.claude.com/docs/en/whats-new/2026-w30` bzw. `…/2026-w32`.
- **Mehrwert:** Bis auf die beiden oben ergänzten Auto-Mode-Punkte enthalten die Digests **nichts, was hier nicht schon stünde** — sie sind die redaktionelle Rückschau auf Releases, die diese Datei bereits einzeln dokumentiert hat. Der praktische Wert liegt in der Gewichtung: Die Digests sagen, welche drei Features Anthropic selbst für die folgenreichsten der Woche hält, und sie liefern zu jedem einen lauffähigen Einstiegs-Befehl. Für künftige Crawls ist die Lehre, dass Digest-Seiten mit **mehreren Wochen Verzug** erscheinen können — ein 404 heißt „noch nicht", nicht „kommt nicht".
- **Version:** Keine CLI-Version — Dokumentations-Nachtrag zu v2.1.214–v2.1.224
...
### [Session-Budgets für Managed Agents — harte Ausgabengrenze mit Stop-Reason `budget_reached`]
- **Was:** Eine Claude-Managed-Agents-Session lässt sich jetzt mit einem **Budget** deckeln: eine harte Obergrenze für die Ausgaben der Session, abgerechnet zu den öffentlichen Listenpreisen. Erreicht eine Session ihr Budget, **pausiert** sie mit dem neuen Stop-Reason `budget_reached`, statt weitere Modell-Requests zu starten; wird das Budget erhöht oder entfernt, läuft sie weiter. Deployments akzeptieren dasselbe Budget und legen es auf jede Session, die sie starten.
- **Einsatz:** Budget beim Anlegen der Session (bzw. am Deployment) setzen — Doku: `platform.claude.com/docs/en/managed-agents/budgets`. Auf `stop_reason: "budget_reached"` reagieren, um Sessions kontrolliert fortzusetzen oder zu beenden.
- **Mehrwert:** Ein durchgedrehter Agent kann nicht mehr unbemerkt Kosten produzieren — statt Nachrechnen im Billing gibt es einen Hard-Stop, den man bewusst wieder aufmacht. Für automatisierte Deployments (Cron, Webhooks) ist das die fehlende Sicherung gegen Endlos-Loops.
- **Version:** Platform-Release-Notes 07.08.2026 (Claude API) — keine CLI-Version

### [Advisor für Managed-Agents-Sessions — ein stärkeres Modell mitten im Turn befragen]
- **Was:** Der primäre Thread einer Managed-Agents-Session kann jetzt **mitten im Turn** einen *Advisor* konsultieren: ein Modell, das mindestens so leistungsfähig ist wie das des Agents selbst, für strategische Rückfragen. Konfiguriert wird er als Eintrag `{"type": "advisor"}` im Multiagent-Roster des Agents, mit dem zu befragenden `model`.
- **Einsatz:** Im Multiagent-Roster des Agents einen `{"type": "advisor"}`-Eintrag mit `model` anlegen — Doku: `platform.claude.com/docs/en/managed-agents/multiagent-orchestration#give-the-session-an-advisor`.
- **Mehrwert:** Man kann einen günstigen Agent für die Fleißarbeit fahren und trotzdem an den kniffligen Stellen ein starkes Modell befragen, ohne die ganze Session darauf umzustellen — Kosten-/Qualitäts-Abwägung auf Entscheidungsebene statt pauschal pro Session.
- **Version:** Platform-Release-Notes 07.08.2026 (Claude API) — keine CLI-Version

### [`inference_geo` — Inferenz-Region für Managed Agents selbst festlegen]
- **Was:** Neuer Schalter `inference_geo` innerhalb des `model`-Objekts: Damit bestimmt man, **wo** die Modell-Inferenz eines Managed-Agents-Agents läuft. Setzbar beim Anlegen des Agents oder als Override für eine einzelne Session; verfügbare Geos und Preise stehen in der Data-Residency-Doku.
- **Einsatz:** `inference_geo` im `model`-Objekt beim Agent-Create setzen bzw. pro Session überschreiben — Doku: `platform.claude.com/docs/en/manage-claude/data-residency`.
- **Mehrwert:** Data-Residency-Vorgaben (EU, US, …) lassen sich pro Agent — und bei Bedarf pro Session — durchsetzen, statt sie auf Account- oder Provider-Ebene festzuklopfen. Das Gegenstück zu `ANTHROPIC_BEDROCK_REGION_PREFIX` auf CLI-Seite.
- **Version:** Platform-Release-Notes 07.08.2026 (Claude API) — keine CLI-Version

### [Managed Agents laden Skills direkt aus dem gemounteten GitHub-Repo]
- **Was:** Mountet eine Managed-Agents-Session ein GitHub-Repository, werden **Skills aus dessen `.claude/skills`-Verzeichnis im Repo-Root beim Session-Start automatisch erkannt** und stehen dem Agent für diese Session zur Verfügung — ohne sie separat hochzuladen oder zu registrieren.
- **Einsatz:** Skills wie gewohnt unter `.claude/skills/` im Repo pflegen und das Repo in der Session mounten — Doku: `platform.claude.com/docs/en/managed-agents/skills#load-skills-from-a-github-repository`.
- **Mehrwert:** Dieselben Skills, die lokal in Claude Code greifen, gelten jetzt auch in Managed-Agents-Sessions — versioniert im Repo, per Pull Request reviewbar, ohne zweite Ablage. Skill-Drift zwischen lokaler und gehosteter Ausführung entfällt.
- **Version:** Platform-Release-Notes 07.08.2026 (Claude API) — keine CLI-Version

### [Claude Opus 4.1 ist retired — Requests liefern jetzt Fehler]
- **Was:** Das Modell **Claude Opus 4.1** (`claude-opus-4-1-20250805`) wurde zum 05.08.2026 abgeschaltet; alle Requests dagegen geben ab sofort einen Fehler zurück. Empfohlene Migration ist **Claude Opus 5** (nicht mehr Opus 4.8 wie in der ursprünglichen Deprecation-Ankündigung). Forschende können über das External Researcher Access Program weiterhin Zugang beantragen.
...
- **Einsatz:** Modell in der Dreams-Konfiguration auf `claude-opus-5` setzen — Doku: `platform.claude.com/docs/en/managed-agents/dreams#limits`.
- **Mehrwert:** Dreams-Läufe können auf dem stärksten verfügbaren Modell laufen, statt für anspruchsvolle Aufgaben auf die kleineren Varianten ausweichen zu müssen.
- **Version:** Platform-Release-Notes 01.08.2026 (Claude API) — keine CLI-Version
...
- [Platform Release Notes](https://platform.claude.com/docs/en/release-notes) — Achtung: Diese URL antwortet seit dem 05.09. mit HTTP 307 und leitet auf den Änderungsverlauf der System-Prompts um; die tatsächlichen Platform-Release-Notes stehen unter […/release-notes/overview.md](https://platform.claude.com/docs/en/release-notes/overview.md)
