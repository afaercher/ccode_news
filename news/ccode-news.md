...
> Letzte Aktualisierung: 2026-09-04 12:00 UTC (**Crawl 04.09. 12:00 UTC — Bestätigungslauf: keine Bewegung in allen vier Quellen.** CLI unverändert **v2.1.260** (npm-Publish 03.09. 22:32 UTC, GitHub-Release 23:48 UTC), **kein v2.1.261 bis 12:00 UTC**, `time.modified` weiter 03.09. 23:47 UTC, Tags still (`latest`=`next`=2.1.260, `stable` 2.1.236). Rhythmus: 12:00 UTC ist 05:00 Freitagmorgen Westküste — die Releases der Werktags-Serie seit 17.08. kamen fast durchweg nach 17:00 UTC (Mo 17:07, Di 17:15/22:25, Mi 21:21, Do 22:32), ein Freitags-Release wäre also frühestens beim 18:00-Lauf, sicher erst am 05.09. 06:00 sichtbar; der leere Vormittag ist kein Signal für ein Serienende. `CHANGELOG.md` unverändert: 2.1.260 -> 2.1.259 -> 2.1.258 -> 2.1.257 -> 2.1.252, die Releases-API listet dieselbe Folge (v2.1.249 und v2.1.253-256 bleiben ohne Publish). **What's-New** byte-identisch zum Vortagsabruf, Week 34 oben, `whats-new/2026-w35` und `-w36` weiterhin **HTTP 404** — der Digest-Rückstand bleibt bei zwei Wochen, Week 31 fehlt seit über fünf Wochen. **Platform-Release-Notes** byte-identisch, Top weiter 01.09. (Fable 5.1/Mythos 5.1, Per-Message-Effort). **Blog** unverändert, dieselben fünfzehn Slugs, Top weiter der Commerce-Agents-Beitrag vom 28.08., kein Beitrag vom 03. oder 04.09. Alle vier Quellen plus npm-Registry und Releases-API per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 0.** — Vorheriger **Crawl 04.09. 06:00 UTC — Donnerstag-Release nachgeholt: v2.1.260** (npm-Publish 03.09. 22:32 UTC, GitHub-Release 23:48 UTC, `CHANGELOG.md` deckungsgleich, **68 Punkte**: sechs neue Funktionen, 38 Fehlerbehebungen, ein Revert, neun Verbesserungen, acht Verhaltensänderungen, eine Entfernung, sieben VS-Code-Punkte). Kern: **`/diff`-Panel** im Fullscreen-Modus, **Ursache für Prompt-Cache-Misses** in `/cost`, **Revert der v2.1.259-Änderung** (`Read()`-Deny-Regeln auf Bash-Argumente) wegen Fehlalarmen bei `npm run build`, Permission-Fixes für Pfade mit Klammern und kaputte Regex-Muster, Fable-5.1-Fixes (`/model`-Picker, Prompt-Cache nach Tool-Results, `[1m]`-Pin), Auto-Compact kurz vor 1M. **Tags:** `latest`=`next`=**2.1.260**, `stable` weiter 2.1.236. Werktags-Serie seit 17.08. läuft (Mo–Do diese Woche je ein Release). **Platform unverändert** (Top 01.09.). **What's-New unverändert** Week 34 oben, Week 35/36 weiterhin 404. **Blog:** Detailabgleich nachgeholt — zwei bisher nicht erfasste Beiträge ohne Claude-Code-Bezug: „Claude for Commerce Agents" (28.08.) und „Claude for Teachers now available for schools and districts" (26.08.), als Sammel-Eintrag aufgenommen; Liste bei fünfzehn Slugs. Quellen: npm-Registry, GitHub-Releases-API, `CHANGELOG.md`, `whats-new.md`, `release-notes/overview.md`, Blog-HTML alle per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 8.** — Ältere Crawl-Historie in den Git-Commits.)
