...
> Letzte Aktualisierung: 2026-09-06 06:00 UTC (**Crawl 06.09. 06:00 UTC — Wochenend-Release: v2.1.263 ist da, v2.1.262 ist übersprungen.** Der Samstag ist nicht leer geblieben: **v2.1.263** wurde am **06.09. um 02:07:58 UTC** auf npm publiziert — das ist **Samstag, 19:07 Uhr US-Pazifikzeit**. Der 18:00-Lauf hatte genau dieses Abendfenster als einzige verbliebene Möglichkeit benannt (2.1.241 kam an einem Samstag um 16:58 PT); die Prognose ist eingetreten. Das GitHub-Release folgte **46 Minuten** später um **02:54:20 UTC** — deutlich schneller als die 2h09min bei v2.1.261. Die Tags stehen jetzt auf `latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**; `time.modified` **06.09. 02:53:41 UTC**; der Git-Ref `v2.1.263` antwortet HTTP **200**. Die Lücke zum Vorgänger betrug **32,3 Stunden**. **Der Inhalt ist allerdings nicht veröffentlicht:** `CHANGELOG.md` (630786 -> **630840 Bytes**, +54) enthält für 2.1.263 genau eine Zeile — „Bug fixes and reliability improvements“ — und das GitHub-Release ist wortgleich. Was in dieser Version steckt, ist von außen nicht feststellbar; der npm-Wrapper gibt nichts preis (7 Dateien, **182844 Bytes**, exakt wie v2.1.261, der Code liegt in den plattformspezifischen `optionalDependencies`). **Fund dieses Laufs — zwei Muster, die zusammengehören:** Erstens ist **v2.1.262 übersprungen** und bleibt es: keine npm-Version, kein `time`-Eintrag, kein Git-Tag (HTTP 404), kein Changelog-Abschnitt. Das ist kein Einzelfall — im Nummernraum **2.1.200 bis 2.1.263** (64 Nummern) sind nur **56 publiziert**, **acht fehlen** (230, 244, 249, 253, 254, 255, 256, 262), darunter ein Block von vier aufeinanderfolgenden. Übersprungene Nummern tauchen **nie nachträglich** auf, weder auf npm noch im Changelog. Praktische Folge: Aus einer Versionslücke lässt sich **nicht** schließen, dass ein Abruf etwas verpasst hat. Zweitens korrelieren die inhaltslosen Platzhalter-Releases mit der Uhrzeit: Von den **10 Versionen**, deren Changelog nur aus „Bug fixes and reliability improvements“ besteht (2.1.165, 167, 168, 190, 220, 226, 240, 241, 250, 263), wurden **acht nach 15:00 Uhr PT oder am Wochenende** publiziert, und **alle drei Wochenend-Publishes seit August** (2.1.240, 2.1.241, 2.1.263) sind Platzhalter. Ein Release außerhalb der Kernzeit bringt also mit hoher Wahrscheinlichkeit **keine dokumentierten Neuerungen** — für einen Crawler heißt das: Wochenend-Version einsammeln, aber keinen Feature-Ertrag erwarten. Dazu unten ein eigener Eintrag. **What's-New** byte-identisch (**14051 Bytes**), Week 34 weiterhin oben; `whats-new/2026-w35`, `-w36` und `-w37` antworten alle **HTTP 404** — der Digest-Rückstand bleibt bei rechnerisch drei Wochen. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`). **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der `Last Published`-Stempel steht den vierten Lauf in Folge auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 2** (v2.1.263 und das Muster aus Nummernlücken und Platzhaltern). — Vorheriger **Crawl 05.09. 18:00 UTC — dritter Lauf des Samstags, erneut keine Bewegung in allen vier Quellen.** Die CLI steht unverändert auf **v2.1.261** (npm-Publish 04.09. 17:49:34 UTC, GitHub-Release 19:58:10 UTC); ein **v2.1.262 gibt es auch um 18:00 UTC nicht** — der Git-Ref `v2.1.262` antwortet HTTP **404**, `time.modified` steht weiterhin auf 04.09. **19:57:25 UTC**, die Tags sind still (`latest` = `next` = **2.1.261**, `stable` **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den beiden Vorläufen (**630786 Bytes**), die Releases-API listet deckungsgleich v2.1.261 an der Spitze. **Rhythmus — der leere Samstag ist statistisch unauffällig:** Seit dem letzten Publish sind **24,2 Stunden** vergangen. Unter den 57 Publishes seit dem 01.07. gibt es 56 Abstände, und dieser liegt damit erst auf **Rang 20** — völlig gewöhnlich. Aussagekräftiger sind die Spitzen: Die drei größten Lücken **innerhalb** des normalen Betriebs sind allesamt Freitag→Montag und liegen bei **71–74 Stunden** (2.1.251 -> 2.1.252 mit 73,6 h, 2.1.201 -> 2.1.202 mit 71,8 h, 2.1.233 -> 2.1.234 mit 71,5 h); käme die nächste Version am Montagabend, wäre die Lücke rund 72 h und träfe das Muster exakt. Der Ausreißer bleibt der Sommerstillstand 2.1.220 -> 2.1.221 mit **239 h** (24.07. bis 03.08.). **Zum Zeitfenster:** 18:00 UTC ist 11:00 Uhr Samstagvormittag an der US-Westküste. Von den beiden echten Samstags-Publishes lag 2.1.240 bei **06:03 PT** — dieser Vormittagsslot ist heute ungenutzt verstrichen; 2.1.241 kam dagegen um **16:58 PT**, also 23:58 UTC. Ein Samstags-Release wäre ab jetzt nur noch in diesem Abendfenster möglich und damit erst beim **Lauf am Sonntag 06:00 UTC** sichtbar. **Fund dieses Laufs — der `stable`-Kanal hängt planmäßig zwei Wochen zurück:** Der Tag `stable` zeigt auf **2.1.236**, und diese Version wurde am **19.08. um 18:45 UTC** publiziert — sie ist heute **17 Tage alt**. Zwischen `stable` und `latest` liegen **19 publizierte Versionen** (2.1.237 bis 2.1.261). Der Zeiger selbst steht dabei nicht fest: Er ist am **03.09. von 2.1.231 auf 2.1.236 gesprungen** — von einer Version vom 13.08. auf eine vom 19.08., also ein Nachrücken um sechs Tage, das den Abstand zu `latest` gerade nicht verkleinert hat. `stable` ist damit erkennbar kein „letztes gutes Release“, sondern ein bewusst **rund zwei Wochen zurückgehaltener Kanal** — relevant für alle, die per `@stable` pinnen und sich fragen, warum ein am Vortag veröffentlichter Fix bei ihnen nicht ankommt. Dazu unten ein eigener Eintrag. **Platform-Release-Notes** byte-identisch (**103914 Bytes**), abgerufen über die korrekte Adresse `…/release-notes/overview.md` — die beim 12:00-Lauf gefundene 307-Weiterleitung der dokumentierten URL besteht fort, die Größenprüfung greift; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`). **What's-New** byte-identisch (14051 Bytes), Week 34 oben; `whats-new/2026-w35`, `-w36` und nun auch `-w37` antworten **HTTP 404** — der Digest-Rückstand wächst damit rechnerisch auf drei Wochen, Week 31 fehlt seit über fünf Wochen. **Blog** byte-identisch (763730 Bytes), dieselben fünfzehn Slugs in gleicher Reihenfolge, Top weiter „Building commerce agents with Claude“ (28.08.); der Webflow-Stempel `Last Published` steht den dritten Lauf in Folge auf **03.09. 17:24 UTC** — seit Donnerstag nicht einmal ein Republish. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (der `stable`-Kanal). — Ältere Crawl-Historie in den Git-Commits.)
