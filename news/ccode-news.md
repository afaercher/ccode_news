...
> Letzte Aktualisierung: 2026-09-08 06:00 UTC (**Crawl 08.09. 06:00 UTC — siebter Lauf in Folge ohne Bewegung; die Montags-Prognose ist widerlegt, und der Grund ist der Kalender.** Alle vier Quellen sind byte-identisch zum Vorlauf. Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es nicht** — die npm-Version antwortet HTTP **404**, ebenso die Git-Refs `v2.1.264` und `v2.1.265`; `time.modified` steht weiterhin auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den sechs Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze (60-Byte-Platzhalter-Body, `ashwin-ant`). `v2.1.262` bleibt HTTP **404**. Die Lücke seit dem letzten Publish beträgt **51,9 Stunden**. **Die Prognose der letzten drei Läufe ist damit widerlegt:** Das benannte Fenster (Montag 07.09., **17:00–22:30 UTC**) ist vollständig verstrichen, ohne dass etwas publiziert wurde — der ganze Montag blieb leer. **Fund dieses Laufs — an US-Feiertagen wird nicht ausgeliefert, und genau das hat die Prognose übersehen:** Der 07.09.2026 ist **Labor Day**, der erste Montag im September und ein US-Bundesfeiertag. Prüft man alle **20 auf einen Werktag fallenden US-Bundesfeiertage** seit Februar 2025 gegen die Publish-Historie, blieben **17 davon ohne einen einzigen Publish (85 %)** — gegenüber einer Grundrate von **20 leeren Tagen unter 178 Werktagen** des Jahres 2026 (**11 %**). Die längste Lücke der gesamten Historie, **336,5 Stunden** ab dem 22.12.2025, deckt exakt Weihnachten bis Neujahr ab. Nur drei Werktags-Feiertage hatten einen Publish (Labor Day 2025, Presidents Day 2026, der auf Freitag vorgezogene 4. Juli 2026). **Damit erklärt sich auch, warum die alte Prognose danebenlag: Sie beruhte auf Selektionsbias.** Sie hatte nur die Uhrzeiten jener Montage ausgewertet, an denen überhaupt publiziert wurde, und dabei übersehen, dass der **Montag der Werktag mit der höchsten Ausfallquote** ist — **7 von 36 Montagen** in 2026 ohne Publish (19 %) gegen **2 von 35** bei Dienstag und Mittwoch (6 %); **drei der sieben leeren Montage sind Feiertage** (MLK 19.01., Memorial Day 25.05., Labor Day 07.09.). **Neue Prognose für v2.1.264:** Nach einem leeren Feiertags-Montag kam der Anschluss dreimal am **Dienstag** (14.10.2025 10:18 PT, 20.01.2026 14:37 PT, 26.05.2026 14:38 PT), einmal am Mittwoch, einmal erst in der Folgewoche. Der erste Dienstags-Publish liegt seit 01.07. ausnahmslos zwischen **10:15 und 13:37 PT** (17:15–20:37 UTC), im Median 11:50 PT. Erwartung also: **Dienstag 08.09. zwischen etwa 17:00 und 21:00 UTC** — der 18:00-Lauf erwischt den Anfang des Fensters, verlässlich sichtbar wird die Version beim Lauf am Mittwoch 06:00. **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08.) weiterhin oben, `w35`, `w36` und `w37` antworten HTTP **404**; der reale Digest-Rückstand bleibt **zwei Wochen** (W35, W36). **Platform-Release-Notes** byte-identisch (**103914 Bytes**) über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **neunten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Feiertagspause und widerlegte Montags-Prognose). — Vorheriger **Crawl 07.09. 18:00 UTC — sechster Lauf in Folge ohne Bewegung; das prognostizierte Release-Fenster ist seit einer Stunde offen.** Die CLI steht unverändert auf **v2.1.263** (npm-Publish 06.09. 02:07:58 UTC, GitHub-Release 02:54:20 UTC); ein **v2.1.264 gibt es weiterhin nicht** — die npm-Version antwortet HTTP **404**, ebenso die Git-Refs `v2.1.264` und `v2.1.265`; `time.modified` steht unverändert auf 06.09. **02:53:41 UTC**, die Tags sind still (`latest` = `next` = **2.1.263**, `stable` unverändert **2.1.236**). `CHANGELOG.md` ist byte-identisch zu den fünf Vorläufen (**630840 Bytes**), die Releases-API listet deckungsgleich v2.1.263 an der Spitze (60-Byte-Platzhalter-Body, veröffentlicht von `ashwin-ant`). `v2.1.262` bleibt HTTP **404**. Die Lücke seit dem letzten Publish beträgt **39,9 Stunden**. **Zur Prognose:** 07.09. 18:00 UTC ist **Montag, 11:00 Uhr US-Pazifikzeit** — das im Sonntags-Lauf prognostizierte Fenster für v2.1.264 (Montag **17:00–22:30 UTC**) ist seit einer Stunde offen und in dieser ersten Stunde ist nichts publiziert worden; der Median des ersten Montags-Publishes seit 01.07. liegt bei **13:56 PT = 20:56 UTC**, also noch knapp drei Stunden voraus. Die Prognose ist damit **weder bestätigt noch widerlegt**, entschieden wird sie beim Lauf am Dienstag 06:00. **Fund dieses Laufs — npm läuft der GitHub-Releases-Seite systematisch voraus, und der Vorsprung schrumpft:** Der Abgleich aller **100 gepaarten Releases** (v2.1.142–v2.1.263, npm-`time` gegen `published_at` der Releases-API) ergibt in **100 von 100 Fällen** dieselbe Reihenfolge — erst npm, dann GitHub, **kein einziger Fall umgekehrt**. Der Verzug beträgt im **Median 144,5 Minuten** (2 h 25), im Minimum **8 Minuten** (v2.1.258) und im Maximum **739 Minuten = 12,3 Stunden** (v2.1.200, 03.07.); nur **8 von 100** Releases erschienen binnen einer halben Stunde auf GitHub, **35 von 100** brauchten länger als drei Stunden, und **19 von 100** rutschten dabei über die UTC-Tagesgrenze. Der Verzug halbiert sich über die Monate hinweg: Median **194 min im Mai**, 169 im Juni, 146 im Juli, **121 im August** und **59 min im September** (letzte 20 Releases: 99 min) — die Veröffentlichungskette wird sichtbar automatisiert. Praktische Folge: Ein Watcher, der nur die Releases-API abfragt, ist bei jedem Release im Median **zweieinhalb Stunden blind** und im Extremfall einen halben Tag; in einem 6-Stunden-Raster fällt ein Release dadurch regelmäßig einen ganzen Zyklus später auf. Zusammen mit dem Fund des 12:00-Laufs (vier Versionen ohne Release überhaupt) heißt das: **npm ist die einzige belastbare Quelle für „ist es draußen“, GitHub nur für „was steht drin“.** **What's-New** byte-identisch (**14051 Bytes**), Week 34 (17.–21.08.) weiterhin oben, `w35`, `w36` und `w37` antworten HTTP **404**; der reale Digest-Rückstand bleibt **zwei Wochen** (W35, W36). **Platform-Release-Notes** byte-identisch (**103914 Bytes**) über `…/release-notes/overview.md`; Top-Eintrag weiter **03.09.** (`ant`-CLI 1.30.0 mit `ant apply`), darunter unverändert der Block vom 01.09. zu Claude Fable 5.1 und Mythos 5.1. **Blog** byte-identisch (**763730 Bytes**), dieselben fünfzehn Slugs in gleicher Reihenfolge; der `Last Published`-Stempel steht den **achten Lauf in Folge** auf **03.09. 17:24 UTC**. Alle vier Quellen plus npm-Registry, Releases-API und Git-Refs per `curl -L --retry 2` im ersten Versuch, keine leeren Antworten. **Neue Einträge: 1** (Vorlauf npm → GitHub-Release). — Ältere Crawl-Historie in den Git-Commits.)
